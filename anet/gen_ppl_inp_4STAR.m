function inp = gen_ppl_inp_4STAR(star)
% inp = gen_ppl_inp_4STAR
...
if isfield(star,'good_ppl')
    sat_rad = sat_rad(star.good_ppl,star.aeronetcols(2,star.vis_pix));
elseif isfield(star,'good_alm')
    sat_rad = sat_rad(star.good_alm,star.aeronetcols(2,star.vis_pix));
else
    !Not PPL or ALM?
    return
end
geom.WAVE = star.w(star.aeronetcols(2,star.vis_pix));
aods = mean(star.sun_aods.aod(:,:));
if ~isfield(star,'bdrf')
    star.bdrf =[...
...
for w= length(geom.WAVE):-1:1
    geom.WAVE_(w).albedo = fill_albedo(geom.WAVE(w),star.bdrf);
    ...
    if ~isfield(star,'PWV')
        ...
        geom.WAVE_(w).PWV = mean(star.PWV);
    end
    % Now we're gettting there!
    % We could screen the values for this wavelength here...
%     geom.WAVE_(w).meas = [aods(w), sat_rad(:,w)'./1000];
    layer_alt = (unique(round(0.1.*(star.Alt(star.good_ppl)-mean(star.Alt(star.good_ppl))))./0.1) ...
        + round(0.1.*mean(star.Alt(star.good_ppl)))./0.1)./1000; % 10 meter resolution.
    geom.WAVE_(w).HLYR =  layer_alt; % convert to km.
    layer_alts = ((round(0.1.*(star.Alt(star.good_ppl)-mean(star.Alt(star.good_ppl))))./0.1) ...
        + round(0.1.*mean(star.Alt(star.good_ppl)))./0.1)/1000;
    ...
    for ly = 1: length(layer_alt);
        good_lyr = star.good_ppl;
        good_lyr(star.good_ppl) = layer_alts==layer_alts(ly);
        sunel = unique(round(10.*(star.sunel(star.good_ppl&good_lyr)-mean(star.sunel(star.good_ppl&good_lyr))))./10)...
            + round(10.*mean(star.sunel(star.good_ppl&good_lyr)))./10; % 0.1 degree resolution
        sunza = 90-sunel;
        sunels =(round(10.*(star.sunel(star.good_ppl&good_lyr)-mean(star.sunel(star.good_ppl&good_lyr))))./10)...
            + round(10.*mean(star.sunel(star.good_ppl&good_lyr)))./10;
        sunzas = 90-sunels;
        geom.WAVE_(w).HLYR_(ly).SZA = sunza;
        for sz = 1: length(sunza)
            good_sza = good_lyr;
            good_sza(good_lyr) = (sunzas ==sunzas(sz));
            % star.El_true
            OZAs = [90-star.El_true(star.good_ppl&good_sza)']';
            geom.WAVE_(w).HLYR_(ly).SZA_(sz).OZA = OZAs;
            phis = abs(round(10.*star.sunaz(star.good_ppl&good_sza) - 10.*star.Az_true(star.good_ppl&good_sza)))./10;
            %%
            % If normal PPL, sort by OZA.
            % If normal ALM, sort by PHI
            % Else, sort by both?  We need a 1:1 mapping between meas and
            % the angular pairs.  So, whatever we sort by we need to
            % identify the measurements consistent with the angles.
            if star.isPPL
                % There is only one PHI per Observatioon Zenith Angle (OZA).  This is PPL
                NPHI = 1;
                PHI = inp.geom.WAVE_(IW).HLYR_(ITAU).SZA_(ISZA).PHI(zi);
                geom.WAVE_(1).HLYR_(1).SZA_(1).PHI = PHI;
            else
                ...
                geom.WAVE_(1).HLYR_(1).SZA_(1).OZA_(1).PHI = [0.0, PHI]
            end
            for oz = 1:length(OZAs)
                ...
                geom.WAVE_(w).meas = [aods(w), sat_rad(:,w)'./1000];
               %geom.WAVE_(IW).HLYR_(ITAU).SZA_(ISZA).PHI
% The following logic on gen_aip_cimel_strings distinguishes PPL from ALM.
% 
...
%                     NPHI = length(inp.geom.WAVE_(IW).HLYR_(ITAU).SZA_(ISZA).OZA_(zi).PHI);
%                     PHI = inp.geom.WAVE_(IW).HLYR_(ITAU).SZA_(ISZA).OZA_(zi).PHI;
%                     % Many PHI per OZA, this is ALM scan
%                 end
               
%The lines below were copied from "gen_ppl_inp_cimel" just as example
...
inp.Latitude = mean(star.Lat(star.good_ppl));
...
for m = 1:length(modis_wave)
    alb = fill_albedo(modis_wave(m),star.bdrf);
    inp.mod_bdrf(m) = {[modis_wave(m)   alb]};
end
if star.isPPL
