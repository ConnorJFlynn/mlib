function aod_SD_nmode = gen_naod_bases
% Connor, 2020-08-??
...
% This differs from gen_aod_bases in that it normalized the PSDs and AOD
% spectra to unity so fit coefs will immediately reflect the relative
% signficance on AOD.
...
n_i = [1.55,1.55,1.55,1.55] + j.*[0.022,0.022,0.022,0.022];
...
wl = logspace(log10(300),log10(2100),100);
...
vmr = [logspace(log10(.1),log10(.35),7),logspace(log10(.9),log10(5.5),6)];
v_std = [.375,.4,.45 ,.45 .5, .5, .55, .5, .55, .55, .55, .55, .5];
...
clear aod_mode
naod_md = aod_md ./ (max(aod_md,[],2)*ones(size(wl)));
aod_mode.vmr = vmr;
aod_mode.std = v_std;
aod_mode.log_wl = log(wl');
aod_mode.log_modes = log(aod_md');
aod_mode.log_nmodes = log(naod_md');
...
save([strrep(which('fit_aod_basis'),'fit_aod_basis.m','aod_SD_nmode.mat')],'-struct','aod_mode');

clear aod_mode; aod_SD_nmode = load([strrep(which('fit_aod_basis'),'fit_aod_basis.m','aod_SD_nmode.mat')]);

figure; plot(exp(aod_SD_nmode.log_wl), (exp(aod_SD_nmode.log_nmodes)), '-'); logx; logy
legend([string(round(1e4.*(aod_SD_nmode.vmr))/10)]+" nm");

faod = exp(aod_SD_nmode.log_nmodes(:,3)); 
caod = (exp(aod_SD_nmode.log_nmodes(:,11))+exp(aod_SD_nmode.log_nmodes(:,12)))./100;
figure; plot(exp(aod_SD_nmode.log_wl), caod + faod, '-'); logx; logy;
return
