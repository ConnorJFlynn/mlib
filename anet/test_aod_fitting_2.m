...
%The vmr and std span the observed PDFs from SGP from 2017-2020.  
...
vmr = [logspace(log10(.1),log10(.25),4),logspace(
v_std = [.375,.45, .6, .5, .55, .5];
...
for ci = length(cen):-1:1
    bin_rad(ci,:) = logspace(log10(vmr(ci)./4),log10(vmr(ci)*3),30);
    PSD(ci,:) = LnNormal(bin_rad(ci,:), vmr(ci), dev(ci));    
    ss(1) = subplot(1,2,1);
    %     plot(bin_rad, PSD(ci,:),'-',bins, PSD2(ci,:),'-');hold('on');logx;
    plot(bin_rad(ci,:), PSD(ci,:),'-');hold('on');logx;
    [aod_md(ci,:), ssa_md(ci,:)] = anet_mie(wl,ni, bin_rad(ci,:), PSD(ci,:));
    ss(2) = subplot(1,2,2);
