function [inp,line_num] = gen_sky_inp_4STAR(star,good_sky_)
...
if sum(good_sky)==0
   ...
   if size(good_sky,2)==1
    good_sky = repmat(good_sky,size(star.wl_ii))&(~isnan(sat_rad));
   else
      good_sky = good_sky&(~isnan(sat_rad));
   end
    
    if isfield(star.toggle, 'sky_dOD')
        star.dOD = star.toggle.sky_dOD;
    else
        star.dOD = 0;
    end
...
%     end
    ...
    %    good_sky = repmat(good_sky,size(star.wl_ii))&(~isnan(sat_rad));
    % end
    % Now apply anet screens to remove SA < 3.5, any
    ...
    if length(star.wl_ii)~=length(star.TOD)
        tod = star.TOD(star.wl_ii);
        aod = star.AOD(star.wl_ii);
        agod = star.AGOD(star.wl_ii);
    else
        tod = star.TOD;
        aod = star.AOD;
        agod = star.AGOD;
    ...
    if ~isfield(star,'brdf')
        if isfield(star,'sfc_alb')
            ...
            [flight_alb, ~] = get_ssfr_flight_albedo(mean(star.t(good_sky(:,1))),[0.470,0.555,0.659,0.858,1.24, 1.64, 2.13]');
        end
    ...
    for w= length(geom.WAVE):-1:1
        ...
        res = 1./150;
        layer_alt = (unique(round(res.*(star.Alt(good_sky(:,w))-mean(star.Alt(good_sky(:,w)))))./res) ...
            + round(res.*mean(star.Alt(good_sky(:,w))))./res)./1000; % 25 meter resolution.
    ...
    % The wind_speed and land_fraction are used to infer the surface albedo
    % of the ocean. We don't use this when flying with SSFR and it is not
    % clear how we'd use it otherwise unless we're flying at low altitude
    % similar to a ground installation.
    ...
    % fname = ['4STAR_.input'];
    pname_inp = 'C:\z_4STAR\work_2aaa__\4STAR_\';
    if ~isadir(pname_inp) mkdir([pname_inp]); end
