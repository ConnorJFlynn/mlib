...
for H = gv
    ...
    for L=1:3
        QI_1 = (QIS(H, 1, 13:22, :, L, m, n)); 
        if all(all(isNaN(QI_1))) &&n~=1
            QI_1 = QIS(H, 1, 13:22, :, L, m, 1);
        end
        QI_2 = (QIS(H, 3, 13:22, :, L, m, n)); 
        if all(all(isNaN(QI_2))) &&n~=1
            QI_2 = QIS(H, 3, 13:22, :, L, m, 1);
        end
        ...
        new_QIS = [squeeze(QI_1); squeeze(QI_2); squeeze(QI_3); squeeze(QI_4)];        
        ax(1) = subplot(3,1,L);
