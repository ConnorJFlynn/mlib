function [QIS, MER, RMS, VERS, files] = read_all_SITA_Sept_samplefiles_v5(dpath)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
QIS(:,2,:,2,4,:,:) = QIS(:,2,:,1,4,:,:); %Propagate NLB SSP0 into SSP1
QIS(:,2,:,3,4,:,:) = QIS(:,2,:,1,4,:,:); %Propagate NLB SSP0 into SSP2
MER(:,2,:,2,4,:,:) = MER(:,2,:,1,4,:,:); MER(:,2,:,3,4,:,:) = MER(:,2,:,1,4,:,:); % NLB MER
RMS(:,2,:,2,4,:,:) = RMS(:,2,:,1,4,:,:); RMS(:,2,:,3,4,:,:) = RMS(:,2,:,1,4,:,:); % NLB RMS
QIS(:,5,:,2,4,:,:) = QIS(:,5,:,1,4,:,:); % Propagate OUX SSP0 into SSP1
QIS(:,5,:,3,4,:,:) = QIS(:,5,:,1,4,:,:); % SSP0 into SSP2
MER(:,5,:,2,4,:,:) = MER(:,5,:,1,4,:,:); MER(:,5,:,3,4,:,:) = MER(:,5,:,1,4,:,:); % OUX MER
RMS(:,5,:,2,4,:,:) = RMS(:,5,:,1,4,:,:); RMS(:,5,:,3,4,:,:) = RMS(:,5,:,1,4,:,:); % OUX RMS

% Propagate NGE SSP2 into SSG3
% From Reed Espinosa 2020-09-01
% One other thing I noticed is that my NGE SPG3 lidar-only results are also missing 
% (at all wavelengths). It should be okay to duplicate my SSP2 lidar-only into SSG3, 
% those two runs would have been identical (lidar has no altitude or geometry 
% dependence this time). 

QIS(:,4,:,4,:,:,:) = QIS(:,4,:,3,:,:,:); % QI(:,4,1,4,1,2,1)= QI(:,4,1,3,1,2,1);
MER(:,4,:,4,:,:,:) = MER(:,4,:,3,:,:,:); 
RMS(:,4,:,4,:,:,:) = RMS(:,4,:,3,:,:,:); 

%Propagate NLP SSP2 into SSG3
QIS(:,1,:,4,:,:,:) = QIS(:,1,:,3,:,:,:); % QI(:,4,1,4,1,2,1)= QI(:,4,1,3,1,2,1);
MER(:,1,:,4,:,:,:) = MER(:,1,:,3,:,:,:); 
RMS(:,1,:,4,:,:,:) = RMS(:,1,:,3,:,:,:);

% ... and also from Reed...
% Also, it looks like there is something wrong with my NGE GV11 AEFRF Column. 
% I’ll try to dig into it later but for now would it be possible to just 
% comment that variable out?
QIS(11,4,:,:,:,:,:) = NaN; 
MER(11,4,:,:,:,:,:) = NaN; 
RMS(11,4,:,:,:,:,:) = NaN; 
