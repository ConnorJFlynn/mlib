function plot_ACCP_cases_V1(QIS,MER,RMS,gv,sfc,reso,pptname)
% plot_AACP_cases_V1(QIS,MER,RMS,gv,srfc,res)
...
% typ(16:20) "DRS6c", "DRS6d", "DRS6e", "DRS6f","DRSg", 
% typ(21:22) "DRS6h", "DRS6i"];


    for L=1:3
        % NGE_DRS(grp=4,type 13:22))
        QI_1 = (QIS(H, 4, 13:22, :, L, m, n)); 
        if all(all(isNaN(QI_1))) &&n~=1
            QI_1 = QIS(H, 4, 13:22, :, L, m, 1);
        end
        % NLB_ICA (grp=2, type(3:12)
        QI_2 = (QIS(H, 2, 3:12, :, L, m, n)); 
        if all(all(isNaN(QI_2))) &&n~=1
            QI_2 = QIS(H, 2, 3:12, :, L, m, 1);
        end
%         QI_3 = (QIS(H, 4, 13:22, :, L, m, n)); 
%         if all(all(isNaN(QI_3))) &&n~=1
%             QI_3 = QIS(H, 4, 13:22, :, L, m, 1);
%         end
%         QI_4 = (QIS(H, 5, 13:22, :, L, m, n)); 
%         if all(all(isNaN(QI_4))) &&n~=1
%             QI_4 = QIS(H, 5, 13:22, :, L, m, 1);
%         end            
        
        
        new_QIS = [squeeze(QIS(H, 1, 13:22, :, L, m, n));squeeze(QIS(H, 3, 13:22, :, L, m, n))];
        
        new_QIS = [squeeze(QI_1); squeeze(QI_2)];        
        ax(1) = subplot(3,1,L);
        ...
        set(ax,'XTick',[1:40]);
        set(ax,'XTickLabel',{[group{4},' DRS all'],'6a','6b','6c','6d','6e','6f','6g','6h','6i',...
    ...
    png_out = ['GV[',num2str(H),']_',srfc{m},'.cases.V',sprintf('%02.0f',10.*dv),'.png'];
