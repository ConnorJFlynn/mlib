...
clear QIS MER RMS
[QIS, MER, RMS] = read_all_ACCP_8K_samplefiles_v4(ACCP_set);
...
% gv = 22;
n_str = '';n = 0;
pptname = [ACCP_pngs,'..',filesep,'ACCP_3x3.QMR.v5.',datestr(now,'yyyymmdd'),n_str,'.ppt'];
while isafile(pptname)
    n = n+1; n_str = num2str(n);
    pptname = [ACCP_pngs,'..',filesep,'ACCP_3x3.QMR.v5.',datestr(now,'yyyymmdd'),n_str,'.pptx'];
end

plot_ACCP_QMR_V5(QIS,MER,RMS,gv,'LNDD','RES1', pptname);
plot_ACCP_QMR_V5(QIS,MER,RMS,gv,'LNDV','RES1', pptname)
plot_ACCP_QMR_V5(QIS,MER,RMS,gv,'OCEN','RES1', pptname);

pptname = [ACCP_pngs,'..',filesep,'ACCP_3x1.Qall_6a-i.v2.',datestr(now,'yyyymmdd'),n_str,'.pptx'];
plot_ACCP_cases_V2(QIS,MER,RMS,gv,'LNDD','RES1', pptname);
plot_ACCP_cases_V2(QIS,MER,RMS,gv,'LNDV','RES1', pptname);
