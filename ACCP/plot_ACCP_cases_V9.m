function plot_ACCP_cases_V9(QIS,MER,RMS,gv,sfc,reso,pptname)
% plot_AACP_cases_V8(QIS,MER,RMS,gv,srfc,res)
...
% V5: Added NLS for DRS
% V6: ? Forgot to capture reason for version change?
% V7: fixed index error (was "3" instead of "1" on line 97)
% V8: commenting out conditional resolution.  Use supplied reso, default to RES1
% V9: Adding RES5 for GV48-53, else RES1
dv = 9.0;
...
% typ(21:22) "DRS6h", "DRS6i"];
    ...
    for L=1:3
        ...
        QI_0 = (QIS(H, 1, 13:22, :, L, m, n)); 
        if all(all(isNaN(QI_0))) &&n~=1
            QI_0 = QIS(H, 1, 13:22, :, L, m, 1);
        end
...
%         new_QIS = [squeeze(QIS(H, 1, 13:22, :, L, m, n));squeeze(QIS(H, 3, 13:22, :, L, m, n))];
        ...
        new_QIS = [squeeze(QI_0);squeeze(QI_1); squeeze(QI_2); squeeze(QI_3); squeeze(QI_4); squeeze(QI_5)];        
        ax(1) = subplot(3,1,L);
        ...
        set(ax,'XTickLabel',{...
            [group{1},' DRS all'],'6a','6b','6c','6d','6e','6f','6g','6h','6i',...
            [group{3},' DRS all'],'6a','6b','6c','6d','6e','6f','6g','6h','6i',...
