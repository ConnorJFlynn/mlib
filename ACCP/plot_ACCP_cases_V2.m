...
png_out = ['GV[n]_[SRFC].[GRP].all_6a-i.Vvv.png']; %ACCP_3x1.cases
...
%         new_QIS = [squeeze(QIS(H, 1, 13:22, :, L, m, n));squeeze(QIS(H, 3, 13:22, :, L, m, n))];
        ...
        new_QIS = [squeeze(QI_1); squeeze(QI_2); squeeze(QI_3)];        
        ax(1) = subplot(3,1,L);
    ...
    png_out = ['GV[',num2str(H),']_',srfc{m},'.all_6a-i.V',sprintf('%02.0f',10.*dv),'.png'];
