function plot_OQS_xprime_2x2_v1(OQS_p, OQS)
% plot_OQS_xprime_2x2_v1(OQS_p, OQS)
% Compare prime and nonprime results provided by Kathy NLP and Sharon NLB
% v1: Puzzling NaNs in NLP non-prime?
...
sq(1) = subplot(2,2,1); hold('off');% for NAD Layr
bb = bar([1:length(layr)],squeeze(OQS_p(layr,:,1,srfc,2)),'barwidth',1,'edgecolor','none'); ylim([-.15,1]);
hold('on');sq(1).ColorOrderIndex = 1;
bar([1:length(layr)],squeeze(OQS(layr,:,1,srfc,2))-squeeze(OQS_p(layr,:,1,srfc,2)),'barwidth',.3,'edgecolor','k','facecolor','k'); ylim([-.1,1]);
...
title ('Layer-resolved GVs  [thin bars = nonprime-prime]');

sq(2) = subplot(2,2,2); hold('off');%for NAD Prof
bar([1:length(prof)],squeeze(OQS_p(prof,:,1,srfc,2)),'barwidth',1,'edgecolor','none'); ylim([-.1,1]);
hold('on');sq(2).ColorOrderIndex = 1; 
lg = legend('SSP0','SSP1','SSP2','SSG3','location','northwest');
bar([1:length(prof)],squeeze(OQS(prof,:,1,srfc,2))-squeeze(OQS_p(prof,:,1,srfc,2)),'barwidth',.3,'edgecolor','k','facecolor','k'); ylim([-.1,1]);
...
title ('Profile-resolved GVs  [thin bars = nonprime-prime]');

sq(3) = subplot(2,2,3); hold('off');% for NAN Layr
bar([1:length(layr)],squeeze(OQS_p(layr,:,2,srfc,2)),'barwidth',1,'edgecolor','none'); ylim([-0.1,1]);
hold('on');sq(3).ColorOrderIndex = 1;
bar([1:length(layr)],squeeze(OQS(layr,:,2,srfc,2))-squeeze(OQS_p(layr,:,2,srfc,2)),'barwidth',.3,'edgecolor','k','facecolor','k'); ylim([-0.1,1]);
...
set(gca,'position',get(gca,'position')+nudge);
sq(4) = subplot(2,2,4); hold('off');%for NAN Prof
bar([1:length(prof)],squeeze(OQS_p(prof,:,2,srfc,2)),'barwidth',1,'edgecolor','none'); ylim([-0.1,1]);
hold('on');sq(4).ColorOrderIndex = 1;
bar([1:length(prof)],squeeze(OQS(prof,:,2,srfc,2))-squeeze(OQS_p(prof,:,2,srfc,2)),'barwidth',.3,'edgecolor','k','facecolor','k'); ylim([-0.1,1]);
...
n_str = '';n = 1;
fname = [ACCP_pngs,'OQS_and_prime_',SFC{srfc},'.png'];
while isafile(fname)
    n = n+1; n_str = ['.',num2str(n)];
    fname = [ACCP_pngs,'OQS_and_prime_',SFC{srfc},n_str,'.png'];
end
saveas(gcf,fname); saveas(gcf,strrep(fname,'.png','.fig'));

pause(2)
close(gcf);
    %OQS_p_Layer_Ocean.png
end
...
sq(1) = subplot(2,2,1); hold('off');% for NAD Layr
bar([1:length(layr)],squeeze(OQS_p(layr,:,1,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(1).ColorOrderIndex = 1;
...
yl2 = ylabel({'NAD =';'(NADLC1+NADBC0)/2'});
% yl2 = ylabel('NAD');set(yl2,'rotation',0,'position',[0,.45]);
title ('Layer-resolved GVs  [faint bars for non-prime]');

sq(2) = subplot(2,2,2); hold('off');%for NAD Prof
bar([1:length(prof)],squeeze(OQS_p(prof,:,1,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
lg = legend('SSP0','SSP1','SSP2','SSG3','location','northwest');
hold('on');sq(2).ColorOrderIndex = 1;
...
yl1 = ylabel({'NAD =';'(NADLC1+NADBC0)/2'});
% yl1 = ylabel('NAD');set(yl1,'rotation',0,'position',[8.9862    0.54]);
title ('Profile-resolved GVs  [faint bars for non-prime]');

sq(3) = subplot(2,2,3); hold('off');% for NAN Layr
bar([1:length(layr)],squeeze(OQS_p(layr,:,2,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(3).ColorOrderIndex = 1;
bar([1:length(layr)],squeeze(OQS(layr,:,2,srfc,2)),'barwidth',1,'edgecolor','none','facealpha',.4); 
ylim([-.05,1]);
% set(gca,'position',[0.0669     0.421    0.3753   0.2365]);
set(gca,'xtick',[1:length(layr)]);set(gca, 'ytick',[0:.2:1])
set(gca,'XTickLabel',layr_x); %set(sql(2),'xaxislocation','top');
...
sq(4) = subplot(2,2,4); hold('off');%for NAN Prof
bar([1:length(prof)],squeeze(OQS_p(prof,:,2,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(4).ColorOrderIndex = 1;
bar([1:length(prof)],squeeze(OQS(prof,:,2,srfc,2)),'barwidth',1,'edgecolor','none','facealpha',.4); 
ylim([-.05,1]);
% set(gca,'position',[0.4558    0.421    0.4792   0.2365]);
set(gca,'xtick',[1:length(prof)]);set(gca, 'ytick',[0:.2:1])
set(gca,'XTickLabel',prof_x); %set(sql(2),'xaxislocation','top');
...
ylim(sq,[-.05,1]);
mt = mtit(['Objective Weighted Scores w/wo prime: ',SFC{srfc}]);
set(mt.th,'fontsize',15,'position',[0.5000    1.0400]);
n_str = '';n = 1;
fname = [ACCP_pngs,'OQS_prime_alpha_',SFC{srfc},'.png'];
while isafile(fname)
    n = n+1; n_str = ['.',num2str(n)];
    fname = [ACCP_pngs,'OQS_prime_alpha_',SFC{srfc},n_str,'.png'];
end
saveas(gcf,fname); saveas(gcf,strrep(fname,'.png','.fig'));
pause(2)
close(gcf);
...
o=2; clear sq
sq(1)= subplot(2,1,1);
bb = bar([1:length(lay)],squeeze(OQS_p(lay,:,1,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(1).ColorOrderIndex = 1;
bar([1:length(lay)],squeeze(OQS(lay,:,1,srfc,2)),'barwidth',1,'edgecolor','none','facealpha',.4); ylim([0,1]);
...
sq(2)= subplot(2,1,2);
bb = bar([1:length(lay)],squeeze(OQS_p(lay,:,2,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(2).ColorOrderIndex = 1;
bar([1:length(lay)],squeeze(OQS(lay,:,2,srfc,2)),'barwidth',1,'edgecolor','none','facealpha',.4); ylim([0,1]);
...
nn = 1; n_str = [];
png_out = ['QO6_prime.layer-resolved.',SFC{srfc},n_str,'.png'];
...
bb = bar([1:length(prf)],squeeze(OQS_p(prf,:,1,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(1).ColorOrderIndex = 1;
bar([1:length(prf)],squeeze(OQS(prf,:,1,srfc,2)),'barwidth',1,'edgecolor','none','facealpha',.4); ylim([0,1]);
% set(gca,'position',[0.1300    0.69    0.7750    0.24]);
...
sq(2)= subplot(2,1,2); hold('off');
bb = bar([1:length(prf)],squeeze(OQS_p(prf,:,2,srfc,2)),'barwidth',.5,'edgecolor','k'); ylim([0,1]);
hold('on');sq(2).ColorOrderIndex = 1;
bar([1:length(prf)],squeeze(OQS(prf,:,2,srfc,2)),'barwidth',1,'edgecolor','none','facealpha',.4); ylim([0,1]);
%  set(gca,'position',[0.1300    0.4166    0.7750    0.24]);
set(gca,'xtick',[1:length(prf)]);set(gca, 'ytick',[0:.2:1]);
...
linkaxes(sq,'x')
nn = 1; n_str = [];
png_out = ['QO6_prime.profile-resolved.',SFC{srfc},n_str,'.png'];
while isafile([ACCP_pngs,png_out])
    nn = nn+1; n_str = ['_',num2str(nn)];
    png_out = ['QO6_prime.profile-resolved.',SFC{srfc},n_str,'.png'];
end
...
% bar([1:length(layr)],mean(squeeze(OQS_p(layr,:,1,srfc,:)),3),'barwidth',1,'edgecolor','none'); ylim([0,1]);
% hold(sq(1),'on');  sq(1).ColorOrderIndex = 1; 
% bar([1:length(layr)],max(squeeze(OQS_p(layr,:,1,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% hold(sq(1),'on');  sq(1).ColorOrderIndex = 1; 
% bar([1:length(layr)],min(squeeze(OQS_p(layr,:,1,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% 
% % set(gca,'position',[0.6293    0.7357    0.3301    0.18]);
% set(gca,'xtick',[1:length(layr)]);set(gca, 'ytick',[0:.2:1])
% set(gca,'XTickLabel',xt); %set(sql(2),'xaxislocation','top');
...
% bar([1:length(prof)],mean(squeeze(OQS_p(prof,:,1,srfc,:)),3),'barwidth',1,'edgecolor','none'); ylim([0,1]);
% hold(sq(2),'on');  sq(2).ColorOrderIndex = 1; 
% bar([1:length(prof)],max(squeeze(OQS_p(prof,:,1,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% hold(sq(2),'on');  sq(2).ColorOrderIndex = 1; 
% bar([1:length(prof)],min(squeeze(OQS_p(prof,:,1,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% 
% % set(gca,'position',[0.6293    0.7357    0.3301    0.18]);
% set(gca,'xtick',[1:length(prof)]);set(gca, 'ytick',[0:.2:1])
% set(gca,'XTickLabel',xtt); %set(sql(2),'xaxislocation','top');
...
% bar([1:length(layr)],mean(squeeze(OQS_p(layr,:,2,srfc,:)),3),'barwidth',1,'edgecolor','none'); ylim([0,1]);
% hold(sq(3),'on');  sq(3).ColorOrderIndex = 1; 
% bar([1:length(layr)],max(squeeze(OQS_p(layr,:,2,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% hold(sq(3),'on');  sq(3).ColorOrderIndex = 1; 
% bar([1:length(layr)],min(squeeze(OQS_p(layr,:,2,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% 
% % set(gca,'position',[0.6293    0.7357    0.3301    0.18]);
% set(gca,'xtick',[1:length(layr)]);set(gca, 'ytick',[0:.2:1])
% set(gca,'XTickLabel',layr_x); %set(sql(2),'xaxislocation','top');
...
% bar([1:length(prof)],mean(squeeze(OQS_p(prof,:,2,srfc,:)),3),'barwidth',1,'edgecolor','none'); ylim([0,1]);
% hold(sq(4),'on');  sq(4).ColorOrderIndex = 1; 
% bar([1:length(prof)],max(squeeze(OQS_p(prof,:,2,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% hold(sq(4),'on');  sq(4).ColorOrderIndex = 1; 
% bar([1:length(prof)],min(squeeze(OQS_p(prof,:,2,srfc,:)),[],3),'barwidth',1,'edgecolor','k','facecolor','none'); ylim([0,1]);
% 
% % set(gca,'position',[0.6293    0.7357    0.3301    0.18]);
% set(gca,'xtick',[1:length(prof)]);set(gca, 'ytick',[0:.2:1])
% set(gca,'XTickLabel',prof_x); %set(sql(2),'xaxislocation','top');
