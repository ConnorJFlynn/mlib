function ACCP_SITA_Sept_pngs_ppts_v25(gv, gv2, do_plot)
...
% There is a little problem here in that the function "apply_OWCs_C1_v1"
% writes out a mat file with objective-weighted scores, but doesn't know or
% distinguish whether those scores have adjustements applied.  
...
Qall = compute_Qall(QIS); % No objective weighting applied, only mean over objective-specified cases
qnan = isNaN(QIS); len = length(QIS(~qnan));
meanRMSE = compute_Qall(RMS);
...
% The following adjustments and weights logically come later, but have been
% added here for expediency of getting to see these downstream results
% producing all the plots and PPTs.
% How about redoing this with a boolean do_plot field?
%
...
if do_plot(6)
    plot_OQS_wwo_adj_3x2_v2(OQS_C1,OQ_C1);% wi/wo adjustments, transparent overlay
    plot_OQSadj_v2(OQS_C1);
end
