function plot_Qall_C0_C1_v2B_4x2(Qall,pptname)
...
for sfc = 1:length(srfc)
    ...
    set(gca,'XTickLabelRot',0); set(gca,'FontSize',10);
    yl = ylabel('(NADLC0+NADBC0)/2 + adj'); set(yl,'rotation',90); 
    set(yl,'position',[-0.3151    0.5619 ]);
    ...
    yl = ylabel('NADxC_0C_1 Adj'); set(yl,'rotation',90); set(yl,'position',[-0.5, 0.6053]);
    
    ylim(sq,[0,1]);
    mt = mtit(['Mean Quality Scores, NAD, NAN with LC0, LC1:',srfc{sfc}]);
...
% 
    ...
    set(gca,'XTickLabel',layr_x);
    set(gca,'XTickLabelRot',45); set(gca,'FontSize',10);
    yl = ylabel('NANLC_0C_1 Adj'); set(yl,'rotation',90); set(yl,'position',[-.5    0.5000]);       
