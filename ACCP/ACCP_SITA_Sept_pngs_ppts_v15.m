function ACCP_SITA_Sept_pngs_ppts_v15(gv, gv2)
...
OQS = apply_OWCs(QIS,gv); % Apply objective weights before reviewer adjustment
QISadj = apply_Sept_adj_2(OQS);

% Generate comparable Objective-Weighted Q-scores wi/wio adjustments Lidar-Only (for VF group?)
OQSLO = apply_OWCs_LO(QIS,gv);
QISLO_adj = apply_Sept_adj_LO(OQSLO);
% OQS_LO = apply_LO_OWCs(QIS,gv);
 % Apply reviewer adjustements
