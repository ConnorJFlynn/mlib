function OQS = apply_OWCs_LO(QIS,gv)
% OQS_LO = apply_OWCs_LO(QIS,gv)
...
%OQS(obj,gv,obs,sfc,pltf)
    ...
    for OO = 1:4
        OC = OCW(:,OO);
            L = 1; % for NADLC0
            OQ_1 = nanmean(squeeze(QIS(H, 1, DRS_cases,  :,L, m, R)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./mean([OC(land_case);OC(ocen_case)]);
            if all(isNaN(OQ_1)) &&R~=1
...
for OO = 1:4
    for pp = 1:length(PFM)
        NN_ = ''; NN = 1;
        OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'LO_OQS_weighted_for_O',num2str(OO+4),'_',PFM{pp},NN_,'.csv'];
...
out.OQSLO_struct = '%OQSLO(obj,gv,obs,sfc,pltf)';
out.obj = [5:8]; out.gv = [1:65]; out.obs = ["NADL","NAN","OND"]; out.sfc = ["Land","Ocean"]; out.plfm = ["SSP0","SSP1","SSP2","SSG3"];
out.OQS = OQS;
save([getnamedpath('ACCP_set'),'SITA_OQSLO_',datestr(now,'yyyymmdd_HHMM'),'.mat'],'-struct','out');
