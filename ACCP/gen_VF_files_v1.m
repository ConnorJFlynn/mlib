function gen_VF_files_v1(QIS,gv,obs,reso)
% gen_VF_files_v1
% v1: 2020-09-02

% Assume that NAD = (NADLC0+NADBC0)/2, NAN=NANLC0, and OND = ONDPC0
%read in weights table
OCW = ACCP_Obj_Case_Weights;
OC5 = OCW(:,1); OC6 = OCW(:,2);OC7 = OCW(:,3); OC8 = OCW(:,4);
%v3: fixed order of average (DRS_all, ICA_all) relative to cases

if ~isavar('pptname')
    d_now = now; n_str = [];
    pptname = ['ACCP_Q8a-o.',datestr(now,'yyyymmdd'),n_str,'.pptx'];
end
...
% QIS(gv,grp,typ,pltf,obs,srfc,res)
% tic
 %Produce 16 files, one for each OBJ, one for each platform
 obs = ["NADLC0", "NADBC0", "NANLC0","ONDPC0"];
VF_Q_NAD = NaN([65,4,4]);% 65 GV for 3 observation modes (NAD, NAN, OND) for all 4 platform and all 4 objective O5-O8
VF_Q_NAN = VF_Q_NAD; VF_Q_OND = VF_Q_NAD;
...
for H = gv
    ...
    % OC5 are the weights for OC5 for the different cases.
        % OC5(land_case) * QI(:,:,land_case_case1,:,:,:,:,:) would be for land_case1
        % OC5(land_case) * QI(:,:,land_case+case2,:,:,:,:,:) would be for land_case2
        % Can I use nanmean to do it in one shot?  like:
        % VF_QI_O5 = nanmean(squeeze(OC5(land_case) * QI(:,:,land_case*[0,case2],:,:,:,:,:))))
    %NAD
    QI_1 = size(squeeze(QIS(H,2,DRS_cases,:,1,1,n)));
