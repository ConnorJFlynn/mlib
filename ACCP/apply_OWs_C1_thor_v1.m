function [OQS] = apply_OWs_C1_thor_v1(QIS,adj)
...
% NAD from mean of NADBC0 obs[2] and NADLC1 obs[5] not any more
% NAN from mean of NANLC0 obs[3]and NANLC1 obs[6] not any more
...
% v1: Breaking out OBS for Tyler Thorsen
% NAD scores for: (1) lidar-only clear-sky, (2) lidar-only thin-cloud, (3) lidar+polarimeter clear-sky. 
% NAN scores broken down into: (1) lidar-only clear-sky, (2) lidar-only thin-cloud.
% QIS(GV,GRP,TYP,PFM,OBI,SFC,REZ)
% OQS(gv[65],pfm[4],obi[6],sfc[2],obj[4])
if ~isavar('adj')
    adj = [];
end
OBI = ["NADLC0", "NADBC0", "NANLC0", "ONDPC0",...
    "NADLC1","NANLC1","NADLC2","NANLC2"];  
OBI = ["NADLC0","NADLC1","NADBC0", "NANLC0","NANLC1","ONDPC0"];  
pfm = ["SSP0","SSP1","SSP2","SSG3"];
OQS = NaN([65,4,8,2,4]); % obi NADLC0 NADLC1, ADBC0 NANLC0 NANLC1 ONDPC0 OBI([1,5,2,3,6,4]) subseleted at end
...
%             L = 5; % for NADLC1
            ...
            OQS(H,:,L,sfc,OO) = meannonan(new_OQ.*ONAN)./mean(OCWs); % These are mean Qs for this GV and Objective weights.
            end 
...
out.OQS_struct = ['%OQS',adj,'(gv(65),pfm(4),obs(6),sfc(2),obj(4))'];
out.gv = [1:65]; 
out.pfm = ["SSP0","SSP1","SSP2","SSG3"];
out.obs = ["NADLC0, NADLC1, NADBC0, NANLC0, NANLC1, ONDPC0"]; 
out.sfc = ["Land","Ocean"]; 
out.obj = [5:8]; 
out.(['OQS',adj]) = OQS;
save([getnamedpath('VF_out'),'SITA_OQS',adj,'_C0C1_thor_',datestr(dnow,'yyyymmdd_HHMM'),'.mat'],'-v7.3','-struct','out');
