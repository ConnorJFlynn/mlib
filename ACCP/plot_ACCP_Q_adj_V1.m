function plot_ACCP_Q_adj_V1(QIS,gv,reso,pptname)
% plot_ACCP_Q_adj_V1(QIS,MER,RMS,gvreso)
...
% OND = adj(:,[10:13]);
   ...
   [adj] = ACCP_QI_adj_v0; adj_gv = interp1(adj(:,1), [1:length(adj(:,1))],H,'nearest');
    for L=1:3 % obs counter obs = ["NAD", "NAN", "OND"];
        ...
        for m = 1:3 % srfc = ["LNDD", "LNDV", "OCEN"];
            ...
            new_QIS = [newQIS;meanQ; weighted_Q; weighted_Q+adj_L];
            subplot(3,3,m+3*(L-1))
            ...
            set(gca,'XTickLabel',{'NLP-DRS','NLS-DRS','NGE-DRS','OUX-DRS',...
                'NLB-ICA','OUX-ICA','NLS-SPA','NGE-SPA','mean','weighted','adj' })
