function [OQS] = apply_OWCs_C1_v1(QIS,adj,out)
% OQS = apply_OWCs_C1_v1(QIS,out)
...
for OO = 1:4
    for pp = 1:length(pfm)
        NN_ = ''; NN = 1;
        OQS_file = [getnamedpath('VF_out'),'QS_for_O',num2str(OO+4),'_',pfm{pp},'_C0C1',NN_,'.csv'];
...
% toc
% OQS(gv,pfm,obi,sfc,obj(4))
out.OQS_struct = '%OQS_C1(gv(65),pfm(4),obs(3),sfc(2),obj(4))';
out.gv = [1:65]; 
out.pfm = ["SSP0","SSP1","SSP2","SSG3"];
out.obs = ["NAD=mean(NADLC1,NADBC0)","NAN=mean(NANLC0,NANLC1)","OND"]; 
out.sfc = ["Land","Ocean"]; 
out.obj = [5:8]; 
out.OQS = OQS;
save([getnamedpath('VF_out'),'SITA_OQS_C0C1_',datestr(dnow,'yyyymmdd_HHMM'),'.mat'],'-struct','out');
