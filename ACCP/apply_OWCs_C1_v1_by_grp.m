function [OQS] = apply_OWCs_C1_v1_by_grp(QIS,out)
% OQS = apply_OWCs_C1_v1_by_grp(QIS,out)
% Accepts a 7D QIS matrix
% Applies Objective-specific weights based on TYP
% Adds Obj[5-8] dimension while eliminating TYP, and RES dims to produce 5D OQS
...
for OO = 1:4
    for sfc = 1:2
        ...
        for H = gv
            ...
            new_OQ1 = [OQ_1; OQ_5; OQ_2; OQ_3; OQ_4]; % old, newQIS 8x4, so now 5x4?
            % Only use cases where all 4 platforms are reported
            ONAN = double(~isNaN(new_OQ1));
            ONAN(isNaN(new_OQ1)) = NaN; anyNaN = any(isNaN(ONAN)')'; ONAN(anyNaN,:) = NaN;
            meanOQ_NADLC1 = meannonan(new_OQ1.*ONAN)./mean(OCWs); % These are mean Qs for this GV and Objective weights.
            ...
            new_OQ2 = [OQ_1; OQ_5; OQ_2; OQ_3; OQ_4]; % old, newQIS 8x4, so now 5x4?
            % Only use cases where all 4 platforms are reported
            ONAN = double(~isNaN(new_OQ2));
            ONAN(isNaN(new_OQ2)) = NaN; anyNaN = any(isNaN(ONAN)')'; ONAN(anyNaN,:) = NaN;
            meanOQ_NADBC0 = meannonan(new_OQ2.*ONAN)./mean(OCWs); % These are mean Qs for this GV and Objective weights.
            temp = nanmean([meanOQ_NADLC1; meanOQ_NADBC0]);
            tmp2 = NaN([size(new_OQ1),2]);tmp2(:,:,1) = new_OQ1; tmp2(:,:,2) = new_OQ2;
            % OQS(gv,pfm,obi,sfc,obj(4))
            OQS(H,:,:,1,sfc,OO) = [nanmean(tmp2,3)./mean(OCWs); temp];
            ...
            ONAN = double(~isNaN(new_OQ1));
            ONAN(isNaN(new_OQ1)) = NaN; anyNaN = any(isNaN(ONAN)')'; ONAN(anyNaN,:) = NaN;
            meanOQ_NANLC0 = meannonan(new_OQ1.*ONAN)./mean(OCWs); % These are mean Qs for this GV and Objective weights.
            ...
            ONAN = double(~isNaN(new_OQ2));
            ONAN(isNaN(new_OQ2)) = NaN; anyNaN = any(isNaN(ONAN)')'; ONAN(anyNaN,:) = NaN;
            meanOQ_NANLC1 = meannonan(new_OQ2.*ONAN)./mean(OCWs); % These are mean Qs for this GV and Objective weights.
            temp = nanmean([meanOQ_NANLC0; meanOQ_NANLC1]);
            tmp2 = NaN([size(new_OQ1),2]);tmp2(:,:,1) = new_OQ1; tmp2(:,:,2) = new_OQ2;
            OQS(H,:,:,2,sfc,OO) = [nanmean(tmp2,3)./mean(OCWs); temp];
            ...
            new_OQ = [OQ_1; OQ_5; OQ_2; OQ_3; OQ_4]; % old, newQIS 8x4, so now 5x4?
            % Only use cases where all 4 platforms are reported
            ...
            OQS(H,:,:,3,sfc,OO) = [new_OQ./mean(OCWs); meanOQ_ONDPC0];
            %OQS(obj,gv,obs,sfc,pltf
...
for OO = 1:4
    for pp = 1:length(pfm)
        NN_ = ''; NN = 1;
        OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'QS_for_O',num2str(OO+4),'_',pfm{pp},NN_,'.csv'];
...
% toc
% OQS(gv,pfm,obi,sfc,obj(4))
out.OQS_struct = '%OQS(gv(65),grp(5+1),pfm(4),obs(3),sfc(2),obj(4))';
out.gv = [1:65]; out.pfm = ["SSP0","SSP1","SSP2","SSG3"];
% GRP = ["NLP", "NLB", "NLS", "NGE", "OUX"];  
out.grp = ["NLP","NLB","NLS","NGE","OUX","Mean"];
out.obs = ["NAD=mean(NADLC1,NADBC0)","NAN=mean(NANLC0,NANLC1)","OND"]; out.sfc = ["Land","Ocean"]; 
out.obj = [5:8]; 
out.OQS = OQS;
save([getnamedpath('ACCP_set'),'SITA_OQS_C1_by_grp_',datestr(now,'yyyymmdd_HHMM'),'.mat'],'-struct','out');
