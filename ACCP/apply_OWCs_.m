...
for OO = 1:4
    for pp = 1:length(PFM)
        NN_ = ''; NN = 1;
        OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'QS_weighted_for_O',num2str(OO+4),'_',PFM{pp},NN_,'.csv'];
        while isafile(OQS_file)
            NN = NN + 1; NN_ = sprintf('.v%d',NN);
            OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'QS_weighted_for_O',num2str(OO+4),'_',PFM{pp},NN_,'.csv'];
        end
        fid = fopen(string([getnamedpath('ACCP_set'),'..',filesep,'QS_weighted_for_O',num2str(OO+4),'_',PFM{pp},'.csv']),'w+');
