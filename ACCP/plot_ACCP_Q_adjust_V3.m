function [QI_adj] = plot_ACCP_Q_adjust_V3(QIS,gv,reso,pptname)
% plot_ACCP_Q_adj_V3(QIS,gv,reso,pptname)
...
% V2: only use RES5 for GV 48-53, exclude certain GV from  NGE SPA and DRS
% V3: Replaced weighted mean with mean
...
% OND = adj(:,[10:13]);
    ...
    for L=1:3 % obs counter obs = ["NAD", "NAN", "OND"];
        ...
        for m = 1:3 % srfc = ["LNDD", "LNDV", "OCEN"];
            ...
            %QIS(gv,grp,orb/typ,pltf,obs,srfc,res)
            % QIS_adj(H,L,pltf,srfc)
%             QI_adj(H,L,:,m) = weighted_Q + adj_L;
            QI_adj(H,:,L,m) = weighted_Q + adj_L; %L=obs, m = SRFC
            % For QIs in each column (pltf),
            new_QIS = [newQIS; ,meanQ; squeeze(QI_adj(H,:,L,m))];
            ...
            set(gca,'XTickLabel',{'NLP-DRS','NLS-DRS','NGE-DRS','OUX-DRS',...
                'NLB-ICA','OUX-ICA','NLS-SPA','NGE-SPA','mean QS','adjusted' })
