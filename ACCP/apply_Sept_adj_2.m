function OQSadj = apply_Sept_adj_2(OQS);
% OQSadj = apply_OWCs(QIS)
...
for OO = 1:4
    ...
    for pp = 1:length(PFM)
        NN_ = ''; NN = 1;
        OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'OQS',num2str(OO+4),'adj_',PFM{pp},NN_,'.csv'];
...
out.OQSadj_comment = '%OQSadj(obj,gv,obs,sfc,pltf)';
out.obj = [5:8]; out.gv = [1:65]; out.obs = ["NAD=mean(NADL,NADB)","NAN","OND"]; out.sfc = ["Land","Ocean"]; out.plfm = ["SSP0","SSP1","SSP2","SSG3"];
out.OQSadj = OQSadj;
save([getnamedpath('ACCP_set'),'SITA_OQSadj_',datestr(now,'yyyymmdd_HHMM'),'.mat'],'-struct','out');
