function [QIS, MER, RMS, VERS, files] = read_all_SITA_Sept_samplefiles_v9(dpath)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% v3: 2020-09-01, Commenting out NGE GV11 and propagating NGE SSP2 to SSG3
% v4: 2020-09-02, Have reviewed changes at end of file attempting to
% propagate NGE SSP2 into SSG3, and to assign Q for GV 11 to 999 and
% corresponding errors to 0.
% v5: 2020-09-03, Propagate NLP SSP2 into SSG3
% V6: unmask NGE GV 11
% v7: propagate OUX SSP2 to SSG3 for C0,C1,C2
% v8: Introduce LIC
% v9: Update propagation of retrievals 
if ~isavar('dpath')
...
Ocean_cases= ["8a","8b","8c","8g","8h","8k","8l","NR1"]; 
...
%     end
    ...
    if ~isempty(grp_ind)&&~isempty(typ_ind)&&~isempty(plt_ind)&&~isempty(obs_ind)&&~isempty(srfc_ind)&&~isempty(res_ind)
        ...
        if fid>0
            tmp = fgetl(fid); tmp = fgetl(fid); tmp = fgetl(fid); fseek(fid,0,-1);
            if isempty(findstr(tmp,','))
            [blah, endpos] = textscan(fid,'%*f %f %f %f', 'headerlines',2);    
            else
            ...
            bla1 = blah{1}; 
            badQ = (bla1>998.9 & bla1<999.1)|(bla1<-998.9 & bla1>-999.1); 
            bla1(badQ) = NaN;
%             if length(bla1)==75; bla1(46:47) = [];end  %Catch and correct malformed data files
            % Catch cases
            bla2 = blah{2};
            bad = (bla2>998.999&bla2<999.0001)|(bla2<-998.999&bla2>-999.0001);
            if any(badQ&~bad)
                bla2(bla2==0)=999;  
                bad = (bla2>998.999&bla2<999.0001)|(bla2<-998.999&bla2>-999.0001); % Set MER==0 to 999
                if any(badQ&~bad)
...
%             if length(bla2)==75; bla2(46:47) = [];end %Catch and correct malformed data files
            ...
            bla3 = blah{3};
            bad = bla3>(998.999&bla3<999.0001) | (bla3<-998.999&bla3>-999.0001);
            if any(badQ&~bad)
                bla3(bla3==0)=999;  
                bad = (bla3>998.999&bla3<999.0001)|(bla3<-998.999&bla3>-999.0001); %Set RMS==0 to 999
                if any(badQ&~bad)
                    g_str = sprintf('%d ',find(badQ&~bad));
...
% GRP = ["NLP", "NLB", "NLS", "NGE", "OUX", "LIC"];    
% PFM = ["SSP0", "SSP1", "SSP2", "SSG3"];                     %4  
% OBI = ["NADLC0", "NADBC0", "NANLC0", "ONDPC0","NADLC1","NANLC1","NADLC2","NANLC2"]; 
% QIS = NaN([65,length(GRP),length(TYP),length(PFM),length(OBI),length(SFC),length(REZ)]);

%Propagate NLP=1, SSP2 into SSG3
QIS(:,1,:,4,[1 3 5:8],:,:) = QIS(:,1,:,3,[1 3 5:8],:,:); 
MER(:,1,:,4,[1 3 5:8],:,:) = MER(:,1,:,3,[1 3 5:8],:,:); 
RMS(:,1,:,4,[1 3 5:8],:,:) = RMS(:,1,:,3,[1 3 5:8],:,:);

% NLB = 2; Commented out at Sharon's request
% QIS(:,2,:,2,4,:,:) = QIS(:,2,:,1,4,:,:); %Propagate NLB SSP0 into SSP1 for obs=ONDPC0
% MER(:,2,:,2,4,:,:) = MER(:,2,:,1,4,:,:);
% RMS(:,2,:,2,4,:,:) = RMS(:,2,:,1,4,:,:);
% QIS(:,2,:,3,4,:,:) = QIS(:,2,:,1,4,:,:); %Propagate NLB SSP0 into SSP2 for obs=ONDPC0
% MER(:,2,:,3,4,:,:) = MER(:,2,:,1,4,:,:); % NLB MER
% RMS(:,2,:,3,4,:,:) = RMS(:,2,:,1,4,:,:); % NLB RMS

%NLS = 3

%NGE = 4
QIS(:,4,:,4,[1 3 5:8],:,:) = QIS(:,4,:,3,[1 3 5:8],:,:);
MER(:,4,:,4,[1 3 5:8],:,:) = MER(:,4,:,3,[1 3 5:8],:,:); 
RMS(:,4,:,4,[1 3 5:8],:,:) = RMS(:,4,:,3,[1 3 5:8],:,:); 

% OUX = 5
QIS(:,5,:,2,4,:,:) = QIS(:,5,:,1,4,:,:); % Propagate OUX SSP0 into SSP1 for obs=ONDPC0
MER(:,5,:,2,4,:,:) = MER(:,5,:,1,4,:,:);
RMS(:,5,:,2,4,:,:) = RMS(:,5,:,1,4,:,:);
QIS(:,5,:,3,4,:,:) = QIS(:,5,:,1,4,:,:); % SSP0 into SSP2 for obs=ONDPC0
MER(:,5,:,3,4,:,:) = MER(:,5,:,1,4,:,:); 
RMS(:,5,:,3,4,:,:) = RMS(:,5,:,1,4,:,:); 
% QIS(gv,grp,case,pfm,obi,sfc,res);
%Propagate OU SSP2 into SSG3 for NADLC0, NANLC0,NADLC1, NANLC1, NADLC2, NANLC2
QIS(:,5,:,4,[1 3 5:8],:,:) = QIS(:,5,:,3,[1 3 5:8],:,:);
MER(:,5,:,4,[1 3 5:8],:,:) = MER(:,5,:,3,[1 3 5:8],:,:); 
RMS(:,5,:,4,[1 3 5:8],:,:) = RMS(:,5,:,3,[1 3 5:8],:,:);

%LIC = 6, Propagate SSP2 into SSG3 for lidar-only retrievals.
QIS(:,6,:,4,[1 3 5:8],:,:) = QIS(:,6,:,3,[1 3 5:8],:,:);
MER(:,6,:,4,[1 3 5:8],:,:) = MER(:,6,:,3,[1 3 5:8],:,:); 
RMS(:,6,:,4,[1 3 5:8],:,:) = RMS(:,6,:,3,[1 3 5:8],:,:); 


out.QIS = QIS; out.MER = MER; out.RMS = RMS; out.VERS = VERS; out.files = files;
save([dpath,'SITA_Q8aoMR.',datestr(now,'yyyymmdd_HHMMSS'),'.mat'],'-struct','out');
end
