function OQS = apply_LO_OWCs(QIS,gv)
% OQS = apply_LO_OWCs(QIS,gv) Lidar-only results
...
%OQS(obj,gv,obs,sfc,pltf)
    ...
    for OO = 1:4
        OC = OCW(:,OO);
            L = 1; % for NADLC0
            OQ_1 = nanmean(squeeze(QIS(H, 1, DRS_cases,  :,L, m, R)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_1)) &&R~=1
                OQ_1 = nanmean(squeeze(QIS(H, 1, DRS_cases,  :,L, m, 1)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            end
            % We want to take the mean and be left with a length of 4 (pltf)
            OQ_2 = nanmean(squeeze(QIS(H, 3,DRS_cases,  : ,L, m, R)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_2)) &&R~=1
                OQ_2 = nanmean(squeeze(QIS(H, 3,DRS_cases,  : ,L, m, 1)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            end
            OQ_3 = nanmean(squeeze(QIS(H,4,DRS_cases,:,L,m,R)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_3)) && (R~=1)
                OQ_3 = nanmean(squeeze(QIS(H,4,DRS_cases,:,L,m,1)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            end
            OQ_4 = nanmean(squeeze(QIS(H, 5,DRS_cases,:,L,m,R)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_4)) &&R~=1
                OQ_4 = nanmean(squeeze(QIS(H,5,DRS_cases,:,L,m,1)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            end
            OQ_5 = nanmean(squeeze(QIS(H,2, ICA_cases,:,L,m,R)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_5)) &&R~=1
                OQ_5 = nanmean(squeeze(QIS(H,2,ICA_cases,:,L,m,1)).*([OC(land_case);OC(land_case)]*ones([1,4])))./5;
            end
            ...
            %OQS(obj,gv,obs,sfc,pltf)
            meanOQ(meanOQ>1)=1;meanOQ(meanOQ<0)=0;
            OQS(OO,H,1,1,:) = meanOQ;
            %OQS(obj,gv,obs,sfc,pltf
            ...
            meanOQ = meannonan(new_OQ.*ONAN); % These are mean Qs for this GV and O5.
            meanOQ(meanOQ<0)=0; meanO(meanOQ>1)=1;
            OQS(OO,H,3,1,:) = zeros(size(meanOQ));
            %OQS(obj,gv,obs,sfc,pltf
    ...
    for OO = 1:4
        OC = OCW(:,OO);
            L = 1; % for NADLC0
            OQ_1 = nanmean(squeeze(QIS(H, 1, DRS_cases,  :,L, m, R)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_1)) &&R~=1
                OQ_1 = nanmean(squeeze(QIS(H, 1, DRS_cases,  :,L, m, 1)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            end
            % We want to take the mean and be left with a length of 4 (pltf)
            OQ_2 = nanmean(squeeze(QIS(H, 3,DRS_cases,  : ,L, m, R)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_2)) &&R~=1
                OQ_2 = nanmean(squeeze(QIS(H, 3,DRS_cases,  : ,L, m, 1)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            end
            OQ_3 = nanmean(squeeze(QIS(H,4,DRS_cases,:,L,m,R)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_3)) && (R~=1)
                OQ_3 = nanmean(squeeze(QIS(H,4,DRS_cases,:,L,m,1)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            end
            OQ_4 = nanmean(squeeze(QIS(H, 5,DRS_cases,:,L,m,R)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_4)) &&R~=1
                OQ_4 = nanmean(squeeze(QIS(H,5,DRS_cases,:,L,m,1)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            end
            OQ_5 = nanmean(squeeze(QIS(H,2, ICA_cases,:,L,m,R)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            if all(isNaN(OQ_5)) &&R~=1
                OQ_5 = nanmean(squeeze(QIS(H,2,ICA_cases,:,L,m,1)).*([OC(ocen_case);OC(ocen_case)]*ones([1,4])))./5;
            end
            new_OQ = [OQ_1; OQ_2; OQ_3; OQ_4; OQ_5]; % old, newQIS 8x4, so now 5x4?
            % Only use cases where all 4 platforms are reported
            ONAN = double(~isNaN(new_OQ));
            ...
            temp = nanmean([meanOQ_NADLC0; meanOQ_NADBC0]); temp(temp>1) = 1; temp(temp<0)=0;
            OQS(OO,H,1,2,:) = temp;
            %OQS(obj,gv,obs,sfc,pltf
...
for OO = 1:4
    for pp = 1:length(PFM)
        NN_ = ''; NN = 1;
        OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'LidarOnly_QS_weighted_for_O',num2str(OO+4),'_',PFM{pp},NN_,'.csv'];
        while isafile(OQS_file)
            NN = NN + 1; NN_ = sprintf('.v%d',NN);
            OQS_file = [getnamedpath('ACCP_set'),'..',filesep,'LidarOnly_QS_weighted_for_O',num2str(OO+4),'_',PFM{pp},NN_,'.csv'];
        end
        fid = fopen(OQS_file,'w+');
        fprintf(fid,'%s,\t%s,\t%s,\t%s \n','GV_name', 'Lidar Daytime', 'Lidar Nighttime', 'Off nadir no lidar');
        for H = 1:length(gv)
        for ss = length(srfc):-1:1
            fprintf(fid,'%s,\t %2.2f,\t %2.2f,\t %2.2f \n', string([GVnames{H},srfc{ss}]), OQS(OO,H,1,ss,pp), OQS(OO,H,2,ss,pp), zeros(size(OQS(OO,H,3,ss,pp))));
