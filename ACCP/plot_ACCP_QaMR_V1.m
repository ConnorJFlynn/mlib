function plot_ACCP_QaMR_V1(QIS,QI_adj,MER,RMS,gv,sfc,reso,pptname)
% plot_ACCP_QaMR_V1(QIS,QI_adj,MER,RMS,gv,sfc,reso,ppt)
...
png_out = ['GV[n]_[SRFC].3x3.QaMR.png']; %ACCP_3x3.QMR.v5
if ~isavar('pptname')
    dnow = now; n_str = [];
...
for H = gv
    ...
    for L=1:3 % obs counter obs = ["NAD", "NAN", "OND"];
        ...
        new_QIS = [newQIS;meanQ; QI_adj(H,:,L,m)];
        subplot(3,3,1+3*(L-1))
        ...
        set(gca,'XTickLabel',{'NLP-DRS','NLS-DRS','NGE-DRS','OUX-DRS',...
            'NLB-ICA','OUX-ICA','NLS-SPA','NGE-SPA','mean Q','adjusted' })
