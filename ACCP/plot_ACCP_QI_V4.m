function plot_ACCP_QI_V4(QIS,MER,RMS,gv,sfc,reso,pptname)
% plot_AACP_fig_V3(QIS,MER,RMS,gv,sfc,reso)
...
png_out = ['GV[n]_[SRFC].3x3.meanQI.png'];
if ~isavar('pptname')
    pptname = [ACCP_pngs,'..',filesep,'AACP_3x3.meanQI.ppt'];
end
...
for H = gv
    ...
    for L=1:3 
        QI_1 = squeeze(QIS(H, 1, 13, :, L, m, n)); 
        if all(isNaN(QI_1)) &&n~=1
            QI_1 = QIS(H,1,13,:,L,m,1);
        end
        QI_2 = squeeze(QIS(H, 3, 13, :, L, m, n)); 
        if all(isNaN(QI_2)) &&n~=1
            QI_2 = QIS(H,3,13,:,L,m,1);
        end
        QI_3 = squeeze(QIS(H, 4, 13, :, L, m, n)); 
        if all(isNaN(QI_1)) &&n~=1
            QI_3 = QIS(H,4,13,:,L,m,1);
        end
        QI_4 = squeeze(QIS(H, 5, 13, :, L, m, n)); 
        if all(isNaN(QI_2)) &&n~=1
            QI_4 = QIS(H,5,13,:,L,m,1);
        end        
        QI_5 = squeeze(QIS(H, 2, 3, :, L, m, n)); 
        if all(isNaN(QI_1)) &&n~=1
            QI_5 = QIS(H,2,3,:,L,m,1);
        end
        QI_6 = squeeze(QIS(H, 5, 3, :, L, m, n)); 
        if all(isNaN(QI_2)) &&n~=1
            QI_6 = QIS(H,5,3,:,L,m,1);
        end        
        QI_7 = squeeze(QIS(H, 3, 1, :, L, m, n)); 
        if all(isNaN(QI_1)) &&n~=1
            QI_7 = QIS(H,3,1,:,L,m,1);
        end
        QI_8 = squeeze(QIS(H, 4, 1, :, L, m, n)); 
        if all(isNaN(QI_2)) &&n~=1
        ...
        ME_1 = squeeze(MER(H, 1, 13, :, L, m, n)); 
        if all(isNaN(ME_1)) &&n~=1
            ME_1 = MER(H,1,13,:,L,m,1);
        end
        ME_2 = squeeze(MER(H, 3, 13, :, L, m, n)); 
        if all(isNaN(ME_2)) &&n~=1
            ME_2 = MER(H,3,13,:,L,m,1);
        end
        ME_3 = squeeze(MER(H, 4, 13, :, L, m, n)); 
        if all(isNaN(ME_1)) &&n~=1
            ME_3 = MER(H,4,13,:,L,m,1);
        end
        ME_4 = squeeze(MER(H, 5, 13, :, L, m, n)); 
        if all(isNaN(ME_2)) &&n~=1
            ME_4 = MER(H,5,13,:,L,m,1);
        end        
        ME_5 = squeeze(MER(H, 2, 3, :, L, m, n)); 
        if all(isNaN(ME_1)) &&n~=1
            ME_5 = MER(H,2,3,:,L,m,1);
        end
        ME_6 = squeeze(MER(H, 5, 3, :, L, m, n)); 
        if all(isNaN(ME_2)) &&n~=1
            ME_6 = MER(H,5,3,:,L,m,1);
        end        
        ME_7 = squeeze(MER(H, 3, 1, :, L, m, n)); 
        if all(isNaN(ME_1)) &&n~=1
            ME_7 = MER(H,3,1,:,L,m,1);
        end
        ME_8 = squeeze(MER(H, 4, 1, :, L, m, n)); 
        if all(isNaN(ME_2)) &&n~=1
        ...
        RM_1 = squeeze(RMS(H, 1, 13, :, L, m, n)); 
        if all(isNaN(RM_1)) &&n~=1
            RM_1 = RMS(H,1,13,:,L,m,1);
        end
        RM_2 = squeeze(RMS(H, 3, 13, :, L, m, n)); 
        if all(isNaN(RM_2)) &&n~=1
            RM_2 = RMS(H,3,13,:,L,m,1);
        end
        RM_3 = squeeze(RMS(H, 4, 13, :, L, m, n)); 
        if all(isNaN(RM_1)) &&n~=1
            RM_3 = RMS(H,4,13,:,L,m,1);
        end
        RM_4 = squeeze(RMS(H, 5, 13, :, L, m, n)); 
        if all(isNaN(RM_2)) &&n~=1
            RM_4 = RMS(H,5,13,:,L,m,1);
        end        
        RM_5 = squeeze(RMS(H, 2, 3, :, L, m, n)); 
        if all(isNaN(RM_1)) &&n~=1
            RM_5 = RMS(H,2,3,:,L,m,1);
        end
        RM_6 = squeeze(RMS(H, 5, 3, :, L, m, n)); 
        if all(isNaN(RM_2)) &&n~=1
            RM_6 = RMS(H,5,3,:,L,m,1);
        end        
        RM_7 = squeeze(RMS(H, 3, 1, :, L, m, n)); 
        if all(isNaN(RM_1)) &&n~=1
            RM_7 = RMS(H,3,1,:,L,m,1);
        end
        RM_8 = squeeze(RMS(H, 4, 1, :, L, m, n)); 
        if all(isNaN(RM_2)) &&n~=1
    ...
    mt = mtit(['GV[',num2str(H),'] ',strrep(GVnames{H},'_',' '), '; sfc[',srfc{m},']']);
    set(mt.th,'fontsize',15,'position',[0.5000    1.0400]);
    set(cf,'Visible',false);
    png_out = ['GV[',num2str(H),']_[',srfc{m},'].3x3.meanQI.png'];
