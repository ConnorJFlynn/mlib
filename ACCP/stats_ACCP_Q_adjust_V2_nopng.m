function [QI_adj, Q_mean, Q_std,Q_min,Q_max] = stats_ACCP_Q_adjust_V2_nopng(QIS,gv,reso)
% [QI_adj, Qstd,Qmin,Qmax] = stats_ACCP_Q_adj_V2(QIS,MER,RMS,gvreso)
...
Q_std = QI_adj; Q_mean = Q_std;
Q_min= QI_adj; Q_max =QI_adj;
...
% OND = adj(:,[10:13]);
    ...
    for L=1:3 % obs counter obs = ["NAD", "NAN", "OND"];
        ...
        for m = 1:3 % srfc = ["LNDD", "LNDV", "OCEN"];
            ...
            ws = ws.*QNAN;
            Qmean = nanmean(newQIS.*QNAN); Q_mean(H,:,L,m) = Qmean;
            Qstd = nanstd(newQIS.*QNAN); Q_std(H,:,L,m) = Qstd;
            Qmin = nanmin(newQIS.*QNAN); Q_min(H,:,L,m) = Qmin;
            Qmax =nanmax(newQIS.*QNAN); Q_max(H,:,L,m) = Qmax;
            weighted_Q = nanmean(newQIS .* ws)./meannonan(ws);
