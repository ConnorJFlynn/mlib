function plot_SITA_Sept_QMRadj_v2(QIS,MER,RMS,gv,sfc,obs,reso,pptname)
% plot_SITA_Sept_QMRadj_v2(QIS,MER,RMS,gv,sfc,reso,pptname)
...
for H = gv
    ...
    set(cf,'Position',[67 50 1443 741]);
    %     set(cf,'Visible',false); % Comment out to see plots, but slower
    ...
    for LO=1:length(obs) % find OBI indices of supplied obs
        ...
        QI_6 = (squeeze(QIS(H, 6, 63, :, L, m, n)))'; %% LIC-NR1 is QIS(h, 6,  63, :, L, m, n)
        if all(isNaN(QI_6)) &&n~=1
            QI_6 = (squeeze(QIS(H,6,63,:,L,m,1)))'; % GV51,2,3
            % QIS = NaN([73,length(group),length(typ),length(pltf),length(obs),length(srfc),length(res)]);
        end        
        ...
        newQIS = [QI_1; QI_2; QI_3; QI_4; QI_5;QI_6]; % old, newQIS 8x4, so now 5x4?
        ...
        new_QIS = [newQIS;meanQ;meanQ+adj];
        ...
        ME_6 = (squeeze(MER(H, 6, 63, :, L, m, n)))';
        if all(isNaN(ME_6)) &&n~=1
            ME_6 =  (squeeze(MER(H, 6, 63, :, L, m, 1)))';
        end        
        new_MER = squeeze([ME_1; ME_2; ME_3; ME_4; ME_5; ME_6]);
        ...
        RM_6 = (squeeze(RMS(H, 6, 63, :, L, m, n)))';
        if all(isNaN(RM_6)) &&n~=1
            RM_6 = (squeeze(RMS(H, 6, 63, :, L, m, 1)))';
        end
        
        new_RMS = squeeze([RM_1; RM_2; RM_3; RM_4; RM_5; RM_6]);
    ...
    if ~cir
        ...
        png_out = ['GV[',num2str(H),']_QRMadj_3xN.',srfc{m},'.png'];
        while isafile([ACCP_pngs,png_out])
            nn = nn+1; n_str = ['_',num2str(nn)];
            png_out = ['GV[',num2str(H),']_QRMadj_3xN.',srfc{m},n_str,'.png'];
        end
    else
        nn = 1; n_str = ['_',num2str(nn)];
        png_out = ['GV[',num2str(H),']_QRMadj_3xN.cloud.',srfc{m},'.png'];
        while isafile([ACCP_pngs,png_out])
            nn = nn+1; n_str = ['_',num2str(nn)];
            png_out = ['GV[',num2str(H),']_QRMadj_3xN.cloud.',srfc{m},n_str,'.png'];
        end
    end
