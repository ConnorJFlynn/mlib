function plot_SITA_Sept_QMR_V1(QIS,MER,RMS,gv,sfc,obs,reso,pptname)
% plot_SITA_Sept_QMR_V1(QIS,MER,RMS,gv,sfc,reso,pptname)
...
% affecting NGE SPA 
% It looks like I clicked "save" before changing to V15, so this version
% and the saved V13 are functionally equivalent
...
%         weighted_Q = meannonan(newQIS .* ws)./meannonan(ws);
        ...
        ME_1 = mean(squeeze(MER(H, 1, DRS_cases, :, L, m, n)));
        if all(isNaN(ME_1)) &&n~=1
            ME_1 = mean(squeeze(MER(H, 1, DRS_cases, :, L, m, 1)));
        end
        ME_2 = mean(squeeze(MER(H, 3, DRS_cases, :, L, m, n)));
        if all(isNaN(ME_2)) &&n~=1
            ME_2 = mean(squeeze(MER(H, 3, DRS_cases, :, L, m, 1)));
        end
        ME_3 = mean(squeeze(MER(H, 4, DRS_cases, :, L, m, n)));
        if all(isNaN(ME_3)) &&n~=1
            ME_3 = mean(squeeze(MER(H, 4, DRS_cases, :, L, m, 1)));
        end
        ME_4 = mean(squeeze(MER(H, 5, DRS_cases, :, L, m, n)));
        if all(isNaN(ME_4)) &&n~=1
            ME_4 =mean(squeeze(MER(H, 5, DRS_cases, :, L, m, 1)));
        end
        ME_5 = mean(squeeze(MER(H, 2, ICA_cases, :, L, m, n)));
        if all(isNaN(ME_5)) &&n~=1
            ME_5 =  mean(squeeze(MER(H, 2, ICA_cases, :, L, m, 1)));
        end
        ...
        RM_1 = mean(squeeze(RMS(H, 1, DRS_cases, :, L, m, n)));
        if all(isNaN(RM_1)) &&n~=1
            RM_1 = mean(squeeze(RMS(H, 1, DRS_cases, :, L, m, 1)));
        end
        RM_2 = mean(squeeze(RMS(H, 3, DRS_cases, :, L, m, n)));
        if all(isNaN(RM_2)) &&n~=1
            RM_2 = mean(squeeze(RMS(H, 3, DRS_cases, :, L, m, 1)));
        end
        RM_3 = mean(squeeze(RMS(H, 4, DRS_cases, :, L, m, n)));
        if all(isNaN(RM_3)) &&n~=1
            RM_3 =mean(squeeze(RMS(H, 4, DRS_cases, :, L, m, 1)));
        end
        RM_4 = mean(squeeze(RMS(H, 5, DRS_cases, :, L, m, n)));
        if all(isNaN(RM_4)) &&n~=1
            RM_4 = mean(squeeze(RMS(H, 5, DRS_cases, :, L, m, 1)));
        end
        RM_5 = mean(squeeze(RMS(H, 2, ICA_cases, :, L, m, n)));
        if all(isNaN(RM_5)) &&n~=1
            RM_5 = mean(squeeze(RMS(H, 2, ICA_cases, :, L, m, 1)));
        end
