...
% v6b: only generate casebreakouts
...
pptname = ['ACCP_3x3.QMR.v6.',datestr(dnow,'yyyymmdd'),n_str,'.pptx'];
while isafile([PPT_path, pptname])
    n = n+1; n_str = ['_',num2str(n)];
    pptname = ['ACCP_3x3.QMR.v6.',datestr(dnow,'yyyymmdd'),n_str,'.pptx'];
end
...
% 
% plot_ACCP_QMR_V6(QIS,MER,RMS,gv,'LNDD','RES1', [PPT_path pptname]);
% plot_ACCP_QMR_V6(QIS,MER,RMS,gv,'LNDV','RES1', [PPT_path pptname])
% plot_ACCP_QMR_V6(QIS,MER,RMS,gv,'OCEN','RES1', [PPT_path pptname]);
...
grid('off'); xticks([]);yticks([]);set(gca, 'box','on');
tx = text(.1,.5,vi);set(tx,'FontSize',16);
png_name2 = ['ACCP_Qall_6a-i_title_',datestr(dnow,'yyyy-mm-dd'),'.png'];
