function OQS_LO = apply_OWs_C1_LO_v4(QIS)
% OQS_LO = apply_OWs_C1_LO_v4(QIS)
...
% v3 generates no ASCII output files,  shortened name to apply_OWs_C1_LO_v3
% v4 added LIC
...
for OO = 1:4
    for sfc = 1:2
        for H = gv
            ...
            grp =6; % LIC NR1 is QIS(h, 6, 63, :, L, m, n)
            ...
            temp = nanmean([meanOQ_NADLC1; meanOQ_NADLC0]);
            OQS_LO(H,:,1,sfc,OO) = temp;
            % After L==5
            L = 3; % for NANLC0
            grp = 1; % NLP-DRSall is QIS(h, 1, DRS_cases, :, L, m, n)
            QI_1 = squeeze(QIS(H, grp, DRS_cases,  :,L, sfc, R));
