function plot_SITA_Sept_cases_v4(QIS,gv,sfc,obs,reso,pptname)
% plot_SITA_Sept_cases_v3(QIS,MER,RMS,gv,sfc,obs,reso,pptname)
% Attempt to incorporate both DRS and ICA casestudy breakouts
% v3: fixed order of average (DRS_all, ICA_all) relative to cases
% v4: fixed lines 158-159 causing NLB to use DRS instead of ICA and be NAN
...
for H = gv
    ...
    for LO=1:length(obs) % find OBI indices of supplied obs
        ...
        if all(all(isNaN(QI_4))) &&n~=1
            QI_4 = QIS(H, 2, ICA_case1, :, L, m, 1); QI_4_b = QIS(H, 2, ICA_case2, :, L, m, 1);
            if ~all(all(isNaN(squeeze(QI_4_b)))) QI_4 = (QI_4 + QI_4_b)./2; end
        end
    ...
    if cir
        png_out = ['GV[',num2str(H),']_Q8a-o.cloud.',srfc{m},'.png'];
        while isafile([ACCP_pngs,png_out])
            nn = nn+1; n_str = ['_',num2str(nn)];
            png_out = ['GV[',num2str(H),']_Q8a-o.cloud.',srfc{m},n_str,'.png'];
        end
    else
        png_out = ['GV[',num2str(H),']_Q8a-o.',srfc{m},'.png'];
        while isafile([ACCP_pngs,png_out])
            nn = nn+1; n_str = ['_',num2str(nn)];
