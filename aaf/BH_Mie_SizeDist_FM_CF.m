function Boptics=BH_Mie_SizeDist_FM_CF(wvl,Dp,dNdlogDp,dlogDp,RIreal,RIimag)
...
%   Modified to establish agreement with SizeDist_Optics. Fixed factor of
%   pi/2 error.  Applied units conversion to get 1/Mm.  Flipped order of
%   for-loops for speed.
npnts=length(Dp);
...
%     ScaXS_byDiam(i) = GSA_Qsca;
    result(i,:)=Mie(complex(RIreal,RIimag),pi.*Dp(i)/wvl);    %pi * mr*x_range(i)/lambda;
