function status = plot_qc1(anc, field);
% plot_qc1(anc, field);
...
if isfield(anc.vars,field)&& isfield(anc.vars,['qc_',field])&& ~isempty(findstr(anc.recdim.name,char(anc.vars.(field).dims)))
   ...
   if length(tests>0)
      ...
      for x = 1:length(desc)
         tx(x) = text('string', desc(x),'interpreter','none','units','data','position',[1,x],'fontsize',8);
         set(tx(x),'units','normalized','color','black','linestyle','-','edgecolor',[.5,.5,.5]);
