function [qc] = vqc_insert_test(qc,qc_test);
% [qc] = vqc_insert_test(qc,qc_test);
% insert a new qc into an existing vap-type qc field.
% Needs to include a string describing the test
% The bit number of the test is required.
...
if qc_test.bit>last_bit
   for next_bit = last_bit:(qc_test.bit-1)
   ...
   for next_bit = (last_bit+1):-1:(qc_test.bit+1)
...
if isfield(qc_test,'value')
   ...
   qc.data = bitset(qc.data, qc_test.bit,0);
end
return
