...
function [status] = ancputrecslab (cdf, var,rec_start,rec_count)

% [vdata, status] = ANCPUTRECSLAB (cdf, var, start,count)
%
%   Writes slab of record variable data for a specified variable (by ID or name).
...
% VERSION INFORMATION:
%
%   $RCSfile: ancputrecslab.m,v $
%   $Revision: 1.1 $
%   $Date: 2013/12/11 00:49:08 $
%
%   $Log: ancputrecslab.m,v $
...
if (isnumeric(var) == 0)
    varname = var;
    if (isfield(cdf.vars,var))
        varid = cdf.vars.(var).id;
        datatype = cdf.vars.(var).datatype;
        dims = cdf.vars.(var).dims;
    end
else
    varid = var;
...
try
   [ncid] = netcdf.open(cdf.fname,'write');
catch ME
   status = status -1;
   disp(ME.message)
   return;
...
%     [dimlength, status] = mexnc('inq_dimlen', ncid, dimids(d));
    ...
    if dimids(d)==unlimid
       start(d) = rec_start-1;
       count(d) = min([rec_count,size(cdf.vars.(var).data,2)]);
    else
       start(d) = 0;
       count(d) = dimlength;
    end
...
if (nodata)
   ...
   try   
      vdata = cdf.vars.(var).data;
      if (length(dimids) == 0)
         netcdf.putVar(ncid,varid,start,vdata);
      else
         if (length(dimids) == 1)
         ...
         netcdf.putVar(ncid,varid,start,count,vdata);
      end
   catch ME
      status = status -1;
      disp(ME.message)
