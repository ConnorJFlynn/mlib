...
function [cdf] = ancloadcoords (ncfile)

% [cdf] = ancLOADCOORDS (ncfile)
%
...
%   For example,
%
%   mfr = ancloadcoords('mfrsr.nc') populates ancstruct, 'mfr' (with data)
%
%     $Revision: 1.1 $
...
dimnames = fieldnames(cdf.dims);
% Load coordinate data.
fields = fieldnames(cdf.vars);
for q = 1:length(fields)
    vkey = fields{q};
    vval = cdf.vars.(vkey);
    if any(strcmp(vkey,dimnames))||strcmp(vkey,'time_offset')||strcmp(vkey,'base_time')||~any(strcmp(vval.dims,cdf.recdim.name))
