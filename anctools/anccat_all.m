function nc = anccat_all;
% This function is just an empty shell.
% Interatively select a directory and then bundle all *.cdf files in the
...
nc = ancload([directoryname, flist(1).name]);
...
if ~exist([directoryname,flist(i).name],'dir')
    nc = anccat(nc,ancload([directoryname, flist(i).name]));
