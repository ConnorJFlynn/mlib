...
function status = ancsave (ncstruct, ncfile)

% status = ancSAVE (ncstruct, ncfile)
%
...
if (isfield(ncstruct,'atts'))
    % TODO: Set ARM globals (e.g. history, version).

    nextid = 0;
    ncstruct.atts = init_ids(ncstruct.atts);
    while (true)

        % Get element by next id.
        [val,key] = getnextelem(ncstruct.atts,nextid);
        if (strcmp(key,''))
            break;
        end
        nextid = val.id+1;
...
% Define dimensions.
if (isfield(ncstruct,'dims'))
    unlimid = -1;
    nextid = 0;
    ncstruct.dims = init_ids(ncstruct.dims);
    while (true)

        % Get element by next id.
        [val,key] = getnextelem(ncstruct.dims,nextid);
        if (strcmp(key,''))
            break;
        end
        nextid = val.id+1;
        ...
        dlength = val.length;
        if isfield(ncstruct, 'recdim')
            if (val.id == ncstruct.recdim.id)
                dlength = 0;
            end
        end
...
if (isfield(ncstruct,'vars'))
    nextid = 0;
    ncstruct.vars = init_ids(ncstruct.vars);
    while (true)

        % Get element by next id.
        [val,key] = getnextelem(ncstruct.vars,nextid);
        if (strcmp(key,''))
            break;
        end
        nextid = val.id+1;
...
%             val.dims = fliplr(val.dims); % undo flip when variable data was read.
            for d = 1:ndims
                ...
                if (dimids(d) == unlimid)
                    count(d) = ncstruct.dims.(val.dims{d}).length;
                end
            end
            ...
            if (isfield(val,'atts'))
                ...
                while (true)
                    ...
                    if (isfield(aval,'keep') == false || (isfield(aval,'keep') && aval.keep == true))
                        ...
                        try
                           netcdf.putAtt(ncid,vid,akey,aval.data)
                        catch ME
                           disp(ME.message);
                           status = status -1;
...
%             val.dims = fliplr(val.dims); % flip dimensions because matlab read in reverse.
            ...
            if ~any(size(val.data)==0)
               if (ndims == 0)
                  ...
                  try
                     netcdf.putVar(ncid,val.id,start,count,val.data);
                  catch ME
                     disp(ME.message);
                     status = status -1;
                     s = s -1;
            ...
            if (s ~= 0)
                ...
                if (ndims == 1)
                    if (ncstruct.dims.(val.dims{1}).isunlim == true)
                        % Re-orient single-dim, record fields as in ancgetdata.
                        td = td'; %'
                    end
                end
                if numel(val.data)>0 && numel(td)>0
