...
%         vars(v)
        ...
        if ~isfield(cdf.ncdef.vars.(vars{v}), 'datatype')
           cdf.ncdef.vars.(vars{v}) = ancfixdatatype(cdf.ncdef.vars.(vars{v}));
...
%             cdf.vars.(vars{v}).datatype = datatype;
            dispmsg(q, 1, 'VARIABLE', vars{v}, ['Set default datatype to ', anctype(cdf.vars.(vars{v}).datatype)]);
            status = status + 1;
        end

        cdf.vars.(vars{v}) = anccast2datatype(cdf.vars.(vars{v}));
        
        if ~isfield(cdf.vatts, (vars{v}))
            cdf.vatts.(vars{v}) = struct;
            dispmsg(q, 1, 'VARIABLE', vars{v}, 'Added "atts" element');
            status = status + 1;
        end

        if ~isfield(cdf.vatts.(vars{v}), 'long_name')
            cdf.vatts.(vars{v}).long_name = vars{v};
            dispmsg(q, 2, 'ATTRIBUTE', [vars{v},':long_name'], 'Defined');
            status = status + 1;
        end

        if ~isfield(cdf.vatts.(vars{v}), 'units')
            cdf.vatts.(vars{v}).units = 'unitless';
