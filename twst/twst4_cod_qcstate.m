function twst = twst4_cod_qcstate(in_file)
% Reads time, cod, and state from instrument level file
...
if iscell(in_file)&&~isempty(in_file)&&isafile(in_file{1})
   if ~isempty(in_file(2:end))
      twst = cat_twst(twst4_cod_qcstate(in_file{1}),twst4_cod_qcstate(in_file(2:end)));
   end
end
...
if ischar(in_file)&&isafile(in_file)
   ...
   cod_qc_id = netcdf.inqVarID(lv1_id,'qc_cod_valid');
   twst.cod_qc = netcdf.getVar(lv1_id,cod_qc_id)';
   state_id = netcdf.inqVarID(lv1_id,'state');
   twst.rcod_state = netcdf.getVar(lv1_id,state_id)';
