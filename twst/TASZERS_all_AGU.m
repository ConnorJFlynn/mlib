function TASZER_all_AGU
try
   anet_zrad = anet_ppl_zenrad(datenum(2024,06,01), datenum(2024,05,01));
catch
...
try
   cim_cld = rd_cimel_cloudrad_v3(datenum(2024,06,01), datenum(2024,05,01));
catch
...
for tt = datenum(2024,5,1):datenum(2024,5,31)
   ...
   if isafile([getnamedpath('TASZERS'),filesep,'TASZERS_',dstr,'.mat'])
      ...
      %Re-load TWST to get sza with updated twst4_to_struct
      infiles = dirlist_to_filelist(dir_(['sgptwstC1*',dstr,'*.nc'],'TWST10'));
      try
         TWST10= twst4_to_struct(infiles(1:end));
      catch
      ...
      try
         TWST11= twst4_to_struct(infiles(1:end));
      catch
      ...
      pat = ['sgpsaszeC1*vis*',dstr,'_1*.csv;','sgpsaszeC1*vis*',dstr,'_2*.csv;','sgpsaszeC1*vis*',dstr2,'_0*.csv;'];
      % infiles = getfullname('sgpsaszeC1*vis*.csv','Ze1_vis','Select Ze1 vis files');
      infiles = dirlist_to_filelist(dir_(pat,'Ze1_vis'))';
      if ~isempty(infiles{1})
         try
            Ze1_vis = rd_SAS_dualtint_raw(infiles);
         catch
            Ze1_vis = rd_SAS_dualtint_raw(infiles(2:end));
         end
      else
         Ze1_vis = [];
      end
      dstr = datestr(Ze1_vis.time(1),'yyyymmdd');
      pat = strrep(pat,'vis','nir');
      infiles = dirlist_to_filelist(dir_(pat,'Ze1_nir'))';
      % infiles = dirlist_to_filelist(dir_(['sgpsaszeC1*nir*',dstr,'*.csv'],'Ze1_nir'));
      if ~isempty(infiles{1})
         try
            Ze1_nir = rd_SAS_dualtint_raw(infiles);
         catch
      ...
      pat = strrep(pat,'nir','vis'); pat = strrep(pat,'C1','E13');
      infiles = dirlist_to_filelist(dir_(pat,'Ze2_vis'))';
      % infiles = dirlist_to_filelist(dir_(['sgpsaszeE13*vis*',dstr,'*.csv'],'Ze2_vis'));
      if ~isempty(infiles{1})
         try
            Ze2_vis = rd_SAS_dualtint_raw(infiles);
         catch
            Ze2_vis = rd_SAS_dualtint_raw(infiles(2:end));
         end
      else
         Ze2_vis = [];
      end
      pat = strrep(pat,'vis','nir');
      infiles = dirlist_to_filelist(dir_(pat,'Ze2_nir'))';
      % infiles = dirlist_to_filelist(dir_(['sgpsaszeE13*nir*',dstr,'*.csv'],'Ze2_nir'));
      if ~isempty(infiles{1})
         try
            Ze2_nir = rd_SAS_dualtint_raw(infiles);
         catch
            Ze2_nir = rd_SAS_dualtint_raw(infiles(2:end));
         end
      else
         Ze2_nir = [];
      end
      ...
      TASZERS.Ze2_vis = Ze2_vis;
      TASZERS.Ze1_nir = Ze1_nir;
      TASZERS.Ze12nir = Ze2_nir;
      TASZERS.TWST10 = TWST10;
      TASZERS.TWST11 = TWST11;
      %  if (isfield(Ze1_vis,'rate')||isfield(Ze2_vis,'rate'))&&(isa(Ze1_vis.rate,'double')||isa(Ze2_vis.rate,'double'))
      save([getnamedpath('TASZERS'),filesep,'TASZERS_',dstr,'.mat'],'-struct','TASZERS');
