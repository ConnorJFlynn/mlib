...
[t_ainb,t_bina] = nearest(old.time, new.time); % Useful to match times for difference plots;
[h_ainb,h_bina] = nearest(old.vars.height.data, new.vars.height.data); % Match height dims
%%
...
olds(miss1) = NaN;
miss2 = new.vars.(vars{v}).data<-9998&new.vars.(vars{v}).data>-10000;
news = new.vars.(vars{v}).data;
news(miss2) = NaN;
ax(1) = subplot(2,1,1); 
plot(serial2doy(old.time), olds, 'g.',serial2doy(new.time),news, 'ro');
title(vars{v},'interp','none');
xlabel('time (day of year)');
legend('old','new')
ax(2) = subplot(2,1,2);
plot(serial2doy(old.time(t_ainb)), olds(t_ainb) - news(t_bina),'kx');
ylabel('old - new')
%%
...
miss1 = old.vars.(vars{v}).data<-900;
olds = old.vars.(vars{v}).data;
olds(miss1) = NaN;
miss2 = new.vars.(vars{v}).data<-900;
news = new.vars.(vars{v}).data;
news(miss2) = NaN;
ax(1) = subplot(3,1,1); 
...
ylabel('height');
ax(2) = subplot(3,1,2);
imagegap(serial2doy(old.time(t_ainb)), old.vars.height.data(h_ainb),olds(h_ainb,t_ainb)-news(h_bina,t_bina));
colorbar;
title('old - new');
xlabel('time (day of year)');
ylabel('height');
ax(3) = subplot(3,1,3);
imagegap(serial2doy(old.time(t_ainb)), old.vars.height.data(h_ainb),(olds(h_ainb,t_ainb)-news(h_bina,t_bina))./olds(h_ainb,t_ainb));
colorbar;caxis([-1,1]);
title('(old - new)/old');
xlabel('time (day of year)');
ylabel('height');
