...
%       sun.radius  = sun distance from earth in AU used to scale TOA flux. 
...
% 9. Calculate the sun right ascension (in degrees)
sun_right_ascension = sun_right_ascension_calculation(apparent_sun_longitude, true_obliquity, sun_geocentric_position);
...
observer_local_hour = observer_local_hour_calculation(apparent_stime_at_greenwich, location, sun_right_ascension);

% 12. Calculate the topocentric sun position (rigth ascension, declination and
% rigth ascension parallax in degrees)
topocentric_sun_position = topocentric_sun_position_calculate(earth_heliocentric_position, location, observer_local_hour, sun_right_ascension, sun_geocentric_declination);
...
if isfield(location,'T')
    T = location.T;
else
    T = 283;
end
if isfield(location,'P')
    P = location.P;
else
    P = 1010;
end
...
sun = sun_topocentric_zenith_angle_calculate(location, topocentric_sun_position, topocentric_local_hour,T,P);

%-------------------------------------------------
% output the earth sun distance in AU
%--------------------------------------------------
sun.radius = earth_heliocentric_position.radius;
sun.ha = observer_local_hour;
sun.dec= sun_geocentric_declination;
...
if ~isfield(time,'UTC') % CJF: this may be the wrong place for this test.
   time.UTC = 0;
end
...
%%
% JME_ = julian.ephemeris_millenium';

ones_time = ones(size(JME));
L0 = sum(A0*ones_time .* cos(B0*ones_time + (C0 * JME)));
L1 = sum(A1*ones_time .* cos(B1*ones_time + (C1 * JME)));
L2 = sum(A2*ones_time .* cos(B2*ones_time + (C2 * JME)));
L3 = sum(A3*ones_time .* cos(B3*ones_time + (C3 * JME)));
L4 = sum(A4*ones_time .* cos(B4*ones_time + (C4 * JME)));
L5 = A5*ones_time .* cos(B5*ones_time + (C5 * JME));
...
L2 = sum(A2*ones_time .* cos(B2*ones_time + (C2 * JME)));
L3 = sum(A3*ones_time .* cos(B3*ones_time + (C3 * JME)));
L4 = A4*ones_time .* cos(B4*ones_time + (C4 * JME));
...
delta_longitude = ((nutation_terms(:,1)*ones(size(JCE)) + (nutation_terms(:,2) * JCE))) .* sin(tabulated_argument);
delta_obliquity = ((nutation_terms(:,3)*ones(size(JCE)) + (nutation_terms(:,4) * JCE))) .* cos(tabulated_argument);
...
mean_obliquity = p(1).*U.^10 + p(2).*U.^9 + p(3).*U.^8 + p(4).*U.^7 + p(5).*U.^6 + p(6).*U.^5 ...
   + p(7).*U.^4 + p(8).*U.^3 + p(9).*U.^2 + p(10).*U + p(11);
...
function sun_right_ascension = sun_right_ascension_calculation(apparent_sun_longitude, true_obliquity, sun_geocentric_position)
...
sun_right_ascension = atan2(argument_numerator, argument_denominator) .* 180./pi;
% Limit the range to [0,360];
sun_right_ascension = set_to_range(sun_right_ascension, 0, 360);
...
function observer_local_hour = observer_local_hour_calculation(apparent_stime_at_greenwich, location, sun_right_ascension)

observer_local_hour = apparent_stime_at_greenwich + location.longitude - sun_right_ascension;
% Set the range to [0-360]
observer_local_hour = set_to_range(observer_local_hour, 0, 360);


function topocentric_sun_position = topocentric_sun_position_calculate(earth_heliocentric_position, location, observer_local_hour, sun_right_ascension, sun_geocentric_declination)
...
sun_right_ascension_parallax = atan2(nominator, denominator);
% Conversion to degrees. 
topocentric_sun_position.rigth_ascension_parallax = sun_right_ascension_parallax .* 180./pi;

% Topocentric sun rigth ascension (in degrees)
topocentric_sun_position.rigth_ascension = sun_right_ascension + (sun_right_ascension_parallax .* 180./pi);

% Topocentric sun declination (in degrees)
nominator = (sin(sun_geocentric_declination .* pi./180) - (y.*sin(eq_horizontal_parallax .* pi./180))) .* cos(sun_right_ascension_parallax);
denominator = cos(sun_geocentric_declination .* pi./180) - (x.*sin(eq_horizontal_parallax .* pi./180)) .* cos(observer_local_hour .* pi./180);
...
function sun = sun_topocentric_zenith_angle_calculate(location, topocentric_sun_position, topocentric_local_hour,T,P)
