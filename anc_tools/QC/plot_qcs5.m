   ...
   while (f>0)
      ...
      if isfield(anc.vars,fields{f})&&isfield(anc.vars,['qc_',fields{f}])
         if isfield(anc.vars.(['qc_',fields{f}]).atts,'bit_1_description')
            %This is VAP QC
            [status,ax{axi},time_,qc_impact] = plot_vap_qc4(anc, field);
         elseif ~isempty(strfind(anc.atts.zeb_platform.data,'.s1')>0)||...
            ...
            [status,ax{axi},time_,qc_impact] = plot_s_qc4(anc, field);
            
         else
            [status,ax{axi},time_] = plot_mentor_qc4(anc, field);
         end
      else
         %This is not a qc field
         [status,ax{axi},time_] = plot_no_qc4(anc, field);
      end
      zoom(gcf,'on');
      if (axi>1)
         all_ax = [];
...
function [status,ax, time_] = plot_no_qc4(anc, field);
%[status,ax, time_] = plot_no_qc4(anc, field);
