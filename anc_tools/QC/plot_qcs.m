   ...
   while (f>0)
      ...
      if isfield(anc.vars,fields{f})&&isfield(anc.vars,['qc_',fields{f}])
         if isfield(anc.vars.(['qc_',fields{f}]).atts,'bit_1_description')
            %This is VAP QC
            [status,ax{axi},time_,qc_impact] = plot_vap_qc(anc, field);
         elseif ~isempty(strfind(anc.atts.datastream.data,'.s1')>0)||...
            ...
            [status,ax{axi},time_,qc_impact] = plot_s_qc(anc, field);
            
         else
            [status,ax{axi},time_,qc_impact] = plot_mentor_qc(anc, field);
         end
      else
         %This is not a qc field
         [status,ax{axi},time_] = plot_no_qc(anc, field);
      end
      var_fig = gcf;
      zoom(var_fig,'on');
      if (axi>1)
...
function [status,ax, time_] = plot_no_qc(anc, field);
