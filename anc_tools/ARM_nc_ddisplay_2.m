...
if ~isempty(fields)
    ...
    over_write = true;
    QCS = ['*QC*'];
    qc_str = ['set QC/QS [',QCS,']'];
    while (f>=0)
        
        ok = menu('',{'Next field';'Previous field';'Select field...';over_str;qc_str;'Settings';'Histogram';'Save image...';'Select datastream...';'Done'});
        % 1 'Next field...';
        % 2 'Previous field';
        % 3 'Select field...';
        
        % 4 'New Figure' or 'Replace';
        % 5 'Set QC mode...';
        % 6 'Plot settings...';
        % 7 'Histogram';
        % 8 'Save image'
        ...
        elseif ok==3       % 3 'Pick Field...';
            ...
            %             f = 1;
            %          end
        elseif ok==4       % 4 'Toggle new figure or overstrike/replace ';
            if over_write
               over_write = false;
               over_str = 'Re-Use';
            else
        ...
        elseif ok==6 % Adjust plot settings...
            plt = adjust_plot_settings(plt);
        elseif ok==9 % Select datastream
           DS = menu('Select DS',{ds_{:}, 'Browse...'});
           if DS > length(ds_)
              anc = anc_bundle_files;
              [~, ds] = fileparts(anc.fname); [ds_stem, ds] = strtok(ds,'.');ds_level = strtok(ds,'.');
              ds_(end+1) = {[ds_stem,'.',ds_level]};
              anc_(end+1) = {anc};
           else
              anc = anc_{DS};
           end
