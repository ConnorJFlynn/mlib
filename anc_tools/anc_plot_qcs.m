...
%                 && any(strcmp(anc.ncdef.recdim.name,anc.ncdef.vars.(field).dims))
    ...
    elseif length(anc.ncdef.vars.(field).dims)>1
        ...
        if isfield(anc.vdata,fields{f})&&isfield(anc.vdata,['qc_',fields{f}])
            if isfield(anc.vatts.(['qc_',fields{f}]),'bit_1_description')
                %This is VAP QC
                [status,ax{axi},time_,qc_impact] = anc_plot_vap_qc(anc, field);
            elseif ~isempty(strfind(anc.gatts.datastream,'.s1')>0)||...
                ...
                [status,ax{axi},time_,qc_impact] = anc_plot_s_qc(anc, field);
                
            else
                [status,ax{axi},time_,qc_impact] = anc_plot_mentor_qc(anc, field);
            end
        else
    ...
    ok = menu('',{'Next';'Previous';'Pick Field...';'New Figure';'Save image...';'Histogram';'Settings';'No QC';'Done'});
    % 1 'Next';
    % 2 'Previous';
    % 3 'Pick Field...';
    % 4 'New Figure';
    ...
    elseif ok==2       % 2 'Previous';
        f = f-1;
        forward = false;
        if f<1
            f = length(fields);
        end
    elseif ok==3       % 3 'Pick Field...';
        first = f;
        listed = 1;
        while listed >= 0
...
while ~done
 set_1 = menu('Select setting to modify:','Marker','MarkerEdgeColor','MarkerFaceColor','LineStyle','Color','Cancel/Abort');
