...
if ~isempty(fields)
   ...
   while (f>=0)
      ...
      elseif ok==4       % 4 'New figure window';
         new_figure = true;
      elseif ok==5
         overwrite = menu('Add plot to currently selected figure?','Overwrite current plot','Add to current plot');
         if overwrite== 1
            overwrite = true;
            overwrite_str = 'Set Hold "OFF"';
            hold(gca,'on');
         else
            overwrite = false;
            overwrite_str = 'Set Hold "ON"';
      ...
      elseif ok==6       % 5 'Set QC mode;
         QC = menu('Set QC mode:','no QC','summary','detailed','only good');
         if QC==1
            QCS = ['*no QC*'];
            qc_mode = 0;
         elseif QC==2
               ...
               if qc_mode==0 || ~isfield(anc.vdata,['qc_',fields{f}])
                  ...
                  if qc_mode==1 || (isfield(anc.gatts,'datastream')&&~isempty(strfind(anc.gatts.datastream,'.s1')>0))||...
                     ...
                     if isfield(anc.vatts.(['qc_',fields{f}]),'bit_1_description')
                        [status,ax{axi},time_,qc_fig] = anc_plot_vap_qcd(anc, field);
