...
if iscell(anc)
   anc_ = anc;
   anc = anc_{1};
   for n = length(anc_):-1:2
      tmp = anc_{n};
      [~, ds] = fileparts(tmp.fname); [ds_stem, ds] = strtok(ds,'.');ds_level = strtok(ds,'.');
      ds_(n) = {[ds_stem,'.',ds_level]};
   end
end
...
%    axi = 0;
   ...
   while (f>=0)
      ok = menu('',{'Pick field...';'Next';'Previous';'New Figure';...
         ['Toggle Hold <',hold_str,'>'];['QC mode <',QCS,'>'];'Settings for Masked/NO QC';...
         'Modify selected plot...';'Trim time series';'Select more data...'; 'X-Y regression'; ...
         'Histogram'; 'Export to ASCII';'Save image...';'Done'});
      % 1 'Pick field...'
      % 2 'Next -->'
      % 3 'Previous <--'
      % 4  New figure
      % 5 Toggle Hold ON/OFF
      % 6 QC mode
      % 7 Settings for Masked/No QC
      % 8 Modify selected plot... 
      % 9 Trim time series
      % 10 Select more data...
      % 11 X-Y regression 
      % 12 Generate a histogram
      % 13 Export to ASCII
      % 14 Save the image ...
      % 14 'Done'}
      ...
      elseif ok==7 % Settings for Masked/NO QC, Adjust plot settings...
         plt = adjust_plot_settings(plt);         
      elseif ok==8 % Modify selected plot
          done_mod = false;
          [~] = menu('Select a plot to modify, then select OK','OK'); pause(0.05);
          mod_ax = gca;
          while ~done_mod
              opt = menu('Modify plot: ', 'Labels, symbols, etc.', 'X limits reset','Y limits','Toggle y-log', 'Done');
              if opt==5
                  done_mod = true;
              elseif opt==4 % Toggle y-log
                  if strcmp(get(mod_ax,'yscale'),'linear')
                      logy
                  else
                      liny;
                  end
              elseif opt==3 % Set y-limits
                  ylims = menu('Use manual or Auto y-limits? ','Manual','Auto');
                  if ylims == 1
                      set(mod_ax, 'ylimmode','manual');
                      ax_ylim = ylim;
                      prompt = {'Y-max:','Y-min:'};
                  ...
                  elseif ylims ==2
                      set(mod_ax, 'ylimmode','auto');
                  end
              elseif opt==2 % Reset X-limits
                  xlim(mod_ax,[min(anc.time), max(anc.time)]); last_lim = xlim(mod_ax);
              elseif opt==1 % Modify legend
                  ...
                  lg = 0;kids = get(mod_ax,'children'); kids = flipud(kids);
                  colors = {'b','g','r','c','m','y','k','w'};
                  linestyle = {'-'  '--'  ':'  '-.'  'none'};
                  while lg<=length(leg_str)
                      lg = menu('Modify series ', [leg_str,{'Done'}]);
                      if lg<=length(leg_str)
                          pp = 1;
                          while pp < 9
                              pp = menu('Modify:','Legend','Marker','Size','EdgeColor','FaceColor','Linestyle','LineColor','LineWidth','Done');
                              if pp==1 %Legend
                                  leg_str(lg) = {input(['<',leg_str{lg},'> '],'s')};
                                  legend(leg_str);
                              elseif pp==2 %Marker
                                  Marker =  {'+'  'o'  '*'  '.'  'x'  'square'  'diamond'  'v'  '^'  '>'  '<'  'pentagram'  'hexagram'  'none'};
                                  mk = menu('Marker:',Marker);
                                  set(kids(lg),'Marker',Marker{mk});
                              elseif pp ==3 %MarkerSize
                                  msize = get(kids(lg),'MarkerSize');
                                  mdone = false;
                                  while ~mdone
                                      ms = menu('Adjust marker size:','/2','-1','+1','*2','Done');
                                      if ms==1
                                          msize = msize/2; msize(msize<1) = 1;
                                      elseif ms==2
                                          msize = msize-1; msize(msize<1) = 1;
                                      elseif ms==3
                                          msize = msize+1; 
                                      elseif ms==4
                                          msize = msize*2;
                                      elseif ms==5
                                       ...
                                       set(kids(lg),'MarkerSize',msize);
                                  end                                  
                              elseif pp ==4 %EdgeColor
                                 colr = menu('Edge Color: ',[colors,{'[r g b]'}]);
                                 if colr<=length(colors)
                                     set(kids(lg),'MarkerEdgeColor',colors{colr});
                                 else
                                     rgb = sscanf(input('Enter a RBG values: ','s'),'%d')'; 
                                     rgb = rgb./max(rgb); rgb(rgb<0)=0;                                  
                                     set(kids(lg),'MarkerEdgecolor',rgb);
                                 end
                              elseif pp==5 %FaceColor
                                  colr = menu('Face Color: ',[colors,{'[r g b]'}]);
                                  if colr<=length(colors)
                                      set(kids(lg),'MarkerFaceColor',colors{colr});
                                  else
                                      rgb = sscanf(input('Enter a RBG values: ','s'),'%d')';
                                      rgb = rgb./max(rgb); rgb(rgb<0)=0;
                                      set(kids(lg),'MarkerFacecolor',rgb);
                                  end
                              elseif pp == 6 % Linestyle
                                  lstyle = menu('LineStyle: ',linestyle);
                                  set(kids(lg),'LineStyle',linestyle{lstyle});
                              elseif pp == 7 % LineColor
                                  colr = menu('LineColor: ',[colors,{'[r g b]'}]);
                                  if colr<=length(colors)
                                      set(kids(lg),'Color',colors{colr});
                                  else
                                      rgb = sscanf(input('Enter a RBG values: ','s'),'%d')';
                                      rgb = rgb./max(rgb); rgb(rgb<0)=0;
                                      set(kids(lg),'LineColor',rgb);
                                  end
                              elseif pp ==8 % LineWidth
                                  lwidth = get(kids(lg),'LineWidth');
                                  ldone = false;
                                  while ~ldone
                                      ms = menu('Adjust linewidth:','/2','-1','+1','*2','Done');
                                      if ms==1
                                          lwidth = floor(lwidth/2); msize(lwidth<1) = 1;
                                      elseif ms==2
                                          lwidth = lwidth-1; lwidth(lwidth<1) = 1;
                                      elseif ms==3
                                          lwidth = lwidth+1; 
                                      elseif ms==4
                                          lwidth = lwidth*2;
...
%          set(gca,'ylim',ylim, 'ylimmode','manual');
      elseif ok==9 % Trim time-series
         ready = menu('Zoom into a figure and then select the values to exclude: ',...
            'Exclude before ...','Exclude after ...',...
            'Exclude only displayed times', 'Exclude all times not displayed',...
            'Cancel/abort');
         xl = xlim; yl = ylim;
         if ready==1
           [~, anc] = anc_sift(anc, anc.time<xl(1));
         elseif ready==2
            [~, anc] = anc_sift(anc, anc.time>xl(2));
         elseif ready==3
            [~, anc] = anc_sift(anc, (anc.time>=xl(1)&anc.time<=xl(2)));
         elseif ready==4
            anc = anc_sift(anc, (anc.time>=xl(1)&anc.time<=xl(2)));
         end
      elseif ok==10 % Select more data
%          new_figure = true;
         if isempty(who('anc_'))
            anc_(1) = {anc};
         end
         ...
         if DS > length(ds_)
            clear anc
            anc = anc_bundle_files;
            [pname_new, ds] = fileparts(anc.fname); [ds_stem, ds] = strtok(ds,'.');ds_level = strtok(ds,'.');
            dstream = [ds_stem,'.',ds_level];
            found = false; d = 0;
            while ~found && d< length(ds_)
               d = d+1;pname_d = fileparts(anc_{d}.fname);
               if strcmp(ds_(d),dstream) && strcmp(pname_new,pname_d)
                   found = true;
                   anc = anc_cat(anc, anc_{d}); anc_(d) = {anc};
               end
               if strcmp(ds_(d),dstream) && ~strcmp(pname_new,pname_d)
                   dv = 1;
                   while any(strcmp(ds_,[dstream,sprintf(' %d',dv)]))
                       dv = dv +1;
                   end
                   dstream = [dstream,sprintf(' %d',dv)];
               end
            end
            if ~found
               ds_(end+1) = {dstream};
...
%          [~, az_to_id] = sort(id_to_az);
%          fields_az = fields(id_to_az);
      elseif ok==11       %11 X-Y regression    
              [~,anc_fname] = fileparts(anc.fname);
              extras.dsname = strtok(anc_fname,'.');
              extras.plotname = field;
              extras.trim = true;
              menu('Click on a plot containing two timeseries for X-Y regression, and then select OK...','OK')              
              XY_regression_of_timeseries(gcf,extras);
      elseif ok==12   % 12 Generate histogram
         gen_hist(anc, field,ax);         
      elseif ok==13   % 13  Export to ASCII
      ...
      if f>0
         ...
         if new_figure
            if isempty(anc.ncdef.vars.(field).dims{1})
               fig_99 = figure_(double(fig_99)+1);
               new_figure = false;
            else
               if ok~=8
                  axi = axi +1;
...
%                   newpos(1) = newpos(1)+0.9*newpos(3);
         ...
         % If not changing overwrite, axes scale, trimming, computing X-Y regression
         % generating a histogram, exporting data, saving to file, histogram, or done
         % Comment: I can see now that it might be worthwhile/better to
         % define individual booleans for these instead of just using the
         % value of OK
         if ~(ok==5 || ok==8|| ok==9 || ok==11 || ok==12 || ok==13 || ok==14 )
            % Determine kind of field: eg static, coord, rec, n-dim
            % If static, then re-use figure 99 which will not be linked to
            % the other axes.
            if isempty(anc.ncdef.vars.(field).dims{1})
