...
function [cdf,status] = anc_link (ncfile)
...
%   For example,
%
%   mfr = anc_LINK('mfrsr.nc') populates acstruct, 'mfr'
%
%     $Revision: 1.1 $
%     $Date: 2013/12/11 00:49:08 $
%
%   See also anc_LOAD, anc_GETDATA, anc_SAVE, anc_STRUCT.
%
...
try
   ...
   disp(ME.message);
    ...
    cdf.ncdef.atts.(attname).id = g;
    cdf.ncdef.atts.(attname).datatype = atttype;
    cdf.gatts.(attname) = attval;
end
...
for d = 0:(ndims-1)
   ...
   disp(ME.message);
    ...
    if (d == recdim)
        isunlim = true;
        recdim = 'recdim';
        cdf.ncdef.(recdim).name = dimname;
        cdf.ncdef.(recdim).id = d;
        cdf.ncdef.(recdim).length = dimlength;
    end

    dimname = legalizename(dimname);
    cdf.ncdef.dims.(dimname).id = d;
    cdf.ncdef.dims.(dimname).length = dimlength;
    cdf.ncdef.dims.(dimname).isunlim = isunlim;
    if isunlim==true
        cdf.ncdef.recdim.id = d;
        cdf.ncdef.recdim.name = dimname;
        cdf.ncdef.recdim.length = dimlength;
...
%       netcdf.close(ncid);
   ...
   cdf.ncdef.vars.(vname).id = v;
    cdf.ncdef.vars.(vname).datatype = vtype;
    clear vdimnames;
    vdimnames{1} = '';
    for d = 1:vndims
       try
...
%     vdimnames = fliplr(vdimnames);
    cdf.ncdef.vars.(vname).dims = vdimnames;
    for a = 0:(vnatts-1)
try
...
%           netcdf.close(ncid);
        ...
        cdf.ncdef.vars.(vname).atts.(attname).id = a;
        cdf.ncdef.vars.(vname).atts.(attname).datatype = atttype;
        cdf.vatts.(vname).(attname) = attval;
    end
end
