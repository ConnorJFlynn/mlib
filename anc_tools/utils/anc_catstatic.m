function nc = anc_catstatic(filelist)
...
[stem,dl] = strtok(fname, '.');
dl = strtok(dl,'.');
disp(['Saving ',stem,'.',dl,'.globals.',datestr(nc.time(1),'yyyymmdd'),'_',datestr(nc.time(end),'yyyymmdd'),'.mat'])
save([pname,filesep,'..',filesep, stem,'.',dl,'.globals.',datestr(nc.time(1),'yyyymmdd'),'_to_',datestr(nc.time(end),'yyyymmdd'),'.mat'],'-struct','nc');
