...
% Lorientian looks good for 1e-2 to 1e-4.  
% Very distinct behavior ~635, flat scattering contribution
% Maybe beats in longer wavelengths?
% Evaluate separating a baseline + lorientzian?
...
stray_405_vis.nsig = stray_405_vis.sig./max(stray_405_vis.sig);
    ...
    title('405 stray vis')
    nm = stray_405_vis.nm;
    nor = stray_405_vis.sig./max(stray_405_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
    nm_ = nm<xl(1)|nm>xl(2);
    hold('on'); plot(nm(v_), y_prime,'ro',nm( nm_), lorentp(nm(nm_), params), 'r-');
...
figure; plot(stray_450_vis.nm, stray_450_vis.sig./max(stray_450_vis.sig),'-k');legend('450 nm')
    title('450 stray vis')
nor = stray_450_vis.sig./max(stray_450_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
...
figure; plot(stray_520_vis.nm, stray_520_vis.sig./max(stray_520_vis.sig),'-k');legend('520 nm')
    title('520 stray vis')
nor = stray_520_vis.sig./max(stray_520_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
...
nor = stray_635_vis.sig./max(stray_635_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
...
figure; plot(stray_650_vis.nm, stray_650_vis.nsig,'-k');legend('650 nm')
    title('650 stray vis')
    nor = stray_650_vis.sig./max(stray_650_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
...
figure; plot(stray_778_vis.nm, stray_778_vis.nsig,'-k');legend('778 nm')
    title('778 stray vis')
    nor = stray_778_vis.sig./max(stray_778_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
...
figure; plot(stray_850_vis.nm, stray_850_vis.nsig,'-k');legend('850 nm')
    title('850 stray vis')
    nor = stray_850_vis.sig./max(stray_850_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
...
figure; plot(stray_980_vis.nm, stray_980_vis.nsig,'-k');legend('980 nm');
    title('980 stray vis');
 nor = stray_980_vis.sig./max(stray_980_vis.sig);
    v = axis;
    v_ = nm>v(1)&nm<v(2)&nor>v(3)&nor<v(4);
    [y_prime, params] = lorentzfit(nm(v_), nor(v_));
    xl = xlim;
