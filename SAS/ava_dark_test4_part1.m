...
% Avantes CMOS. Looks strange on data collection with low Tint shoing high
% dark counts.
...
% pname = 'C:\case_studies\ARRA\SAS\data_tests\dark_characterization\CCD_0911136U1_2\';
 pname = 'C:\case_studies\ARRA\SAS\data_tests\dark_characterization\CMOS_0911130U1_1\';
...
noNaNs = ~any(isnan(ava.Sample));
   ...
   %%
  figure; 
  plot(ava.tint(~first), ava.mean_samp(~first), 'o');
    xlabel('integration time [ms]')
ylabel('dark counts')
  title(['Spectrometer SN: ',char(ava.Spec_desc), ':internal averaging, delay of 20 ms'],'interp','none')
saveas(gcf,[ava.pname, ava.Spec_desc{:},'_dark_vs_tint.png']);
  %%
