...
wl = [400:50:850];
filts = [415, 500, 615, 673, 870];
for f = length(filts):-1:1;
   wl = abs(spec.nm-filts(f))<10;
   cos_corr(:,f) = mean(spec.cos_corr(~dark_zind,wl),2);
end


%%
figure; lines = plot(spec.Angle(~dark_zind), cos_corr(:,2:end-1),'-');
