...
% This is for second test for HOU as teh first didn't work quite right bcuz nir tint hi was 450 ms
...
figure; plot(zenir.wl, max(zenir.rate(zevis.t_int_ms==vlo,:)),'r-')
xl = xlim; 
xl_ = zenir.wl>xl(1) & zenir.wl<xl(2);

shut_ = zevis.Shutter_open_TF==0; 
shut_(1:end-1) = shut_(1:end-1)|shut_(2:end); shut_(2:end) = shut_(1:end-1)|shut_(2:end);
zenir.rate(:,~xl_) = NaN;zenir.rate(shut_,:) = NaN;
zevis.rate(shut_,~xl_) = NaN;
zevis.rate(zevis.spec==2.^16) = NaN;
zenir.rate(zenir.spec==2.^16) = NaN;
nrate_hi = zenir.rate(zevis.t_int_ms==vhi,xl_);
ntemp_hi = zenir.Temp_chiller_C(zevis.t_int_ms==vhi);
...
figure; plot(zenir.time(zevis.t_int_ms==vhi),nrate_hi(:,100),'.',zenir.time(zevis.t_int_ms==vlo),nrate_lo(:,100),'.')
nrate_rat = nrate_lo(:)./nrate_hi(:);
ntemp = ntemp_hi.*ones([1,222]);ntemp = ntemp(:);
figure; plot(wd_ni(:),nrate_rat,'.'); logy
figure; scatter(fliplr(wd_ni(1:10:end)), fliplr(nrate_rat(1:10:end)), 2, (ntemp(1:10:end))); axis(v)
v = axis;
wd = [0:.05:1]; wd(end) = wd(end) -.005;wd(1) = wd(1) +.005;
for wd_i = length(wd):-1:2
    sub = wd_ni>wd(wd_i-1) & wd_ni<=wd(wd_i) & nrate_rat>(v(3))&nrate_rat<(v(4));
...
xlabel('Pixel Well Depth');
ylabel('Low Gain Rate / High Gain Rate')
title({'HOU SASZe NIR Dual Rate Test (100 ms / 400 ms)';[datestr(zenir.time(1),'yyyy-mm-dd'),' ~4 hours spanning noon']})
