...
% Read bare_fiber:
bare = SAS_read_Albert_csv(getfullname('*.csv','ava','Select bare fiber spectra'));
...
He = SAS_read_Albert_csv(getfullname('*.csv','ava','Select Hemispheric diffuser spectra'));
He.dark = sum(He.spec,2)<mean(sum(He.spec,2));
He.darks = mean(He.spec(He.dark,:),1);
He.lights = mean(He.spec(~He.dark,:),1)-He.darks;

Ze = SAS_read_Albert_csv(getfullname('*.csv','ava','Select Zenith collimator spectra'));
