...
pname = ['C:\case_studies\ARRA\SAS\data_tests\tilt_tests\AccelerometerRedux\1a_stability\'];
accel_500Hz = loadit(pname);
accel_100Hz = loadit(pname);
accel_50Hz = loadit(pname);
accel_10Hz = loadit(pname);
%%
figure; plot([1:length(accel_500Hz.A3D_x_deg)],[accel_500Hz.A3D_x_deg,accel_500Hz.A3D_y_deg,accel_500Hz.A3D_z_deg],'.');legend('x','y','z')
%%
std([accel_500Hz.A3D_x_deg,accel_500Hz.A3D_y_deg,accel_500Hz.A3D_z_deg])
std([accel_100Hz.A3D_x_deg,accel_100Hz.A3D_y_deg,accel_100Hz.A3D_z_deg])
std([accel_50Hz.A3D_x_deg,accel_50Hz.A3D_y_deg,accel_50Hz.A3D_z_deg])
std([accel_10Hz.A3D_x_deg,accel_10Hz.A3D_y_deg,accel_10Hz.A3D_z_deg])
%%
accel_500Hz.down500.accelX = downIQ(accel_500Hz.A3D_x_deg,50);
accel_500Hz.down500.accelY = downIQ(accel_500Hz.A3D_y_deg,50);
accel_500Hz.down500.accelZ = downIQ(accel_500Hz.A3D_z_deg,50);
%%
...
drift_test = loadit(['C:\case_studies\ARRA\SAS\data_tests\tilt_tests\AccelerometerRedux\1b_drift']);
%%
figure; plot([1:size(drift_test,1)],[drift_test(:,1),drift_test(:,2),drift_test(:,3),...
   drift_test(:,4),drift_test(:,5)], '-');
legend('3Dx','3Dy','3Dz','2Dx','2Dy');
rel_3Dx = drift_test(:,1)./mean(drift_test(:,1));
 rel_3Dy = drift_test(:,2)./mean(drift_test(:,2));
 rel_3Dz = drift_test(:,3)./mean(drift_test(:,3));
 rel_2Dx = drift_test(:,4)./mean(drift_test(:,4));
 rel_2Dy = drift_test(:,5)./mean(drift_test(:,5));
 figure; plot([1:size(drift_test,1)],[rel_3Dx,rel_3Dy,rel_3Dz,rel_2Dx,rel_2Dy,], '-'); 
 legend('3Dx','3Dy','3Dz','2Dx','2Dy');
%%
...
tilt_path = 'C:\case_studies\ARRA\SAS\data_tests\tilt_tests\AccelerometerRedux\1c_tilt';
level = loadit([tilt_path,'\File1_platform_level.txt']);
side1 = loadit([tilt_path,'\File2_platform_side1_raised_little.txt']);
side1x = loadit([tilt_path,'\File3_platform_side1_raised_more.txt']);
%%
side2 = loadit([tilt_path,'\File4_platform_side2_raised_little.txt']);
side2x = loadit([tilt_path,'\File6_platform_diagonal_raised_little.txt']);
%%
figure; 
ax1 = subplot(2,1,1);
plot([1:size(level,1)], [level(:,1),level(:,2),level(:,3),level(:,4),level(:,5),], '-');legend('3dx','3dy','3dz','2dx','2dy'); 
%%

hold('on')
plot([1:size(side1,1)], [side1(:,1),side1(:,2),side1(:,3),side1(:,4),side1(:,5),], '-');
plot([1:size(side1x,1)], [side1x(:,1),side1x(:,2),side1x(:,3),side1x(:,4),side1x(:,5),], '-');
hold('off')
%%
...
plot([1:size(level,1)], [level(:,1),level(:,2),level(:,3),level(:,4),level(:,5),], '-');legend('3dx','3dy','3dz','2dx','2dy'); 
hold('on')
plot([1:size(side2,1)], [side2(:,1),side2(:,2),side2(:,3),side2(:,4),side2(:,5),], '-');
plot([1:size(side2x,1)], [side2x(:,1),side2x(:,2),side2x(:,3),side2x(:,4),side2x(:,5),], '-');
hold('off')
