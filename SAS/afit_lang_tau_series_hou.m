...
if isavar('Vo')&&isstruct(Vo)&&~isfield(Vo,'mfrM1')
   %where (what piece of code) were these Vos.mat files created/saved?
   % produced from command line
   Vo.mfrM1 = load(['C:\Users\Connor Flynn\OneDrive - University of Oklahoma\Desktop\xdata\ARM\adc\mentor\aodfit_be\hou\houmfrsr7nchM1.b1\houmfrsr7nchM1.b1_20221002_Vo.mat']);
   Vo.mfrM1.tag = 'mfrM1';
   rVo.mfrM1 = rVo_from_tau_lang(Vo.mfrM1, 21);
end

if isavar('Vo')&&isstruct(Vo)&&~isfield(Vo,'sasmfr')
   %where (what piece of code) were these Vos.mat files created/saved?
   % produced from command line
   Vo.sasmfr = load(['C:\Users\Connor Flynn\OneDrive - University of Oklahoma\Desktop\xdata\ARM\adc\mentor\aodfit_be\hou\housashemfrM1.b1\housashemfrM1.b1.20210921_20221001_Vo.mat']);
   Vo.sasmfr.tag = 'sasmfr';
   rVo.sasmfr = rVo_from_tau_lang(Vo.sasmfr, 21);
end

% if isavar('Vo')&&~isfield(Vo,'mfrE13')
%    Vo.mfrE13 = load(getfullname(');
...
% end
if isavar('Vo')&&isstruct(Vo)&&~isfield(Vo,'anet')
   Vo.anet = load(['C:\Users\Connor Flynn\OneDrive - University of Oklahoma\Desktop\xdata\ARM\adc\mentor\aodfit_be\hou\houmfrsr7nchM1.b1\houmfrsr7nchM1.b1_20221002_Vo.mat']);
   Vo.anet.tag = 'anet';
...
% if foundstr(dirbeam.fname,'M1.b1')
%    tag = 'mfrM1';
...
if ~isempty(dirbeam_files)
   ...
   if isfield(infile,'vdata')
      ...
      if isfield(infile.vdata, 'direct_normal_415nm') % It is sashe mfr file
