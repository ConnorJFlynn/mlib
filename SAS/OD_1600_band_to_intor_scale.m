...
mfrsr_ch7.nm = filt2.nm;
mfrsr_ch7.Tr = filt2.Tr./sum(filt2.Tr);
figure; plot(mfrsr_ch7.nm, mfrsr_ch7.Tr,'-');

mfrsr_ch7.ray = rayleigh_ht(mfrsr_ch7.nm./1000, 1013.25);
mfrsr_ch7.H2O = interp1(OD_1600_band.nm, h2o_5cm_rod(:,2), mfrsr_ch7.nm,'linear');
nans = isnan(mfrsr_ch7.H2O); sum(nans)
mfrsr_ch7.H2O(nans) = 0;

mfrsr_ch7.CH4 = interp1(OD_1600_band.nm, ch4_1860ppb_rod(:,2), mfrsr_ch7.nm,'linear');
nans = isnan(mfrsr_ch7.CH4); sum(nans)
mfrsr_ch7.CH4(nans) = 0;

mfrsr_ch7.CO2 = interp1(OD_1600_band.nm, co2_410ppm_rod(:,2), mfrsr_ch7.nm,'linear');
nans = isnan(mfrsr_ch7.CO2); sum(nans)
mfrsr_ch7.CO2(nans) = 0;

figure; plot(OD_1600_band.nm, h2o_5cm_rod(:,2), 'o',mfrsr_ch7.nm, mfrsr_ch7.H2O,'x')

fprintf('%2.4e \n',trapz(flipud(mfrsr_ch7.nm), flipud(mfrsr_ch7.Tr.*mfrsr_ch7.H2O))./trapz(flipud(mfrsr_ch7.nm), flipud(mfrsr_ch7.Tr)))
fprintf('%2.4e \n', sum(mfrsr_ch7.Tr.*mfrsr_ch7.H2O))

%     filts[, 2] <- filts[, 2]/sum(filts[, 2])
%         xx <- sum(filts[, 2] * approx(molecule[, 1], exp(-molecule[, 2]), filts[, 1])$y, na.rm = T)
%     return(round(-log(xx), 7))

A = exp(-h2o_5cm_rod(:,2)); 
H2O_Tr = interp1(OD_1600_band.nm, A, mfrsr_ch7.nm,'linear');
nans = isnan(H2O_Tr); sum(nans); 
H2O_Tr(nans) = 0;
sum(mfrsr_ch7.Tr(nans))
sum(mfrsr_ch7.Tr(~nans))
sum(mfrsr_ch7.Tr)
sum(mfrsr_ch7.Tr.*H2O_Tr)
-log(sum(mfrsr_ch7.Tr(~nans).*H2O_Tr(~nans)))
-log(sum(mfrsr_ch7.Tr.*H2O_Tr))
H2O_Tr(nans) = 1;
-log(trapz(flipud(mfrsr_ch7.nm(~nans)), flipud(mfrsr_ch7.Tr(~nans).*H2O_Tr(~nans)))./trapz(flipud(mfrsr_ch7.nm(~nans)), flipud(mfrsr_ch7.Tr(~nans))) )
