...
if ~isavar('a0')
   a0= anc_load(getfullname('housashenirhisun*.nc','sasa0')); % a0.gatts.operation_mode
end

if ~isavar('b1');
   b1 = anc_load(getfullname(['housashenir*',datestr(a0.time(1),'yyyymmdd'),'*.nc'],'sasb1'));
end

a0 = anc_sift(a0, a0.vdata.integration_time==320);
...
[sza, saz, AU, ha, dec, sel, oam] = sunae(a0.vdata.lat, a0.vdata.lon, a0.time(a0.vdata.tag==8));
...
for ti = length(a0.time(a0.vdata.tag==8)):-1:1
...
figure_; plot(b1.time, b1.vdata.direct_horizontal_nir(nm_500,:),'r-', a0.time(a0.vdata.tag==tag(4)), dirh(nm_500,:),'k--')
logy; legend('from b1','from a0'); title('dirh'); dynamicDateTicks

figure_; plot(b1.time, b1.vdata.direct_normal_nir(nm_500,:),'r-', a0.time(a0.vdata.tag==tag(4)), dirn(nm_500,:),'k--')
logy; legend('from b1','from a0'); title('dirn'); dynamicDateTicks

figure_; plot(b1.time, b1.vdata.diffuse_hemisp_nir(nm_500,:),'r-', a0.time(a0.vdata.tag==tag(4)), diff(nm_500,:),'k--')
logy;  legend('from b1','from a0'); title('diff'); dynamicDateTicks

figure; plot(b1.time, b1.vdata.direct_normal_vis(nm_500,:)./b1.vdata.diffuse_hemisp_vis(nm_500,:),'r-', a0.time(a0.vdata.tag==tag(4)), dirn(nm_500,:)./diff(nm_500,:),'k--')
logy;
