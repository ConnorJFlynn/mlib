...
if isfield(inside_spec, 'Temp')
   inside_spec.Temp1 = inside_spec.Temp;
end
if isfield(outside_spec, 'Temp')
   outside_spec.Temp1 = outside_spec.Temp;
end

figure; plot(serial2Hh(trh.time), trh.Temp1+9, 'r',serial2Hh(inside_spec.time), inside_spec.Temp1, 'g',serial2Hh(outside_spec.time), outside_spec.Temp1, 'b'  )
...
%%
lines = recolor(lines, inside_spec.Temp1');colorbar
%%
...
inside_spec.dif_temp = diff(inside_spec.Temp1);
...
lines3 = recolor(lines3,inside_spec.Temp1(2:end)',[-5,10]);
colorbar
title({['Pixel-by-pixel temporal stability of NIR  under changing temperature'],inside_spec.sn})

%%
figure; plot(inside_spec.Temp1(1:400), sum(abs(diff(inside_spec.spec_sans_dark(1:400,:),[],2)),2),'o');
...
figure; plot(inside_spec.Temp1(ii), mean(inside_spec.spec(ii,:),2),'o')
