...
%    sas.pname = [pname,filesep];
   ...
   sas.sig(sas.Shutter_open_TF==1,:) = sas.spec(sas.Shutter_open_TF==1,:)...
      - ones([rows,1]) * mean(sas.spec(sas.Shutter_open_TF==0,:));
   ...
   if isavar('plots') && plots
      figure_(1000); these = plot(sas.lambda, sas.sig(sas.Shutter_open_TF==1,:),'-');
