...
% 
spec = SAS_read_Albert_csv(getfullname_('*.csv','ava','Select cosine correction data'));
...
dark_wt = mean(spec.spec(dark_ii,wl_dk),2);
dark_wt = interp1(dark_ii, dark_wt,[1:length(spec.time)],'linear','extrap');
darks = dark_wt' * norm_dark; % Why am I doing this instead of just subtracting actual darks?
...
first(1:mid) = true;
second(mid:end) = true;
angs = spec.Angle~=0&spec.Shuttered_0~=0&abs(spec.Angle)<81;
angs_pos = spec.Angle~=0&spec.Shuttered_0~=0&(spec.Angle>=0)&(spec.Angle<=81);
angs_neg = spec.Angle~=0&spec.Shuttered_0~=0&(spec.Angle<=0)&(spec.Angle>=-81);
...
%%
figure(1); 
plot([0;spec.Angle(first&angs_pos)], [1;base_(first&angs_pos)./(cos(pi.*spec.Angle(first&angs_pos)./180).*base_0(first&angs_pos))], 'o-',...
   [spec.Angle(second&angs_pos);0], [base_(second&angs_pos)./(cos(pi.*spec.Angle(second&angs_pos)./180).*base_0(second&angs_pos));1], '*-');
%
...
grid('on');
%%
[degs_,ij] = sort([0;spec.Angle(first&angs_pos);spec.Angle(second&angs_pos);0]); 
tmp_ = [1;base_(first&angs_pos)./(cos(pi.*spec.Angle(first&angs_pos)./180).*base_0(first&angs_pos));...
   base_(second&angs_pos)./(cos(pi.*spec.Angle(second&angs_pos)./180).*base_0(second&angs_pos));1];
coscor_ = tmp_(ij);
figure; plot(degs_, coscor_,'.');
He_coscor = [degs_, coscor_];
%%
figure(1); 
plot(spec.Angle(first&angs_pos), base_(first&angs_pos)./(cos(pi.*spec.Angle(first&angs_pos)./180).*base_0(first&angs_pos)), 'o-',...
