...
vis_dark = he_vis.spec(he_vis.Shutter_open_TF==0,:);

figure; lines = semilogy(he_vis.lambda(vis_nm), he_vis.spec(he_vis.Shutter_open_TF==1,vis_nm)-...
   ones(size(he_vis.time(he_vis.Shutter_open_TF==1)))*vis_dark(vis_nm), '-');
lines = recolor(lines, [he_vis.Tag(he_vis.Shutter_open_TF==1)]');
colorbar
%%
