function assist = assist_annew_nlc_FR_pc_raw % ASSIST annew
...
% D:\data\assist_reproc\tmp\tmpassistrawM1
...
pause(.01);
if pick==1
    raw_files = getfullname_('*_ann_*.*','assist_dir','Select a file in the directory to bundle.');
    if ~iscell(raw_files)
        raw_files = {raw_files};
    end
    ...
    rawfiles = getfullname_('*_ann_*.csv','assist_dir');
    if ~iscell(rawfiles)
        rawfiles = {rawfiles};
    end
...
while n<=N-2
    ...
    if ~(...
        ...
        chA.Tb_real_mean_sky = BrightnessTemperature(seq.chA.cxs.x(a_), real(mean(seq.chA.mrad.y(seq.logi.Sky,a_))));
        chB.Tb_real_mean_sky = BrightnessTemperature(seq.chB.cxs.x(b_), real(mean(seq.chB.mrad.y(seq.logi.Sky,b_))));
        chA.mean_Tb_real_sky = mean(BrightnessTemperature(seq.chA.cxs.x(a_), real(seq.chA.mrad.y(seq.logi.Sky,a_))));
        chB.mean_Tb_real_sky = mean(BrightnessTemperature(seq.chB.cxs.x(b_), real(seq.chB.mrad.y(seq.logi.Sky,b_))));
        chA.Tb_real_sky = (BrightnessTemperature(seq.chA.cxs.x(a_), real(seq.chA.mrad.y(seq.logi.Sky,a_))));
        chB.Tb_real_sky = (BrightnessTemperature(seq.chB.cxs.x(b_), real(seq.chB.mrad.y(seq.logi.Sky,b_))));
...
% So the idea is to compare the difference in final calibration when
% different NLC corrections are applied to each scan direction before
% eventual averaging versus applying a scan-directionally averaged nlc
% correction to each direction before averaging.

% There is so little difference between the a2 and zpd values between the
% _raw and _pc versions (2:800 compared to 60:800) that I'm going to just
% examine the difference for the "raw",
% [assist.chA.cxs.y,assist.chA.nlc] = NLC_SSEC_raw_zpd(assist,nlc); 
assist_ = assist;
assist_pc = assist;

[assist,nlc.raw_F] = NLC_split_det_scandir(assist, nlc.raw_F, assist.chA.cxs.zpd_raw,assist.logi.F);
[assist,nlc.raw_R] = NLC_split_det_scandir(assist, nlc.raw_R, assist.chA.cxs.zpd_raw,assist.logi.R);
[assist_avg,nlc.raw_avg] = NLC_split_det_scandir(assist_, nlc.raw_avg, assist_.chA.cxs.zpd_raw,assist_.logi.F|assist_.logi.R);

[assist_pc,nlc.PC_F] = NLC_split_det_scandir(assist_pc, nlc.PC_F, assist_pc.chA.cxs.zpd_ps,assist_pc.logi.F);
[assist_pc,nlc.PC_R] = NLC_split_det_scandir(assist_pc, nlc.PC_R, assist_pc.chA.cxs.zpd_ps,assist_pc.logi.R);
[assist_pc_avg,nlc.PC_avg] = NLC_split_det_scandir(assist_, nlc.PC_avg, assist_.chA.cxs.zpd_ps,assist_.logi.F|assist_.logi.R);
...
assist_avg.chB = assist.chB;
assist_pc.chB = assist.chB;
assist_pc_avg.chB = assist.chB;
...
assist_avg = rad_cal_def_M(assist_avg, emis);
assist_pc = rad_cal_def_M(assist_pc, emis);
assist_pc_avg = rad_cal_def_M(assist_pc_avg, emis);
...
assist_avg.chA.spc.y(assist_avg.logi.F,:) = RadiometricCalibration_4(assist_avg.chA.cxs.y(assist_avg.logi.F,:), assist_avg.chA.cal_F);
assist_avg.chA.spc.y(assist_avg.logi.R,:) = RadiometricCalibration_4(assist_avg.chA.cxs.y(assist_avg.logi.R,:), assist_avg.chA.cal_R);

assist_pc.chA.spc.y(assist.logi.F,:) = RadiometricCalibration_4(assist_pc.chA.cxs.y(assist.logi.F,:), assist_pc.chA.cal_F);
assist_pc.chA.spc.y(assist.logi.R,:) = RadiometricCalibration_4(assist_pc.chA.cxs.y(assist.logi.R,:), assist_pc.chA.cal_R);

assist_pc_avg.chA.spc.y(assist_avg.logi.F,:) = RadiometricCalibration_4(assist_pc_avg.chA.cxs.y(assist_pc_avg.logi.F,:), assist_pc_avg.chA.cal_F);
assist_pc_avg.chA.spc.y(assist_avg.logi.R,:) = RadiometricCalibration_4(assist_pc_avg.chA.cxs.y(assist_pc_avg.logi.R,:), assist_pc_avg.chA.cal_R);
...
assist_avg.chA.mrad.y = ApplyFFOVCorr(assist_avg.chA.cxs.x, assist_avg.chA.spc.y,0.0225);
assist_pc.chA.mrad.y = ApplyFFOVCorr(assist.chA.cxs.x, assist_pc.chA.spc.y,0.0225);
assist_pc_avg.chA.mrad.y = ApplyFFOVCorr(assist_avg.chA.cxs.x, assist_pc_avg.chA.spc.y,0.0225);
...
assist_avg.down.time = retime(1,:);
assist_avg.down.isSky = downsample(assist_avg.logi.Sky,2.*N)>0;
assist_avg.down.isHBB = downsample(assist_avg.logi.H,2.*N)>0;
assist_avg.down.isABB = downsample(assist_avg.logi.A,2.*N)>0;
...
% Ch A with averaged nlc
assist_avg.down.chA.ifg.F = downsample(assist_avg.chA.y(assist.logi.F,:),N);
assist_avg.down.chA.cxs.F = downsample(assist_avg.chA.cxs.y(assist.logi.F,:),N);
assist_avg.down.chA.mrad.F = downsample(assist_avg.chA.mrad.y(assist.logi.F,:),N);
assist_avg.down.chA.var.F = downvariance(assist_avg.chA.mrad.y(assist.logi.F,:),N);
% with pc
% Ch A
assist_pc.down.chA.ifg.F = downsample(assist_pc.chA.y(assist.logi.F,:),N);
assist_pc.down.chA.cxs.F = downsample(assist_pc.chA.cxs.y(assist.logi.F,:),N);
assist_pc.down.chA.mrad.F = downsample(assist_pc.chA.mrad.y(assist.logi.F,:),N);
assist_pc.down.chA.var.F = downvariance(assist_pc.chA.mrad.y(assist.logi.F,:),N);

% Ch A with averaged nlc
assist_pc_avg.down.chA.ifg.F = downsample(assist_pc_avg.chA.y(assist.logi.F,:),N);
assist_pc_avg.down.chA.cxs.F = downsample(assist_pc_avg.chA.cxs.y(assist.logi.F,:),N);
assist_pc_avg.down.chA.mrad.F = downsample(assist_pc_avg.chA.mrad.y(assist.logi.F,:),N);
assist_pc_avg.down.chA.var.F = downvariance(assist_pc_avg.chA.mrad.y(assist.logi.F,:),N);
...
% Ch A
assist_avg.down.chA.ifg.R = downsample(assist_avg.chA.y(assist.logi.R,:),N);
assist_avg.down.chA.cxs.R = downsample(assist_avg.chA.cxs.y(assist.logi.R,:),N);
assist_avg.down.chA.mrad.R = downsample(assist_avg.chA.mrad.y(assist.logi.R,:),N);
assist_avg.down.chA.var.R = downvariance(assist_avg.chA.mrad.y(assist.logi.R,:),N);

% Ch A
assist_pc.down.chA.ifg.R = downsample(assist_pc.chA.y(assist.logi.R,:),N);
assist_pc.down.chA.cxs.R = downsample(assist_pc.chA.cxs.y(assist.logi.R,:),N);
assist_pc.down.chA.mrad.R = downsample(assist_pc.chA.mrad.y(assist.logi.R,:),N);
assist_pc.down.chA.var.R = downvariance(assist_pc.chA.mrad.y(assist.logi.R,:),N);

% Ch A
assist_pc_avg.down.chA.ifg.R = downsample(assist_pc_avg.chA.y(assist.logi.R,:),N);
assist_pc_avg.down.chA.cxs.R = downsample(assist_pc_avg.chA.cxs.y(assist.logi.R,:),N);
assist_pc_avg.down.chA.mrad.R = downsample(assist_pc_avg.chA.mrad.y(assist.logi.R,:),N);
assist_pc_avg.down.chA.var.R = downvariance(assist_pc_avg.chA.mrad.y(assist.logi.R,:),N);
...
assist_avg.down.chA.mrad.y = (assist_avg.down.chA.mrad.F + assist_avg.down.chA.mrad.R)./2;
assist_avg.down.chA.var.y = (assist_avg.down.chA.var.F  + assist_avg.down.chA.var.R);
assist_avg.down.chA.T_bt = BrightnessTemperature(assist.chA.cxs.x, real(assist_avg.down.chA.mrad.y));

assist_pc.down.chA.mrad.y = (assist_pc.down.chA.mrad.F + assist_pc.down.chA.mrad.R)./2;
assist_pc.down.chA.var.y = (assist_pc.down.chA.var.F  + assist_pc.down.chA.var.R);
assist_pc.down.chA.T_bt = BrightnessTemperature(assist_pc.chA.cxs.x, real(assist_pc.down.chA.mrad.y));

assist_pc_avg.down.chA.mrad.y = (assist_pc_avg.down.chA.mrad.F + assist_pc_avg.down.chA.mrad.R)./2;
assist_pc_avg.down.chA.var.y = (assist_pc_avg.down.chA.var.F  + assist_pc_avg.down.chA.var.R);
assist_pc_avg.down.chA.T_bt = BrightnessTemperature(assist_pc.chA.cxs.x, real(assist_pc_avg.down.chA.mrad.y));

assist.down.chB.mrad.y = (assist.down.chB.mrad.F + assist.down.chB.mrad.R)./2;
assist.down.chB.var.y = (assist.down.chB.var.F  + assist.down.chB.var.R);
assist.down.chB.T_bt = BrightnessTemperature(assist.chB.cxs.x, real(assist.down.chB.mrad.y));

figure; ax(1) = subplot(2,1,1); plot(...
   assist.down.chA.mrad.x, mean(real(assist_avg.down.chA.mrad.y(assist.down.isSky,:))),'b-',...
   assist.down.chA.mrad.x, mean(real(assist.down.chA.mrad.y(assist.down.isSky,:))),'r-',...
   assist.down.chA.mrad.x, mean(real(assist_pc_avg.down.chA.mrad.y(assist.down.isSky,:))),'g-',...
   assist.down.chA.mrad.x, mean(real(assist_pc.down.chA.mrad.y(assist.down.isSky,:))),'k-' ...
); 
legend('average nlc','directional nlc','avg pc','dir pc')
ax(2) = subplot(2,1,2); plot(assist_avg.down.chA.mrad.x, mean(real(assist_avg.down.chA.mrad.y(assist.down.isSky,:)))- mean(real(assist.down.chA.mrad.y(assist.down.isSky,:))),'k-'); legend('avg - ind'); linkaxes(ax,'x')
%Dang, this looks too large. Re-check to make sure nlc is being correctly
%applied

figure; plot(assist_avg.down.chA.mrad.x, mean(real(assist_avg.down.chA.mrad.y(assist.down.isSky,:))),'b-',...
   assist_avg.down.chA.mrad.x, mean(real(assist.down.chA.mrad.y(assist.down.isSky,:))),'r-',...
   assist.down.chB.mrad.x, mean(real(assist.down.chB.mrad.y(assist.down.isSky,:))),'k-'); legend('average nlc','directional nlc', 'chB')
