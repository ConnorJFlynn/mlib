...
%          
         ...
         % Compute fft
         [SPC_H.x,SPC_H.y,SPC_H.Idc] = ShiftIgm2RawSpc(IGM_H.x,IGM_H.y);
         [SPC_A.x,SPC_A.y,SPC_A.Idc] = ShiftIgm2RawSpc(IGM_A.x,IGM_A.y);
         [SPC_C.x,SPC_C.y,SPC_C.Idc] = ShiftIgm2RawSpc(IGM_C.x,IGM_C.y);
         
         %%
         I_H = IGM_H.y;
         [~, I] = max(abs(I_H - mean(I_H)));
         I_H = I_H(I); I_H = SPC_H.Idc; % ala D Turner
         I_A = IGM_A.y;
         [~, I] = max(abs(I_A - mean(I_A)));
         I_A = I_A(I);I_A = SPC_A.Idc; % ala D Turner
         I_C = IGM_C.y;
         [~, I] = max(abs(I_C - mean(I_C)));
         I_C = I_C(I);I_C = SPC_C.Idc; % ala D Turner
         ...
         sub = (CalSet_HA.x>320&CalSet_HA.x<480) | (CalSet_HA.x>1850&CalSet_HA.x<2050);
         a_2(Run_number) = mean(a_2_tmp(sub));
         a_2_(Run_number) = mean(a_2_tmp_(sub));
         nlc_coefs.a2(Run_number,dir_col) = a_2(Run_number);
         nlc_coefs.IHLAB(Run_number,dir_col) = I_H;
         nlc_coefs.ICLAB(Run_number,dir_col) = I_C;
         nlc_coefs.IALAB(Run_number,dir_col) = I_A;
