function nlc_coefs = NLC_FandR_fit_inv_Vdc
...
% version assumes Vdc needs to be inverted in sign compared the form 
% in the AERI paper #2.
emis = load('emis.mat');
...
if abbs==hbbs && hbbs==ln2s
    ...
    for Run_number = Nb_of_Runs:-1:1
        ...
        % Operationally I don't determine the location of the zpd this way
        % anymore, but it doesn't impact the "raw" and "phase-shifted" 
        % differences since both are using the same zpd location
...
% em2 = em1;
% em0 = 0.9.*ones(size(em1));em0 = 1.*ones(size(em1));
...
% BB_0 = em0.*Blackbody(BB_x, T_0)+(1-em0).*Blackbody(BB_x, T_mirror);    
        ...
        CalSet_AC_F = CreateCalibrationFromCXS_(SPC_A_F, T_ABB(Run_number),SPC_C_F, T_CBB,T_ABB(Run_number),emis, ones(size(emis)));  
...
%         V_F_raw = (1/MF) * ((2 + f_back)*(-I_C_F_raw));
        ...
        IGM_H_O_F.y_raw = (IGM_H_F.y + V_H_F_raw(Run_number)).^2; % this should be the full "true" igm
...
%         sub1 = (CalSet_HA_F.x>200&CalSet_HA_F.x<460);
        sub2 = (CalSet_HA_F.x>1800&CalSet_HA_F.x<2000);
%         sub2 = sub1 | sub2;
...
%         a_2_tmp_F__ = a_2_tmp_F__ .* abs(a_2_tmp_F)./a_2_tmp_F;
        ...
        a_3_tmp_F_raw = real(diff_resp_F) ./ real(Diff_Resp_fit_F.y_raw);
        % Option 2 - Real part of ratio
...
%         zoom('on');
        %%
        figure(51);
        plot(CalSet_AC_F.x, real(diff_resp_F)./1000, 'r.')
        hold('on')
        ...
        plot(Diff_Resp_fit_F.x, real(a2_F_raw(Run_number) * Diff_Resp_fit_F.y_raw)./1000, '-');
        %plot(Diff_Resp_fit.x, imag(a_2(Run_number) * Diff_Resp_fit.y), 'c-')
        xlabel('Wavenumber [cm^{-1}]')
        ylabel('R^{HA} - R^{AC} [Counts/(mW/(m^2 sr cm^{-1}))]')
...
%         axis([range(1) range(2) 0 1.2e-3*max(abs(CalSet_HA_F.Resp(mm)))])
        ...
        I_H_R_raw(Run_number) = SPC_H_R.Idc_raw; % ala D Turner
        I_A_R_raw(Run_number) = SPC_A_R.Idc_raw; % ala D Turner
        I_C_R_raw(Run_number) = SPC_C_R.Idc_raw; % ala D Turner
...
%         V_R_raw = (1/MF) * ((2 + f_back)*(-I_C_R_raw));
        ...
        IGM_H_O_R.y_raw = (IGM_H_R.y + V_H_R_raw(Run_number)).^2; % this should be the full "true" igm
...
%         legend('Measured','fitted');
        ...
        figure(55);
        plot(CalSet_AC_R.x, real(diff_resp_R)./1000, 'r.')
        hold('on')
        %plot(CalSet_AC.x, imag(diff_gains), 'g-')
        %       lines = plot(Diff_Resp_fit.x, real(a_2(Run_number) * Diff_Resp_fit.y), 'r-', Diff_Resp_fit.x, real(a_2_(Run_number) * Diff_Resp_fit.y), 'k-')
        plot(Diff_Resp_fit_R.x, real(a2_R_raw(Run_number) * Diff_Resp_fit_R.y_raw)./1000, '-');
        %plot(Diff_Resp_fit.x, imag(a_2(Run_number) * Diff_Resp_fit.y), 'c-')
        xlabel('Wavenumber [cm^{-1}]')
        ylabel('R^{HA} - R^{AC} [Counts/(W/(m^2 sr cm^{-1}))]')
        kk = find(Diff_Resp_fit_R.x >= range(1) & Diff_Resp_fit_R.x <= range(2));
...
%         ymin = 1.2*min([real(diff_resp_R(kk)) imag(diff_resp_R(kk)) ...
%             real(a2_R_raw(Run_number) * Diff_Resp_fit_R.y(kk)) ...
%             imag(a2_R_raw(Run_number) * Diff_Resp_fit_R.y(kk))]);
%         ymax = 1.2*max([real(diff_resp_R(kk)) imag(diff_resp_R(kk)) ...
%             real(a2_R_raw(Run_number) * Diff_Resp_fit_R.y(kk)) ...
%             imag(a2_R_raw(Run_number) * Diff_Resp_fit_R.y(kk))]);
        axis([200 2400 ymin ymax]);
