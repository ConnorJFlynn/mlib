...
assist.chA.zpd_value = annot.zpdValue;
%%
...
a2=(1.20755e-6);
     ...
     IHLAB=(34215.5);
     ICLAB=(42256);     
IH0 = mean(annot.zpdValue(HBB_F|HBB_R));
IH_0 =mean(annot.zpdValue(HBB_F|HBB_R));

zpdv = annot.zpdValue;
assist.chA.Vdc = (annot.zpdValue+(2+fBACK).*(IHLAB-ICLAB -IH_0))./(-MF);
assist.chA.Vm = (zpdv +(2+fBACK).*(IHLAB - IH0 -ICLAB))./(-MF);
assist.chA.nonlinearity_factor = 1+ 2.*a2.*assist.chA.Vdc;
 figure; plot([1:length(assist.time)],assist.chA.nonlinearity_factor,'-o')

% igm2.x = assist.chA.flynn_mat.x;
% igm2.y = assist.chA.flynn_mat.y.^2;
% tmp = RawIgm2RawSpc(igm2);
% assist.chA.nonlinearity_offset = a2.*tmp.y; 
%%
assist.chA.spc = RawIgm2RawSpc(assist.chA.flynn_mat);
assist.chA.spc.y = (assist.chA.spc.y .* (assist.chA.nonlinearity_factor*ones(size(assist.chA.spc.x))));
% non-linear correction parameters:
% correction factor (1+2*a2*V)
...
tmp = RawIgm2RawSpc(igm2);
assist.chA.nonlinearity_offset = a2.*tmp.y; 
plot([1:length(assist.time)],assist.chA.nonlinearity_factor,'-o')
%%
% assist.chA.flynn_mat.y = (assist.chA.flynn_mat.y .* assist.chA.nonlinearity_factor) + assist.chA.nonlinearity_offset;

%    + assist.chA.nonlinearity_offset;
...
assist.chA.cxs.HBB_F.x =assist.chA.spc.x;
assist.chA.cxs.HBB_R.x =assist.chA.spc.x;
assist.chA.cxs.CBB_F.x =assist.chA.spc.x;
assist.chA.cxs.CBB_R.x =assist.chA.spc.x;
assist.chA.cxs.Sky_F.x =assist.chA.spc.x;
assist.chA.cxs.Sky_R.x =assist.chA.spc.x;

assist.chA.cxs.HBB_F.y = assist.chA.spc.y(HBB_F,:);
assist.chA.cxs.HBB_R.y = assist.chA.spc.y(HBB_R,:);
assist.chA.cxs.CBB_F.y = assist.chA.spc.y(CBB_F,:);
assist.chA.cxs.CBB_R.y = assist.chA.spc.y(CBB_R,:);
assist.chA.cxs.Sky_F.y = assist.chA.spc.y(Sky_F,:);
assist.chA.cxs.Sky_R.y = assist.chA.spc.y(Sky_R,:);
...
assist.chA.good_wn = assist.chA.mrad.Sky.x>550 &assist.chA.mrad.Sky.x<1800;
%%
...
ylim('auto');
%%
proc_dir = dir([pname,'Processed_Without_NLC*']);
if isempty(proc_dir)
   proc_dir = dir([pname,'Processed_With_NLC*']);
end
if ~isempty(proc_dir)
   proc_A_dir = [pname, proc_dir(1).name,'\Processed_A\Magnitude\'];
   proc_A_mat = loadinto([proc_A_dir, 'A1_U.igm.coad.mrad.coad']);
   proc_B_dir = [pname, proc_dir(1).name,'\Processed_B\Magnitude\'];
   proc_B_mat = loadinto([proc_B_dir, 'B1_U.igm.coad.mrad.coad']);
end

%%
andre_wn_offset = 0.0693; %subtract from Andre x
...
proc_A_mat.good_wn = proc_A_mat.x>550 & proc_A_mat.x<1800;
...
%
figure; plot(proc_A_mat.x(proc_A_mat.good_wn)-andre_wn_offset, 1000*proc_A_mat.subfiles{1}.subfileData(proc_A_mat.good_wn),'k-',...
   proc_B_mat.x(proc_B_mat.good_wn)-andre_wn_offset,1000*proc_B_mat.subfiles{1}.subfileData(proc_B_mat.good_wn),'k-',...
   assist.chA.mrad.Sky.x(assist.chA.good_wn), assist.chA.mrad.Sky.y(1,assist.chA.good_wn),'r-',...
   assist.chB.mrad.Sky.x(assist.chB.good_wn), assist.chB.mrad.Sky.y(1,assist.chB.good_wn),'r-')
   legend('Andre A','Andre B','Flynn A','Flynn B');
   %%
   (trapz(assist.chA.mrad.Sky.x(assist.chA.good_wn), assist.chA.mrad.Sky.y(1,assist.chA.good_wn))-trapz(proc_A_mat.x(proc_A_mat.good_wn),1000*proc_A_mat.subfiles{1}.subfileData(proc_A_mat.good_wn)))./trapz(assist.chA.mrad.Sky.x(assist.chA.good_wn), assist.chA.mrad.Sky.y(1,assist.chA.good_wn))
