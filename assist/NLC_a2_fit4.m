function nlc_coefs = NLC_a2_fit4
...
%             dpolF = -1;  % for B
            ...
            V_H_F_raw(Run_number) = (dpolF/MF) * ((2 + f_back)*(-I_H_F_raw(Run_number) + I_H_F_raw(Run_number) -I_C_F_raw(Run_number)) + I_H_F_raw(Run_number));
            V_A_F_raw(Run_number) = (dpolF/MF) * ((2 + f_back)*(-I_H_F_raw(Run_number) + I_H_F_raw(Run_number) -I_C_F_raw(Run_number)) + I_A_F_raw(Run_number));
            V_C_F_raw(Run_number) = (dpolF/MF) * ((2 + f_back)*(-I_H_F_raw(Run_number) + I_H_F_raw(Run_number) -I_C_F_raw(Run_number)) + I_C_F_raw(Run_number));
            
            V_H_F_pc(Run_number) = (dpolF/MF) * ((2 + f_back)*(-I_H_F_pc(Run_number) + I_H_F_pc(Run_number) -I_C_F_pc(Run_number)) + I_H_F_pc(Run_number));
            V_A_F_pc(Run_number) = (dpolF/MF) * ((2 + f_back)*(-I_H_F_pc(Run_number) + I_H_F_pc(Run_number) -I_C_F_pc(Run_number)) + I_A_F_pc(Run_number));
            V_C_F_pc(Run_number) = (dpolF/MF) * ((2 + f_back)*(-I_H_F_pc(Run_number) + I_H_F_pc(Run_number) -I_C_F_pc(Run_number)) + I_C_F_pc(Run_number));
            ...
            sub1 = (CalSet_HA_F.x>250&CalSet_HA_F.x<350);
            ...
            set(txt_51,'unit','normalized','position',[.05,.95],'verticalalign','cap',...
                ...
                'fontname','Tahoma','fontweight','demi','backgroundcolor','w');
                    ...
                    title({'Re(Resp_HA)- Re(Resp_AC) for pc ZPD';...
                        [datestr(datenum(V),'yyyy-mm-dd'),', Scan direction: Forward:',num2str(Run_number)]},'interp','none');
                    %legend('Real Measured', 'Imaginary Measured', 'Real Model fit', 'Imaginary Model fit')
                    %       legend('Measured',sprintf('a2 = %2.3e',a_2(Run_number)),
                    %       sprintf('(2nd) %2.3e',a_2_(Run_number)));
                    lg = legend('Measured','fitted'); set(lg,'interp','none');
            ...
            % Here is where I begin to flip the Vdc sign
            dpolR = 1;
            V_H_R_raw(Run_number) = (dpolR/MF) * ((2 + f_back)*(-I_H_R_raw(Run_number) + I_H_R_raw(Run_number) -I_C_R_raw(Run_number)) + I_H_R_raw(Run_number));
            V_A_R_raw(Run_number) = (dpolR/MF) * ((2 + f_back)*(-I_H_R_raw(Run_number) + I_H_R_raw(Run_number) -I_C_R_raw(Run_number)) + I_A_R_raw(Run_number));
            V_C_R_raw(Run_number) = (dpolR/MF) * ((2 + f_back)*(-I_H_R_raw(Run_number) + I_H_R_raw(Run_number) -I_C_R_raw(Run_number)) + I_C_R_raw(Run_number));
            
            V_H_R_pc(Run_number) = (dpolR/MF) * ((2 + f_back)*(-I_H_R_pc(Run_number) + I_H_R_pc(Run_number) -I_C_R_pc(Run_number)) + I_H_R_pc(Run_number));
            V_A_R_pc(Run_number) = (dpolR/MF) * ((2 + f_back)*(-I_H_R_pc(Run_number) + I_H_R_pc(Run_number) -I_C_R_pc(Run_number)) + I_A_R_pc(Run_number));
            V_C_R_pc(Run_number) = (dpolR/MF) * ((2 + f_back)*(-I_H_R_pc(Run_number) + I_H_R_pc(Run_number) -I_C_R_pc(Run_number)) + I_C_R_pc(Run_number));
            ...
            %
                    ...
                    %
                    figure(54);
                    plot(CalSet_AC_R.x, real(diff_resp_R)./1000, 'r.',CalSet_AC_F.x(sub), real(diff_resp_R(sub))./1000, 'c.')
                    hold('on')
                    %plot(CalSet_AC.x, imag(diff_gains), 'g-')
                    %       lines = plot(Diff_Resp_fit.x, real(a_2(Run_number) * Diff_Resp_fit.y), 'r-', Diff_Resp_fit.x, real(a_2_(Run_number) * Diff_Resp_fit.y), 'k-')
