function assist = assist_annew_3(pname)% ASSIST annew
% Looking at even and odd reflections yielding an igram of even/odd length.
...
[oddshifted.F.y] = zpd_reflect_odd(assist.F.chA.x,assist.F.chA.y,zpd_shift_F);
[oddshifted.R.y] = zpd_reflect_odd(assist.R.chA.x,assist.R.chA.y,zpd_shift_R);
...
assist_odd.F.chA.cxs.y = fft(zpd_reflect_odd(assist.F.chA.x,assist.F.chA.y,zpd_shift_F),[],2);
assist_odd.R.chA.cxs.y = fft(zpd_reflect_odd(assist.R.chA.x,assist.R.chA.y,zpd_shift_R),[],2);
half = (max(size(assist_odd.F.chA.cxs.y))-1)./2;
odd_x = [-1*half:half];
odd_x_ = ifftshift(odd_x)/Laser_wl/length(odd_x);  
%%
figure(2); 
...
plot(assist.chA.cxs.x(chA_good), real(assist.F.chA.cxs.y(1,chA_good)), 'r',...
   ...
   odd_x_(chA_good), imag(assist_odd.F.chA.cxs.y(1,chA_good)), 'k');
title('Raw spectra from shifted igrams: circular, even and odd reflections');
ylabel('raw cts')
legend('real circ','real even','imag circ','imag even','imag odd');
s1(2) = subplot(3,1,2); 
