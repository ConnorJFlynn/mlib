function assist = assist_annew_14 % ASSIST annew
...
pause(.01);
if pick==1
    raw_files = getfullname_('*_ann_*.csv','assist_dir','Select a file in the directory to bundle.');
    if ~iscell(raw_files)
        raw_files = {raw_files};
    end
...
% [assist.chA.cxs.x,assist.chA.cxs.y,assist.chA.cxs.Idc,assist.chA.cxs.Idc_raw] = ShiftIgm2RawSpc(assist.chA.x,assist.chA.y,assist.chA.laser_wl);
% RawIgm2RawSpc expects shifted ZPD with ZPD at 0th index.
...
%Derived at LR_Tec by Connor from TMP LN2 test after det repair.
% This is a splitter detector.
% It appears that the sense of the ZPD is opposite to that assumed in
% SSEC's Vdc model, so we're testing flipping
if assist.time(1)>datenum('2014-03-27','yyyy-mm-dd')
nlc.a2 =  -(4.625e-08 + 4.837e-08)./2;
nlc.IHLAB = (-1.838e+05 -1.806e+05)./2;
nlc.ICLAB = (3.062e+05 +3.025e+05)./2;

nlc_pc.a2 =  -(4.627e-08 + 4.844e-08)./2;
nlc_pc.IHLAB = (-1.866e+05 -1.866e+05)./2;
nlc_pc.ICLAB = (3.082e+05 +3.083e+05)./2;
elseif assist.time(1)>datenum('2013-03-13','yyyy-mm-dd')
nlc.a2 =  -(6.382e-08 + 6.390e-08)./2;
nlc.IHLAB = ( -4.512e+04  -4.669e+04)./2;
nlc.ICLAB = (7.745e+04 +8.001e+04)./2;

nlc_pc.a2 =  -(6.073e-08 + 6.077e-08)./2;
nlc_pc.IHLAB = ( -4.613e+04 -4.670e+04)./2;
nlc_pc.ICLAB = (7.875e+04 +8.002e+04)./2;
   
elseif assist.time(1)>datenum('2011-02-02','yyyy-mm-dd')

nlc.a2 =   -5.7004e-08;
nlc.IHLAB =  -7.7944e+04;
nlc.ICLAB =  8.5740e+04;

nlc_pc.a2 =  -5.7050e-08;
nlc_pc.IHLAB =  -7.9317e+04;
nlc_pc.ICLAB = 8.6943e+04;
   
   
elseif assist.time(1)>datenum('2010-09-29','yyyy-mm-dd')
end
...
[assist.chA.cxs.y,assist.chA.nlc] = NLC_split_det(assist, nlc, assist.chA.cxs.zpd_raw);
[assist_pc.chA.cxs.y,assist_pc.chA.nlc] = NLC_split_det(assist_pc, nlc_pc,assist.chA.cxs.zpd_ps );
% [assist_ps.chA.cxs.y,assist_ps.chA.nlc] = NLC_SSEC_full(assist_ps, nlc_ps,assist.chA.cxs.zpd_ps);


% assist = assist_noNLC;

% [~,maxii] = max(abs(assist.chA.y),[],2);
% assist.chA.maxii = maxii;
% assist.chA.zpd_shift = maxii-length(assist.chA.x)./2 -1;
% %Position detected zpd at center +1.
% assist.chA.y =  sideshift(assist.chA.x, assist.chA.y, assist.chA.zpd_shift);
% % ShiftIgm2RawSpc expects centered zpd and then applies fftshift to yield ZPD at 0th index.
% % [assist.chA.cxs.x,assist.chA.cxs.y,assist.chA.cxs.Idc,assist.chA.cxs.Idc_raw] = ShiftIgm2RawSpc(assist.chA.x,assist.chA.y,assist.chA.laser_wl);
...
% 
% [~,maxii] = max(abs(assist.chB.y),[],2);
% assist.chB.maxii = maxii;
% assist.chB.zpd_shift = maxii-length(assist.chB.x)./2 -1;
% ABB_F = mean(assist.chB.y(assist.logi.ABB_F,:));
% ABB_R = mean(assist.chB.y(assist.logi.ABB_R,:));
% [~, maxii_ABB_F] = max(abs(ABB_F));
% [~, maxii_ABB_R] = max(abs(ABB_R));
% HBB_F = mean(assist.chB.y(assist.logi.HBB_F,:));
% HBB_R = mean(assist.chB.y(assist.logi.HBB_R,:));
% [~, maxii_HBB_F] = max(abs(HBB_F));
% [~, maxii_HBB_R] = max(abs(HBB_R));
% Sky_F = mean(assist.chB.y(assist.logi.Sky_F,:));
% Sky_R = mean(assist.chB.y(assist.logi.Sky_R,:));
% [~, maxii_Sky_F] = max(abs(Sky_F));
% [~, maxii_Sky_R] = max(abs(Sky_R));
% 
% maxii(assist.logi.ABB_F) = maxii_ABB_F;
% maxii(assist.logi.ABB_R) = maxii_ABB_R;
% maxii(assist.logi.HBB_F) = maxii_HBB_F;
% maxii(assist.logi.HBB_R) = maxii_HBB_R;
% maxii(assist.logi.Sky_F) = maxii_Sky_F;
% maxii(assist.logi.Sky_R) = maxii_Sky_R;
% 
% 
% 
% assist.chB.maxii = maxii;
...
% [zpd_loc,zpd_mag] = find_zpd_fft(assist.chB.y);
