function [C_corr,nlc] = NLC_SSEC_raw_zpd(assist, nlc);
% [C_corr,nlc] = NLC_SSEC(cxs, nlc);
...
IHatZPD_F = mean(assist.chA.cxs.zpd_raw(assist.logi.HBB_F));
static_shift_F = (2+nlc.fBACK).*(nlc.IHLAB-nlc.ICLAB -IHatZPD_F);
% IMatZPD_F = assist.chA.y(assist.logi.F,1);
IMatZPD_F = assist.chA.cxs.zpd_raw(assist.logi.F);
% For this to work, the IMatZPD and lab-measured values must both be using
% the raw ifg amplitude or the magnitude of the ifg of the absolute
% magnitude spectra. 
...
%new
IHatZPD_R = mean(assist.chA.cxs.zpd_raw(assist.logi.HBB_R));
static_shift_R = (2+nlc.fBACK).*(nlc.IHLAB-nlc.ICLAB -IHatZPD_R);
% IMatZPD_F = assist.chA.y(assist.logi.F,1);
IMatZPD_R = assist.chA.cxs.zpd_raw(assist.logi.R);
nlc.Vdc_R = (1./(-nlc.MF)).*(static_shift_R + IMatZPD_R);
% syntax of RawIgm2RawSpc:;
% [assist.chA.cxs.x,assist.chA.cxs.y,assist.chA.cxs.zpd,assist.chA.cxs.zpd_raw] = RawIgm2RawSpc(assist.chA.x,assist.chA.y,assist.chA.laser_wl);
