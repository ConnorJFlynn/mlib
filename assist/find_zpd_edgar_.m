function [zpd_loc,zpd_mag] = find_zpd_edgar(y)
...
for s = size(y,1):-1:1
   ...
   %%
%    plot([1:size(y,2)]- 32768/2,y(s,:),'x-', [1:size(y,2)]- 32768/2,y(s,:),'o-')
   %%
   % positive symmetry test
   imax_i(s) = imax_i(s) -  double(y(s,imax_i(s))<imax(s)) ;
   ii = imax_i(s);
   left_valley = y(s,ii);
   while left_valley >= y(s,ii-1)
      ii = ii -1;
      left_valley = y(s,ii);
   end
   ii = imax_i(s);
      right_valley = y(s,ii);
   while right_valley >= y(s,ii+1)
      ii = ii +1;
      right_valley = y(s,ii);
   end
dpos = abs(right_valley - left_valley);
 ...
 % negative symmetry test
   imin_i(s) = imin_i(s) - double(y(s,imin_i(s)) >= imin(s));

   ii = imin_i(s);
   left_peak = y(s,ii);
   while left_peak <= y(s,ii-1)
      ii = ii -1;
      left_peak = y(s,ii);
   end
   ii = imin_i(s);
      right_peak = y(s,ii);
   while right_peak <= y(s,ii+1)
      ii = ii +1;
      right_peak = y(s,ii);
   end
