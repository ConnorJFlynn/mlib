function [clean] = xap_white(infile);

if ~isavar('infile')
   infile = getfullname('white*.dat','xap_test','Select a white filter file.');
end
...
%    clap_.fname = unique([clap.fname,clap2.fname]);
   ...
   [~, fname] = fileparts(raw.fname);
      ...
      start_time = datenum([d_str, ' ',time_str],'yyyymmdd HHMM');
   ...
   clean.filter_id = unique(clap.filter_id); 
   clean.blue(10) = mean(clap.signal_blue(10,rem(clap.spot_active,2)==0),2);
   clean.green(10) = mean(clap.signal_green(10,rem(clap.spot_active,2)==0),2);
   clean.red(10) = mean(clap.signal_red(10,rem(clap.spot_active,2)==0),2);
   clean.blue(9) = mean(clap.signal_blue(9,rem(clap.spot_active,2)==1),2);
   clean.green(9) = mean(clap.signal_green(9,rem(clap.spot_active,2)==1),2);
   clean.red(9) = mean(clap.signal_red(9,rem(clap.spot_active,2)==1),2);

   for spot = 8:-1:1
      clean.blue(spot) = mean(clap.signal_blue(spot,clap.spot_active==spot),2);
      clean.green(spot) = mean(clap.signal_green(spot,clap.spot_active==spot),2);
      clean.red(spot) = mean(clap.signal_red(spot,clap.spot_active==spot),2);
   end
