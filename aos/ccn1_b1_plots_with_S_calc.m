...
T_bar = 273.17+((ccn1.vdata.T_read_TEC3 + ccn1.vdata.T_read_TEC1)./2);
dT3 = ccn1.vdata.T_read_TEC3-ccn1.vdata.T_read_TEC1;
Q = ccn1.vdata.Q_sample+ccn1.vdata.Q_sheath;
P = ccn1.vdata.P_sample;
B1 = 877; %B1 = 877 K m^-1
B2 = 1.3; %B2 = 1.3 m^-1;
B3 = 3.75e-4; %B3 = 3.75e-4 J / (m s K)
B4 = 1.27e-5; %B4 = 1.27e-5 J/(m s K^2)
B5 = 2.24e-5; %B5 = 2.24e-5 N /(s K^1.94)
TQP = dT3 .* Q .* P;
TBBT = T_bar.*(B3+B4.*T_bar);
B5T = (B5.*T_bar.^1.94); % 
S1 = 100.*B1.*TQP.*(1./(T_bar - B2.*TQP./TBBT).^2).*(1./TBBT - 1./(B5.*T_bar.^1.94));
figure; plot(serial2doys(ccn1.time), S1,'c.');
