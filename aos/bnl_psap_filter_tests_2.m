...
function bnl_psap_filter_tests_2(ss)
...
tap = proc_tap_me(tap_);tap.fname = tap_.fname; tap.pname = tap_.pname;
...
for sn = 1:length(SN)
   ...
   file_out = ['filter_test.',NNN,'.from_',datestr(psap.time(1),'yyyymmdd_HHMM')...
      ,'-',datestr(psap.time(end),'yyyymmdd_HHMM'),'.v1.csv'];
   tic
   disp(['Writing ',NNN])
