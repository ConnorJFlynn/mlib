...
% I have been able to reproduce the AAF front-panel Ba_(BGR) values from Ba_(BGR)_sm (raw) by applying
% Weiss-Bond (not WBO) and by accounting for the difference between the internal and front-panel mass flows. 

figure; plot(serial2Hh(psapo.time), psapo.mass_flow_last,'.')

figure; plot(psapi.time, psapi.Ba_B,'r.'); legend('psapi Ba_F_P'); dynamicDateTicks
figure; plot(psapo.time, [psapo.Ba_B, psapo.Ba_B_sm, psapo.Ba_B_sm_WBO],'.'); dynamicDateTicks; legend('Ba','Ba sm','Ba sm WBO')
...
figure; plot(psap_b1.time(~NaNs), psap_b1.vdata.transmittance_blue(~NaNs),'c.'); dynamicDateTicks
