...
% Second year, 2014, NOAA flow reading is faulty
...
maopsap1mM1 = load(['F:\xfer\asr_2017\from_DMF_project\maoaospsap3w1mM1.b1\maoaospsap3w1mM1.mat']);
maopsap1mS1 = load(['F:\xfer\asr_2017\mao\maoaospsap3w1mS1.b1\maoaospsap3w1mS1.mat']);
...
figure;scatter(M1_1um.vdata.Ba_G_Weiss(mins), S1_1um.vdata.Ba_G_Weiss(sinm),25,serial2doys(M1_1um.time(mins)),'filled'); v = axis;
hold('on'); plot( [0 max(v)], [0 max(v)],'r--');
...
title({'PSAP "Green" Absorption from Co-deployed AOS suites at Brazil AMF 2015';'(Only coincident measurements passing all QC tests are shown)'})
...
dstr = [datestr(dates(1),'mmm'),'-' datestr(dates(2),'mmm'),datestr(dates(1),' yyyy')];
