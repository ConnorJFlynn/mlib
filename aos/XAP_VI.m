...
while ~don
   ...
   while (toc<update) && ~don
    ...
    if isavar('sb')
       xl = xlim;
       xl_j = interp1(Hs, [1:length(Hs)],xl(1),'nearest','extrap');
    else
       xl_j = [];
    end
   ...
   if ~don2
      ...
      xl = xlim; xl_i = interp1(Hs, [1:length(Hs)],xl(1),'nearest','extrap');
      ...
      if ~isempty(xl_j) && xl_i~=xl_j
      M = menu('Select an option for x-axis','Trim to lower x-limit','Reset entirely', 'XAP Adjust','Cancel')
      xl_ = xlim;
      keep = true(size(xap_sig_.time));
      if M ==1
         keep = Hs >= xl_(1);
      elseif M ==2
         keep(1:end-2) = false
      elseif M== 3 
          XAP_adjust(xap, xap_path, xap_filter_file);
      end
...
function XAP_adjust(xap, xap_path, xap_filter_file )
len = 462;
