function [xap, raw] = rd_xap2(infile);
% [xap, raw] = rd_xap2(infile);
% Re-writing to read the entire xap file instead of line-by-line. 
% Will catch broken lines by mismatch between last and first cell lengths.
...
if iscell(infile)&&length(infile)>1
   ...
   if length(A)==50 && ~isempty(A{end})
      ...
      YY = A{1}; HH = A{2}; raw.DT = [string(YY) + string(' ') + string(HH)];
      A(1:2) = [];
