function [clap, raw] = rd_tap_putty(infile);
...
if iscell(infile)&&length(infile)>1
   [clap, raw] = rd_tap_putty(infile{1});
   raw.time = clap.time; clap.fname = raw.fname; 
   [clap2, raw2] = rd_tap_putty(infile(2:end));
...
%    clap_.fname = unique([clap.fname,clap2.fname]);
   ...
   clap.norm_blue = clap.signal_blue./clap.signal_blue(:,1);
   clap.norm_green = clap.signal_green./clap.signal_green(:,1);
   clap.norm_red = clap.signal_red./clap.signal_red(:,1);
   % These raw Tr values will be re-pinned accordingly at times corresponding to spot-advances.
   clap.Tr_blue = NaN(size(clap.norm_blue)); clap.Tr_green = clap.Tr_blue; clap.Tr_red = clap.Tr_blue;
   clap.Tr_blue(1:2:7,:) = clap.norm_blue(1:2:7,:)./clap.norm_blue(9,:); clap.Tr_blue(9,:) = clap.norm_blue(9,:)./clap.norm_blue(10,:);
   clap.Tr_blue(2:2:8,:) = clap.norm_blue(2:2:8,:)./clap.norm_blue(10,:); clap.Tr_blue(10,:) = clap.norm_blue(10,:)./clap.norm_blue(9,:);
   clap.Tr_green(1:2:7,:) = clap.norm_green(1:2:7,:)./clap.norm_green(9,:); clap.Tr_green(9,:) = clap.norm_green(9,:)./clap.norm_green(10,:);
   clap.Tr_green(2:2:8,:) = clap.norm_green(2:2:8,:)./clap.norm_green(10,:); clap.Tr_green(10,:) = clap.norm_green(10,:)./clap.norm_green(9,:);
   clap.Tr_red(1:2:7,:) = clap.norm_red(1:2:7,:)./clap.norm_red(9,:); clap.Tr_red(9,:) = clap.norm_red(9,:)./clap.norm_red(10,:);
   clap.Tr_red(2:2:8,:) = clap.norm_red(2:2:8,:)./clap.norm_red(10,:); clap.Tr_red(10,:) = clap.norm_red(10,:)./clap.norm_red(9,:); 
   % c343ef6c
