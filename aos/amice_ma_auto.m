...
%    ma.AAE1_(t) = -(P_aae);
% end
% figure; plot(ma.time, ma.Bap1_raw(:,2),'xm-'); dynamicDateTicks; sgtitle(ma_name);
% menu('Zoom in to desired region and hit OK when done','OK');
% xl = xlim; xl_ = ma.time>xl(1)&ma.time<xl(2);
% time = ma.time(xl_); 
% Bap = Bap_ss(ma.time, ma.flow1_LPMsm, ma.Tr1, 1,7);
% [v,vAAE] = multi_allan(ma.nm,time, Bap(xl_,:));
% figure; plot(v(2).t, v(2).retval,'r--', vAAE(1).t, vAAE(1).retval,'k--'); logy; logx
% 
