...
% 
% figure; plot(serial2hs(koontz.time), koontz.vdata.transmittance_green, 'ko',...
%    serial2hs(psapr_00.time), psapr_00.Tr_grn,'r.');legend('koontz','psapr 00')

psapr_00.Tr_grn = psapr_00.grn_rel./psapr_00.grn_rel(1691);
psapr_00.Tr_blu = psapr_00.blu_rel./psapr_00.blu_rel(1691);
psapr_00.Tr_red = psapr_00.red_rel./psapr_00.red_rel(1691);
psapr_00.Tr_grn_60s = smooth(psapr_00.Tr_grn,60);
psapr_00.Tr_blu_60s = smooth(psapr_00.Tr_blu,60);
psapr_00.Tr_red_60s = smooth(psapr_00.Tr_red,60);
psapr_00.Tr_grn_15s = smooth(psapr_00.Tr_grn,15);
psapr_00.Tr_blu_15s = smooth(psapr_00.Tr_blu,15);
psapr_00.Tr_red_15s = smooth(psapr_00.Tr_red,15);

K0 = 923.6; K1 = 752.8; K2 = 38.2;
sample_flow = (-K1 + (K1.^2 - 4.*(K0-double(psapr_00.flow_AD)).*K2).^0.5)./(2.*K2) ;
sample_flow = round(smooth(sample_flow,60)*1000)./1000;
dV = zeros(size(sample_flow));
abs_blu = zeros(size(sample_flow));
abs_grn = zeros(size(sample_flow));
abs_red = zeros(size(sample_flow));
abs_blu_60s = zeros(size(sample_flow));
abs_grn_60s = zeros(size(sample_flow));
abs_red_60s = zeros(size(sample_flow));
abs_blu_15s = zeros(size(sample_flow));
abs_grn_15s = zeros(size(sample_flow));
abs_red_15s = zeros(size(sample_flow));

psapr_00.Tr_blu_2 = [psapr_00.Tr_blu(2:end);psapr_00.Tr_blu(end)];
psapr_00.Tr_grn_2 = [psapr_00.Tr_grn(2:end);psapr_00.Tr_grn(end)];
psapr_00.Tr_red_2 = [psapr_00.Tr_red(2:end);psapr_00.Tr_red(end)];
   abs_blu_ = log(psapr_00.Tr_blu)- log(psapr_00.Tr_blu_2);
   abs_grn_ = log(psapr_00.Tr_grn)- log(psapr_00.Tr_grn_2);
   abs_red_ = log(psapr_00.Tr_red)- log(psapr_00.Tr_red_2);


for tt = 61:length(sample_flow)
   dV(tt) = trapz(psapr_00.time(tt-60:tt)*24*60, sample_flow(tt-60:tt));
   abs_blu(tt) = log(psapr_00.Tr_blu(tt-60))- log(psapr_00.Tr_blu(tt));
   abs_grn(tt) = log(psapr_00.Tr_grn(tt-60))- log(psapr_00.Tr_grn(tt));
   abs_red(tt) = log(psapr_00.Tr_red(tt-60))- log(psapr_00.Tr_red(tt));
   abs_blu_60s(tt) = log(psapr_00.Tr_blu_60s(tt-60))- log(psapr_00.Tr_blu_60s(tt));
   abs_grn_60s(tt) = log(psapr_00.Tr_grn_60s(tt-60))- log(psapr_00.Tr_grn_60s(tt));
   abs_red_60s(tt) = log(psapr_00.Tr_red_60s(tt-60))- log(psapr_00.Tr_red_60s(tt));
   abs_blu_15s(tt) = log(psapr_00.Tr_blu_15s(tt-60))- log(psapr_00.Tr_blu_15s(tt));
   abs_grn_15s(tt) = log(psapr_00.Tr_grn_15s(tt-60))- log(psapr_00.Tr_grn_15s(tt));
   abs_red_15s(tt) = log(psapr_00.Tr_red_15s(tt-60))- log(psapr_00.Tr_red_15s(tt));      
end
% dL = 1000.*dL./17.81;
dL = 1000.*dV./17.5;
Bab_blu_ = 60.*1e6.*abs_blu_./dL;   
Bab_grn_ = 60.*1e6.*abs_grn_./dL;  
Bab_red_ = 60.*1e6.*abs_red_./dL;  

Bab_blu = 1e6.*abs_blu./dL;   
Bab_grn = 1e6.*abs_grn./dL;  
Bab_red = 1e6.*abs_red./dL;  
Bab_blu_60s = 1e6.*abs_blu_60s./dL;   
Bab_grn_60s = 1e6.*abs_grn_60s./dL;  
Bab_red_60s = 1e6.*abs_red_60s./dL; 
Bab_blu_15s = 1e6.*abs_blu_15s./dL;   
Bab_grn_15s = 1e6.*abs_grn_15s./dL;  
Bab_red_15s = 1e6.*abs_red_15s./dL; 

k1=1.317; ko=0.866; 
kf = 1./(k1.*psapr_00.Tr_grn + ko);
kf_60s = 1./(k1.*psapr_00.Tr_grn_60s + ko);
kf_15s = 1./(k1.*psapr_00.Tr_grn_15s + ko);
t_offset = +1./(24*60*2)-1./(24*60*60);
figure; plot((psapr_00.time+t_offset), kf.*Bab_grn_,'g.', ...
   (psapr_00.time+t_offset), kf_15s.*Bab_grn_15s,'b.', ...
   (psapr_00.time+t_offset), kf_60s.*Bab_grn_60s,'m.',...
   (spring.time(ss)), spring.GreenAbs(ss),'k.'); legend('Ab_Grn 1s','new 15s', 'new 60s','spring');
