function run_xap_(xap_name, xap_port);
...
me = menu('Select XAP to collect from:','CLAP10','TAP12','TAP13', 'CLAP92');
if me==1
   xap_name = 'CLAP10';
   xap_port = '10';
elseif me==2
...
sgtitle({['Setting up ',xap_name],['Close this figure to stop saving data and exit ']});
...
sgtitle(['Close this figure to stop saving data and exit ',xap_name]);
pause(.1);
while xap.NumBytesAvailable<461; end;
xap_Trs_ = xap_1line(readline(xap));
while xap_Trs_.spot(end)==0
...
while ~done
   ...
   while toc<update
      ...
      xap_Trs.Tr = Tr;
      sgtitle({['Close this figure to stop saving data and exit ',xap_name];['Update: ',num2str(ceil(update-toc))]});
      pause(0.05)
   end
      if xap_Trs.spot>0
   ...
   if xap_Trs.filter_id~=white.filter_id
    ...
    if isgraphics(h_fig)
      ...
      plot(serial2hs(xap_Trs_.time), [xap_Trs_.Tr],'.-'); legend('Tr (red)','Tr (grn)','Tr (blu)'); ylabel('Tr [unitless]');
      % xl = xlim; xlim([floor(xl(1)), ceil(xl(2))])

      sb(3) = subplot(4,1,3); % for Flow
      plot(serial2hs(xap_Trs_.time), [xap_Trs_.flow_lpm],'x-'); legend('Sample Flow'); ylabel('flow [LPM]');
      % xl = xlim; xlim([floor(xl(1)), ceil(xl(2))]);
      sb(4) = subplot(4,1,4); % for Temp
      plot(serial2hs(xap_Trs_.time), [xap_Trs_.T_sample; xap_Trs_.T_case],'-o'); legend('T Sample','T Case'); ylabel('Temp [C]');
      % xl = xlim; xlim([floor(xl(1)), ceil(xl(2))]);
      xlabel('Time [UT]');
      if max(serial2hs(xap_Trs_.time))>24
...
function display_xap(h_fig, xap_name, xap_Trs_, white);
figure_(h_fig);
an(1) = annotation('TextBox',[0.1256    0.9-.05    0.1162    0.0455],'String',['Filter ID: ',num2str(white.filter_id)]);
an(2) = annotation('TextBox',[0.25    0.9-.05    0.1353    0.0455],'String',['Current Spot: ',num2str(xap_Trs_.spot(end))]);%xap_Trs_.spot(end)
an(3) = annotation('TextBox',[0.395    0.9-.05    0.15    0.0455],'String',['Flow [LPM]: ',num2str(xap_Trs_.flow_lpm(end))]);%xap_Trs_.flow_lpm(end)
an(4) = annotation('TextBox',[0.5534    0.9-.05   0.1812    0.0455],'String',['Sample Temp: ',num2str(xap_Trs_.T_sample(end))]);%xap_Trs_.T_sample(end)
an(5) = annotation('TextBox',[0.7433    0.9-.05   0.17    0.0455],'String',['Case Temp: ',num2str(xap_Trs_.T_case(end))]); %xap_Trs_.T_case(end)
if xap_Trs_.spot(end)>0
   an(6) = annotation('TextBox',[0.1256    0.8241-.05    0.1264    0.0455],'String',['Tr (red): ',...
      num2str(xap_Trs_.Tr_raw(1)./white.Tr_red_init(xap_Trs_.spot(end)))],'Color','r');
   an(7) = annotation('TextBox',[0.26    0.8241-.05    0.1264    0.0455],'String',['Tr (grn): ',...
      num2str(xap_Trs_.Tr_raw(2)./white.Tr_grn_init(xap_Trs_.spot(end)))],'Color',[0,.65,0]);
   an(8) = annotation('TextBox',[0.4    0.8241-.05   0.1264    0.0455],'String',['Tr (blu): ',...
      num2str(xap_Trs_.Tr_raw(3)./white.Tr_blu_init(xap_Trs_.spot(end)))],'Color','b');
end
if isfield(white,'loaded')&&  white.loaded
   an(9) =annotation('TextBox',[0.5983    0.7436    0.2195    0.0775],'String',['Filter is FULLY LOADED. Change filter ASAP!'],...
      'Color',[.8,0,0], 'FontWeight','bold','EdgeColor' , 'r');
...
% sb(2) = subplot(4,1,2); % For Tr
% if isfield(xap_Trs_,'Tr')
%    % plot(serial2hs(xap_Trs_.time), [xap_Trs_.Tr],'-'); legend('Tr (red)','Tr (grn)','Tr (blu)'); ylabel('Tr [unitless]')
% end
% sb(3) = subplot(4,1,3); % for Flow
% plot(serial2hs(xap_Trs_.time), [xap_Trs_.flow_lpm],'-'); legend('Sample Flow'); ylabel('flow [LPM]')
% sb(4) = subplot(4,1,4); % for Temp
% plot(serial2hs(xap_Trs_.time), [xap_Trs_.T_sample, xap_Trs_.T_case],'-'); legend('T Sample','T Case'); ylabel('Temp [C]')
% xlabel('Time [UT]');
% if max(serial2hs(xap_Trs_.time))>24
% xticklabels(num2str(rem(xticks,24)));
% end
sgtitle({['Select this figure and then press any key to modify ',xap_name];'Close this figure to stop saving data and exit program'});
...
[out, ind] = sort([xap.secs, xap_.secs]);
