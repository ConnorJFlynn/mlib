function [clap, raw] = rd_tap_bmi_raw_nolf(infile);
% Read malformed Brechtel RAW TAP file without line feeds as provided by
% Jake Lindberg during BNL PSAP filter tests.
% Modified to handle addition of commas in between date and time
if ~isavar('infile')
   infile = getfullname('RAW_TAP_*.dat','tap_tty','Select tap puTTY log file.');
end

if iscell(infile)&&length(infile)>1
   [clap, raw] = rd_tap_bmi_raw_nolf(infile{1});
   raw.time = clap.time; clap.fname = raw.fname; 
   [clap2, raw2] = rd_tap_bmi_raw_nolf(infile(2:end));
...
%    clap_.fname = unique([clap.fname,clap2.fname]);
   ...
   clap = cat_timeseries(clap, clap2); clap.fname = raw.fname;
else
   ...
   while ~done
      this = fgetl(fid);
      ii = ftell(fid);
      test_str = fread(fid,475,'char=>char')'
      commas = length(strfind(test_str,',')); fseek(fid,ii,-1);
   ...
   try 
      start_time = datenum(test_str,'yymmdd HH:MM:SS');
   catch
      start_time = datenum(test_str,'yymmdd,HH:MM:SS');
   end
   ...
   while ~feof(fid)
      bad = ~(commas==48 && strcmp(test_str(10),':') && strcmp(test_str(13),':'));
      while bad && ~feof(fid)
         fseek(fid,ii+1,-1); ii = ftell(fid);
         test_str = fread(fid,475,'char=>char')';
         commas = length(strfind(test_str,',')); colons = length(strfind(test_str,':'));
         bad = ~((commas==48||commas==50) && colons==2 && strcmp(test_str(10),':') && strcmp(test_str(13),':'));
      end
      if ~bad
         A = textscan(strrep(test_str,',',''),fmt_str );
      ...
      if ~feof(fid)
         ii = ftell(fid); test_str = fread(fid,475,'char=>char')';
         commas = length(strfind(test_str,',')); colons = length(strfind(test_str,':'));
      end
   end
   fclose(fid);
