...
%         menu('Press OK to continue','OK')
    ...
    Tr = [prede.filter_1.*prede.soldst.^2./Tr_ray(1,:)./Langley.Vo(1);prede.filter_2.*prede.soldst.^2./Tr_ray(2,:)./Langley.Vo(2);...
        ...
        prede.filter_7.*prede.soldst.^2./Tr_ray(7,:)./Langley.Vo(7)];
    figure; these = plot(prede.airmass, (Tr([2,3,4,5,7],:)),'.'); recolor(these,prede.wl([2:5,7])); logy;
    ...
    figure; these = plot(prede.time, -log(Tr([2,3,4,5,7],:))./(ones(5,1)*prede.airmass),'.'); recolor(these,prede.wl([2:5,7])); 
    leg_str = {sprintf('%3d nm',prede.wl(2))}; leg_str(end+1) = {sprintf('%3d nm',prede.wl(3))};
    leg_str(end+1) = {sprintf('%3d nm',prede.wl(4))};leg_str(end+1) = {sprintf('%3d nm',prede.wl(5))};
    leg_str(end+1) = {sprintf('%3d nm',prede.wl(7))};
    legend(leg_str);
...
%%
figure; plot(serial2doy(Langleys.time(good_days)), Langleys.Vo(:,good_days)./(mean(Langleys.Vo(:,good_days),2)*ones([1,sum(good_days)])),'o-');
...
figure(105); plot(Langleys.wl, [Langleys.Vo,Langleys.Vo_]' - ones([2*sum(good_days),1])*mean([Langleys.Vo,Langleys.Vo_]'),'-s')
