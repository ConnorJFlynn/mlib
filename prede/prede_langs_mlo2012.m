...
for f = 1:length(files)
 prede = read_prede([in_dir,files(f).name]);
...
%%
 ...
 Tr_ray = exp(-tau_ray.*(ones(size(prede.wl))*prede.airmass));
...
[Vo,tau,Vo_, tau_, good(good)] = dbl_lang(prede.airmass(good),(prede.soldst(good).^2).*prede.filter_5(good),2.0,1,2);
pause(1);
[Vo,tau,Vo_, tau_, good(good)] = dbl_lang(prede.airmass(good),(prede.soldst(good).^2).*prede.filter_3(good),3.0,1,2);
pause(1);
[Vo,tau,Vo_, tau_, good(good)] = dbl_lang(prede.airmass(good),(prede.soldst(good).^2).*prede.filter_2(good),3.0,1,2);
pause(1);
[Vo,tau,Vo_, tau_, good(good)] = dbl_lang(prede.airmass(good),(prede.soldst(good).^2).*prede.filter_7(good),3.0,1,2);
...
%    disp(['wavelength = ',num2str(Lambda(L))])
   [Vo(f), tau(f), P,S,Mu,dVo(f)] = lang(prede.airmass(good),(prede.(['filter_',num2str(f)])(good).*prede.soldst(good).^2));
   [Vo_(f),tau_(f), P_] = lang_uw(prede.airmass(good),(prede.(['filter_',num2str(f)])(good).*prede.soldst(good).^2));
   disp(['Done with ',num2str(prede.header.wl(f))])
   figure(1003);
   subplot(2,1,1);
   semilogy(prede.airmass(good), (prede.(['filter_',num2str(f)])(good).*prede.soldst(good).^2),'g.',...
   ...
   subplot(2,1,2);
   semilogy(1./prede.airmass(good), exp(real(log((prede.(['filter_',num2str(f)])(good)).*prede.soldst(good).^2))...
...
save([in_dir,'Vo',filesep, fname,'.Vo.mat'],'Langley');
...
save([in_dir,'Vo',filesep, fname,'.all_Vos.mat'],'Langleys');
