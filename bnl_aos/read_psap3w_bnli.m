function psapr = read_psap3w_bnli(ins);

if ~exist('ins','var') || ~exist(ins,'file')
    ins = getfullname_('*psap3wI*.tsv','bnl_psap');
end
...
D = C{3};
for x = length(D):-1:1
    dd(x) = rem(sscanf(D{x},'%ld'),100);
end
ii_blu = find(rem(dd,4)==1);
ii_grn = find(rem(dd,4)==2);
ii_red = find(rem(dd,4)==3);
ii_flow = find(rem(dd,4)==0);
...
psapr.flow_AD = hex2nm(C{20});
psapr.flow_lpm = C{21};
psapr.flags = C{22};
%%
psapr.red_rel = psapr.red_sig./psapr.red_ref;
psapr.grn_rel = psapr.grn_sig./psapr.grn_ref;
psapr.blu_rel = psapr.blu_sig./psapr.blu_ref;
