...
tic; disp('Reading PSAP SN 046 files...')
...
ins_ = fix_file_timestamp(ins_);
tic; disp('Reading PSAP SN 077 files...')
psap = read_psap_ir(ins_);
toc
flow_AD = psap.flow_AD;
...
ins_ = fix_file_timestamp(ins_);
tic; disp('Reading PSAP SN 090 files...')
psap = read_psap_ir(ins_);
toc
flow_AD = psap.flow_AD;
spot_diam = 4.724; spot_area = pi.*(spot_diam./2).^2;
...
ins_ = fix_file_timestamp(ins_);
tic; disp('Reading PSAP SN 092 files...')
psap = read_psap_ir(ins_);
toc
flow_AD = psap.flow_AD;
spot_diam = 4.724; spot_area = pi.*(spot_diam./2).^2;
...
ins_ = fix_file_timestamp(ins_);
tic; disp('Reading PSAP SN 107 files...')
psap = read_psap_ir(ins_);
toc
flow_AD = psap.flow_AD;
...
ins_ = fix_file_timestamp(ins_);
tic; disp('Reading PSAP SN 110 files...')
psap = read_psap_ir(ins_);
toc
flow_AD = psap.flow_AD;
...
for sn = 1:length(SN)
   ...
   Bx(~caps_G_good) = ang_coef(caps.Ext_B_stp(~caps_G_good), caps.EAE_BR_sm(~caps_G_good),...
      caps.wavelength_B, psap.wavelength_B); 
   ...
   Bx(~caps_G_good) = ang_coef(caps.Ext_B_stp(~caps_G_good), caps.EAE_BR_sm(~caps_G_good),...
      caps.wavelength_B, psap.wavelength_G);    
   ...
   eval([NNN,' = psap;']);
   save([psap.pname{1},filesep,psap.fname{1},'.mat'],'-struct','psap');
   file_out = ['filter_test.',NNN,'.from_',datestr(psap.time(1),'yyyymmdd_HHMM')...
      ,'-',datestr(psap.time(end),'yyyymmdd_HHMM'),'.v',num2str(ver),'.csv'];
  tic
  save([psap.pname{1},filesep,psap.fname{1},'.mat'],'-struct','psap')
  eval(['psap_',NNN,' = psap;']);
  disp(['Writing ',NNN])
  write_out_csv(psap, file_out);
  toc
end
...
%TAP 14
tap_ = rd_tap_bmi_raw(getfullname('RAW_TAP_SN14*.dat','tap_14','Select tap puTTY log file.'));
tap = proc_tap_me(tap_);
tap.wavelength_B = 375;
tap.wavelength_G = 520.6;
tap.wavelength_R = 629.7;
tap_14 = tap_; tap14 = tap;
