function [PileA, PileB, v] = zoom_sift_nolog(range, InMat, YY);
%Doesn't work yet..., not even close...
...
% 2004-12-02 New version of sift to permit repetive zoom as the selection method.
...
while ~done,
    ...
    keep = input('S to switch frames, <CR> to move cropped profiles, or eXit? {S,<CR>,X}', 's');
    if isempty(keep)
        keep = ' ';
    end;
    %!!
    ...
    select_range = find(range>=v(1)&range<=(v(2)));
    select_pile = PileA*(1+frame)/2 + PileB*(1-frame)/2;
    pile_cross_max = any(InMat(select_range,select_pile)>v(4)&InMat(select_range,select_pile)<=v(4) );
    pile_neg = pile_undermax -1;
    pile_under = pile_undermax+pile_neg;

    pile_cross_min = any(InMat(select_range,:)<v(3)&InMat(select_range,:)>=v(3));
    pile_neg = pile_overmin -1;
    pile_over = pile_overmin + pile_neg;
    ...
    else % move cropped profiles to other group
       pile = (pile_under>0 & pile_over>0) - (pile_under<0 | pile_over<0);
