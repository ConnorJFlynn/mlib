function [status] = C1b_corrs(fid_datan, fid_C1)
...
detA = detA(range_img,:) .* (full_corr(range_img)*ones(1,times));
...
detB = detB(range_img,:) .* (full_corr(range_img)*ones(1,times));
% Finally, resize results into range_img of 15km
...
[status] = nc_putvar(fid_C1, 'depol_532nm_SNR', det_SNR(range_img,:));
 ...
 depol_strength = depol_ratio .* det_SNR;
