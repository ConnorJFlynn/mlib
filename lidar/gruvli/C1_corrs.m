function [status] = C1_corrs(fid_datan, fid_C1)
...
load grli_20020703_cals; 
range_near = find((range>0)&(range<4000));
detA_dt = 2.04e-8;
detB_dt = 2.14e-8;
...
% for troubleshooting, undo deadtime correction.  
% image the ratio of the raw and corrected profiles.
...
%    colorbar('vert');
%   	figure; imagesc(detA(range_near,:));
%    axis('xy')
% 	title('Near DetA');
%    colorbar('vert');
...
% Subtract a pre-saved afterpulse profile fitted/smoothed through TableCurve
detB = detB - detB_ap_fit_Hz * ones(1,times);
% Determine background and subtract it.
...
detA_to_detB = 0.04; % determined by eye judging from depolarization of cirrus and blue sky.
