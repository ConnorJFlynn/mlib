...
%2002-12-12 Modified such that if no args are supplied then file_path is called interactively

%disp(['Opening netcdf file ' cdf_file]);

if nargin == 0, 
    [fname pname] = file_path('*.nc; *.cdf');
    if (~isempty(fname));
        data1_to_data29([pname fname]);
    end;
else
% disp(['cdf_file is: ', cdf_file])
[cdfid, status] = ncmex('OPEN', cdf_file, 'NC_WRITE');
if (status < 0) ; disp(['error opening ' cdf_file ]); end;
...
% detector_counts(find(detector_counts>16.5e6))=0.1*(detector_counts(find(detector_counts>16.5e6)));
...
in_range = find((range > 0) & (range<31000));
r2 = ones(size(range));
r2(in_range) = (range(in_range) .* range(in_range));
...
for i = 1:col
   ...
   signal_log(:,i) = signal_log(:,i) - mean(signal_log(bkgnd_range,i));
end;
...
status = ncmex('VARDEF', cdfid, 'copol_532nm', 'NC_FLOAT', ndims, dims);
    ...
    status = ncmex('ATTPUT', cdfid, 'copol_532nm', 'comment_on_range_correction', 'NC_CHAR', length(value), value);
...
status = ncmex('VARDEF', cdfid, 'depol_532nm', 'NC_FLOAT', ndims, dims);
    ...
    value = 'range-corrected only for ranges from 0-30km';
    status = ncmex('ATTPUT', cdfid, 'depol_532nm', 'comment_on_range_correction', 'NC_CHAR', length(value), value);
...
clear signal signal_log signal_SNR sample_std noise bkgnd bkgnd_A bkgnd_A_by_bkgnd_B;
    ...
    status = nc_putvar(cdfid, 'data_level', 29);
    
    %Get history global attribute, add update, write to file. 
    att_name = 'history';
    [value,status] = ncmex('ATTGET', cdfid, 'NC_GLOBAL', att_name);
    new_value = blanks(80);
    in_str = [datestr(now,31) ' :  Promoted to 29; zero ranged, NO-DTC, bkgnd, r2, log',10];
