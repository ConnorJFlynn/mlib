function [hsrl, byht] = proc_gvhsrl;


% tic; hsrl = bundlen_gvhsrl([],20); toc
...
for Tt = length(hsrl.vdata.time):-1:1
   ...
   lg_bg(Tt) = mean(hsrl.vdata.Low_Gain_Total_Backscatter_Channel(end-100:end,Tt));
...
hsrl.vdata.Low_Gain_Total_Backscatter_Channel = hsrl.vdata.Low_Gain_Total_Backscatter_Channel-lg_bg;
...
hsrl.vdata.Low_Gain_Total_Backscatter_Channel = hsrl.vdata.Low_Gain_Total_Backscatter_Channel .* (hsrl.vdata.range.^2);
...
figure; imagesc([1:length(hsrl.time)], hsrl.vdata.range, real(log10(hsrl.vdata.Merged_Combined_Channel))); axis('xy'); 
raycor = hsrl_raycor(hsrl.vdata.range);
% For Jan 22
% v = axis; 
% figure; plot(hsrl.vdata.range, [1e-8.*hsrl.vdata.Molecular_Backscatter_Channel(:,100).* raycor, 6.5e5*hsrl.vdata.Molecular_Backscatter_Coefficient(:,100)],'-'); logy
% axis(v)
raycor = hsrl_raycor(hsrl.vdata.range);
...
hsrl.vdata.Low_Gain_Total_Backscatter_Channel = hsrl.vdata.Low_Gain_Total_Backscatter_Channel .* raycor;
...
%    [hsrl.vdata.Molecular_Backscatter_Channel(:,100),hsrl.vdata.High_Gain_Total_Backscatter_Channel(:,100),...
%    hsrl.vdata.Low_Gain_Total_Backscatter_Channel(:,100),hsrl.vdata.Merged_Combined_Channel(:,100), ...
%    hsrl.vdata.Cross_Polarization_Channel(:,100) ],'-'); logy
% legend('mole','low gain','high gain','merged','cross')
% % for Jan 29
% figure; plot(hsrl.vdata.range, ...
%    [hsrl.vdata.Molecular_Backscatter_Channel(:,1500),hsrl.vdata.High_Gain_Total_Backscatter_Channel(:,1500),...
%    hsrl.vdata.Low_Gain_Total_Backscatter_Channel(:,1500),hsrl.vdata.Merged_Combined_Channel(:,1500), ...
%    hsrl.vdata.Cross_Polarization_Channel(:,1500) ],'-'); logy
% legend('mole','low gain','high gain','merged','cross')
...
h_ = by_ht.height>200 & by_ht.height<5000;
figure; imagesc([1:length(hsrl.time)], by_ht.height(h_), real(log10(by_ht.Merged_Combined_Channel(h_,:)))); axis('xy'); 
figure; imagesc([1:length(hsrl.time)], by_ht.height(h_), real((by_ht.Optical_Depth(h_,:)))); axis('xy'); 
