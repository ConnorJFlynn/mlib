...
for ii = length(Flist):-1:1 % number 450 is an interesting case.
    %% load raw data.
...
%         pcolor(mpl_dat.time_datenum, mpl_dat.rng, real(log10(mpl_dat.cop)));  shading flat; 
        imagegap(serial2Hh(mpl_dat.time_datenum), mpl_dat.rng, real(log10(mpl_dat.cop - ones(size(mpl_dat.rng))*mpl_dat.cop_back')));
%         dynamicDateTicks;
%         h = colorbar;
        ...
        ax(2) = subplot(2,1,2); 
        plot(serial2Hh(mpl_dat.time_datenum), mpl_dat.elev,'-o'); legend('elev');
        set(gca, 'fontsize', 14, 'fontweight', 'bold')
        xlabel('time [UT]'); ylabel('El [deg]');  
        linkaxes(ax,'x');
        if Save_plot
