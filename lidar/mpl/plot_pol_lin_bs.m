function plot_pol_lin_bs(polavg,inarg)
% plot_pol_lin_bs(polavg,inarg)
...
% colormap('hijet');
colormap(comp_map3);
titlestr = ['log_1_0(linear depolarization ratio) for ',datestr(polavg.time(1), 'yyyy-mm-dd '),hh_HH_str1];
title(ax(2),titlestr);
...
% function
caxis(ax(1),cv_lin_bs);
cv_lin_bs = caxis(ax(1));
cv_dpr = caxis(ax(2));
cmap = colormap;
