...
cal_path = 'C:\Users\ixs34\OneDrive - The Pennsylvania State University\Desktop\data\';

% load([cal_path, 'MPL_cal.mat']) % load MPL calibration file
% CJF: renamed to 'miniMPL_cal.mat' in mlib/lidar/mpl directory

load([cal_path, 'miniMPL_cal.mat']) % load MPL calibration file
...
for ii = 1: length(Flist) % number 450 is an interesting case.
    ...
    mpl_dat.time_datenum = datenum(1970,1,1,0,0, double(ncread([Path Flist(ii).name], 'time')));
    mpl_dat.time = ncread([Path Flist(ii).name], 'time');
    mpl_dat.az = ncread([Path Flist(ii).name], 'azimuth_angle');
    mpl_dat.elev = ncread([Path Flist(ii).name], 'elevation_angle');
    mpl_dat.lat = ncread([Path Flist(ii).name], 'gps_latitude');
    mpl_dat.lon = ncread([Path Flist(ii).name], 'gps_longitude');
    mpl_dat.alt = ncread([Path Flist(ii).name], 'gps_altitude');
    mpl_dat.ad_flg = ncread([Path Flist(ii).name], 'ad_data_bad_flag'); % 0: A/D data good, 1: A/D data probably out of sync. Energy monitor collection is not exactly aligned with MCS shots
    mpl_dat.cop = double(ncread([Path Flist(ii).name], 'channel_2')); % co pol is stored in channel 2 (see metadata)
    mpl_dat.crs = double(ncread([Path Flist(ii).name], 'channel_1')); % cross pol is stored in channel 1 (see metadata)
    mpl_dat.cop_back = double(ncread([Path Flist(ii).name], 'background_average_2')); %
    mpl_dat.crs_back = double(ncread([Path Flist(ii).name], 'background_average')); %
    mpl_dat.cop_back_sd = double(ncread([Path Flist(ii).name], 'background_stddev_2')); %
    mpl_dat.crs_back_sd = double(ncread([Path Flist(ii).name], 'background_stddev')); %
    mpl_dat.shots = ncread([Path Flist(ii).name], 'shots_sum'); %
    mpl_dat.energy_monitor = double(ncread([Path Flist(ii).name], 'energy_monitor'))./ 1e3; % converting to micro Joule
    mpl_dat.bin_time = ncread([Path Flist(ii).name], 'bin_time'); %
    mpl_dat.trigger_frequency = ncread([Path Flist(ii).name], 'trigger_frequency'); %
    mpl_dat.measure_t_tot = mpl_dat.bin_time.* 1e6 .* mpl_dat.trigger_frequency.* [diff(mpl_dat.time(1:2)); diff(mpl_dat.time)]; % total receiver measurement time in one averaged bin.
...
%     mpl_dat_cor.crs = mpl_dat.crs .* polyval(MPL_cal.dtc_poly, mpl_dat.crs); % vendor's DTC 
    ...
    mpl_dat_cor.ldr(ldr_rel_ind) = abs(mpl_dat_cor.crs(ldr_rel_ind))./(abs(mpl_dat_cor.cop(ldr_rel_ind)) + abs(mpl_dat_cor.crs(ldr_rel_ind)));
    mpl_dat_cor.ldr_snr = real(1./ sqrt( 1./mpl_dat_cor.crs_snr.^2 + 1./((mpl_dat_cor.cop + mpl_dat_cor.crs)./ sqrt(cop_noise.^2 + crs_noise.^2).^2))); % Flynn's formulation (not published yet)
    mpl_dat_cor.ldr_snr(mpl_dat_cor.ldr_snr <= 0) = nan;         mpl_dat_cor.ldr_snr = 10.*log10(mpl_dat_cor.ldr_snr);
    mpl_dat_cor.cop = mpl_dat_cor.cop .* (repmat(1./MPL_cal_2_use.ol(:,2) .* mpl_dat_cor.rng.^2, 1, size(mpl_dat_cor.cop, 2)) ./ repmat(mpl_dat.energy_monitor', size(mpl_dat_cor.cop, 1), 1)); % overlap, range, and energy corrections.
    mpl_dat_cor.crs = mpl_dat_cor.crs .* (repmat(1./MPL_cal_2_use.ol(:,2) .* mpl_dat_cor.rng.^2, 1, size(mpl_dat_cor.crs, 2)) ./ repmat(mpl_dat.energy_monitor', size(mpl_dat_cor.crs, 1), 1)); % overlap, range, and energy corrections.
    mpl_dat_cor.nrb = mpl_dat_cor.nrb .* (repmat(1./MPL_cal_2_use.ol(:,2) .* mpl_dat_cor.rng.^2, 1, size(mpl_dat_cor.nrb, 2)) ./ repmat(mpl_dat.energy_monitor', size(mpl_dat_cor.nrb, 1), 1)); % overlap, range, and energy corrections.
    ...
    if Plot_process
        ...
        pcolor(mpl_dat.time_datenum, mpl_dat.rng, real(log10(mpl_dat.cop)));  shading flat; h = colorbar;
        ...
        title('copol raw counts'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[counts /microsec]')
        if Save_plot
        ...
        ylim(YLim);  caxis([-3 2]);  colormap jet
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('crosspol raw counts'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[counts /microsec]')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
        ...
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('copol NRB'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[counts*km^2 /(\musec*\muJ)]')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
        ...
        ylim(YLim);  caxis([-2 2]); colormap jet
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('crosspol NRB'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[counts*km^2 /(\musec*\muJ)]')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
        ...
        ylim(YLim);  caxis([-2 2]); colormap jet
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('NRB'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[counts*km^2 /(\musec*\muJ)]')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
        ...
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('NRB SNR'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[dB]')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
        ...
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('LDR'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, 'LDR')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
        ...
        set(gca, 'fontsize', 14, 'fontweight', 'bold', 'xlim', [datenum([dtmp(1:4), 0 0]) datenum([dtmp(1:4)+[0,0,0,1], 0 0])], 'xtick', linspace(datenum([dtmp(1:4), 0 0]), datenum([dtmp(1:4)+[0,0,0,1], 0 0]), 7))
        title('LDR SNR'); dateaxis('x', 0); xlabel('date'); ylabel('range [km]');  ylabel(h, '[dB]')
        if Save_plot
            set(Fig,'PaperPositionMode','auto')
