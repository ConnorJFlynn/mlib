function [lidar, status, save_array] = keep_mplraw(fid, filename);
% [lidar, status] = keep_mplraw(fid,filename);
% reads an entire raw mpl file and populates the lidar structure
% 
% 20005-05-15 Does not cull pre-pulse bins
...
%    statics.shots_summed = round(mean(hk.shots_summed));
    ...
    r.range_offset = (first_bin - 0.5) * range_bin_time * c/2 * 1e-12; % The "0.5" centers the first bin
...
%     signal(1:first_bin,:) = [];
     ...
     signal = ((range.^2)*ones(1,profs)).* signal;
