function [status,polavg] = batch_b1tomat(in_dir, hour_chunk);
% [status,polavg] = batch_a1tomat(in_dir, hour_chunk);
...
len = 1000; % This is the number of netcdf records to read at a time.
   ...
   if ~done
      ...
      for m = 1:len:length(anc_mplpol_.time)
         ...
         if ~isempty(anc_mplpol)
            [polavg_new,tail_ind] = proc_mplpolb1_(anc_mplpol,mpl_inarg);
