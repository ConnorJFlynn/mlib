function status = make_hourly_ps_zangle(statics);
% status = make_daily_ps_zangle(statics);
% In progress...
% The idea here is to create hourly files with ps profiles averaged to the
...
if nargin==0
    ...
    dir_out = ['C:\case_studies\ipa\Alldays\data\ipasmplzangle.c1\'];
...
while mir_ind < length(mirror.start_time)
    ...
    mplps_hour_mavg.fixed_angles = fixed_angles;
    if ~isempty(mplps_hour)


        if ~isempty(ocr_hour)
            overlap = ocr_hour_mavg.range_prof(ocr_hour_mavg.r.lte_10,:)./mplps_hour_mavg.range_prof(mplps_hour_mavg.r.lte_10,:);

            for f = 1:length(fixed_angles)
                ols = find(round(10*mplps_hour_mavg.hk.zenith_angle)/10 ==fixed_angles(f));

                if length(ols)>1
                    overlap_by_angle(:,f) = mean(overlap(:,ols)')';
                elseif length(ols)==1
                    overlap_by_angle(:,f) = (overlap(:,ols));
                else
                    overlap_by_angle(:,f) = NaN(size(overlap(:,1)));
                end
            end
            %figure; plot(mplps_hour_mavg.range(mplps_hour_mavg.r.lte_10),overlap_by_angle)
            mplps_hour_mavg.overlap = overlap;
            %         mplps_hour_mavg.overlap_by_angle = overlap_by_angle;

        else %ocr_hour is empty so we can't define overlap
            ocr_hour_mavg = fill_ocr_with_nans(mplps_hour_mavg);
            overlap = NaN([length(mplps_hour.r.lte_10),length(this_hour.mir)]);
            mplps_hour_mavg.overlap = overlap;

            %         mplps_hour_mavg.overlap_by_angle = NaN([length(mplps_hour.r.lte_10),length(fixed_angles)]);;
        end
        %The line below should work whether or not mplps_hour was empty in
        %which event the previous value for overlap_by_angle will be retained.
        %
        mplps_hour_mavg.overlap_by_angle = overlap_by_angle;
        ...
        status = output_hour_mavg(mplps_hour_mavg, ocr_hour_mavg, statics);
        disp('.')
    end
end
