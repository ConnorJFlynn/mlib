function status = output_hour_mavg(mplps_hour_mavg, ocr_hour_mavg, statics);

%Determine filename and open it.
dir_out = statics.pname;
fstem = statics.fstem;
[d_str,t_str] = strtok(datestr(mplps_hour_mavg.time(1),30),'T');
hh_str = t_str(2:3);
full_name = [dir_out,fstem,'.',d_str,'.',hh_str,'0000.nc'];
[ncid, rcode] = ncmex('create', full_name, 'clobber');
dims.time = mplps_hour_mavg.time;
dims.range = mplps_hour_mavg.range;
dims.ol_range = mplps_hour_mavg.range;
dims.fixed_angles = mplps_hour_mavg.fixed_angles;
ps.statics = mplps_hour_mavg.statics;
ps.statics.datalevel = statics.datalevel;
ps.statics.proc_level = statics.datalevel;
ps.statics.datastream = statics.datastream;
ps.statics.zeb_platform = statics.datastream;
ps.statics.ocr = statics.ocr;
ps.statics.averaging_int = statics.averaging_int;
ps.statics.pname = statics.pname;

status = define_mavg_rprof_c1(ncid, dims, ps.statics);
status = write_mavg_rprof_c1(ncid, mplps_hour_mavg, ocr_hour_mavg);
ncmex('close', ncid);

end %end output_hour_mavg


function status = define_mavg_rprof_c1(cdfid, dims, statics);
% status = define_mavg_rprof_c1(ncid, dims, statics);;
...
function status = write_mavg_rprof_c1(cdfid, ps, ocr);
% status = output_ipasmpl_a1(a1_ncid, mpl_a1_out);

epoch = serial2epoch(ps.time);
base_time = min(epoch);
time_offset = epoch - base_time;
...
epoch_midnight = serial2epoch(floor(ps.time(1)));
time = time_offset + (base_time - epoch_midnight);
varname = 'time';
[status] = nc_putvar(cdfid, varname, time);

varname = 'doy';
[status] = nc_putvar(cdfid, varname, serial2doy0(ps.time));
% varname = 'range';
...
varname = 'nsamples';
[status] = nc_putvar(cdfid, varname, ps.hk.nsamples);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'scene_duration';
[status] = nc_putvar(cdfid, varname, ps.hk.span);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'shots_summed';
[status] = nc_putvar(cdfid, varname, ps.hk.shots_summed);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'copol_shots_summed';
[status] = nc_putvar(cdfid, varname, ps.hk.cop_shots_summed);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'crosspol_shots_summed';
[status] = nc_putvar(cdfid, varname, ps.hk.crs_shots_summed);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'zenith_angle';
[status] = nc_putvar(cdfid, varname, ps.hk.zenith_angle);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'energy_monitor';
[status] = nc_putvar(cdfid, varname, ps.hk.energy_monitor);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'energy_monitor_std';
[status] = nc_putvar(cdfid, varname, ps.hk.energy_monitor_std);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'detector_temp';
[status] = nc_putvar(cdfid, varname, ps.hk.detector_temp);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'laser_temp';
[status] = nc_putvar(cdfid, varname, ps.hk.laser_temp);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'instrument_temp';
[status] = nc_putvar(cdfid, varname, ps.hk.instrument_temp);
if status<0
    disp(['Problem writing ' varname,'!']);
end
varname = 'copol_nsamples';
