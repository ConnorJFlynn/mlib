function status = define_smplps_c1(cdfid, in_time, range, statics);
% status = define_smplps_c1(cdfid, in_time, range, statics);
% Defines a ipasmplocr.a0 file
...
% status = ncmex('VARDEF', cdfid, varname, datatype, 2, [0 1]);
% att_val = 'noise due to counting statistics';
% status = ncmex('ATTPUT', cdfid, varname, 'long_name', 2, length(att_val), att_val) ;
% att_val = 'MHz';
% status = ncmex('ATTPUT', cdfid, varname, 'units', 2, length(att_val), att_val) ;
% att_val = 'converted raw counts to MHz';
% status = ncmex('ATTPUT', cdfid, varname, 'corrections', 2, length(att_val), att_val) ;
% 
% varname = 'cts_snr';
% status = ncmex('VARDEF', cdfid, varname, datatype, 2, [0 1]);
% att_val = 'signal to noise ratio';
% status = ncmex('ATTPUT', cdfid, varname, 'long_name', 2, length(att_val), att_val) ;
% att_val = 'unitless';
% status = ncmex('ATTPUT', cdfid, varname, 'units', 2, length(att_val), att_val) ;
