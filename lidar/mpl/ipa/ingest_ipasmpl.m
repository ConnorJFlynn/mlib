function [status] = ingest_ipasmpl(statics);
%% [status] = ingest_ipasmpl
% Need to modify the below lines to reflect ocr or ps
if nargin==0
    in_dir = ['C:\case_studies\ipa\Alldays\data\ipasmplocrC1.00\'];
...
%%
[dirlist, in_dir] = get_dir_list(in_dir, '*.??w;*.??W');
for ps = 1:length(dirlist)
    disp(['Processing ',dirlist(ps).name,': ',num2str(ps), ' of ',num2str(length(dirlist))])
    mpl = read_mpl([in_dir, dirlist(ps).name]);
