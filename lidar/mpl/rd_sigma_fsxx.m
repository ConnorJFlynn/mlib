function [lidar, status] = rd_sigma_fs(filename,start_here);
   ...
   if ~isstruct(profile_bins)
      ...
      for ch_i = 1:length(ch)
         ...
         if ~isempty(r.bg)
             ...
             if ch_i == 1
             lidar.hk.bg.(ch{ch_i}) = bkgnd_crosspol;
             lidar.hk.std_bg.(ch{ch_i}) = bkgnd_crosspol_std;
             elseif ch_i ==2
             lidar.hk.bg.(ch{ch_i}) = bkgnd_copol;
             lidar.hk.std_bg.(ch{ch_i}) = bkgnd_copol_std;                 
             end
         end
      end
...
lidar.range = range'-min(mode(range_offset));
