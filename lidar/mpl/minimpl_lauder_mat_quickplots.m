function minimpl_uc_mat_quickplots
%-------------------------------------------------------------------------
% Connor Flynn
% Last update: 2020/01/12
%-------------------------------------------------------------------------
% Adapted from Israel's script for TAN miniMPL cruise
%-------------------------------------------------------------------------
...
cal_path = 'E:\mmpl\Lauder2018\calibration_lauder\';
% if isafile([cal_path,'calibration_lauder.nc'])
%     MPL_cal = anc_load([cal_path,'calibration_lauder.nc']);
% end
MPL_cal = load([cal_path,'miniMPL_cal_cjf.mat']);
STR = 'Lauder'; % southern ocean voyage
Flist = dir([Path '*.mat']);
...
for ii = 1:length(Flist) % number 450 is an interesting case.
    ...
    mpl_dat.rng = [1:size(mpl_tmp.vdata.channel_1,1)]'.*3e-2./(unique(mpl_tmp.vdata.bin_time)./2e-7);
    ...
    ol =  10.^interp1(log10(MPL_cal.ol_interp(:,1)), log10(MPL_cal.ol_interp(:,2)), log10(rng),'linear','extrap');
    ap_cop = 10.^interp1(log10(MPL_cal.cjf.rng),log10(MPL_cal.cjf.ap_cop),log10(rng),'linear','extrap');
    ap_crs = 10.^interp1(log10(MPL_cal.cjf.rng),log10(MPL_cal.cjf.ap_crs),log10(rng),'linear','extrap');
...
%     mpl_dat_cor.crs = mpl_dat.crs .* polyval(MPL_cal.dtc_poly, mpl_dat.crs); % vendor's DTC 
    ...
    em = double(mpl.vdata.energy_monitor)./1000;
    mpl_dat_cor.cop = mpl_dat_cor.cop - ap_cop*em - cop_back; 
    mpl_dat_cor.crs = mpl_dat_cor.crs - ap_crs*em - crs_back; 
    ...
    corr = [(rng.^2 ./ol) * (1./em)];
    mpl_dat_cor.cop = mpl_dat_cor.cop .* corr ; % overlap, range, and energy corrections.
    mpl_dat_cor.crs = mpl_dat_cor.crs .* corr;
    mpl_dat_cor.nrb = mpl_dat_cor.nrb .* corr;
