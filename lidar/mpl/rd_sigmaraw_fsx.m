% Modified Dec 12, 2016 to deal with new MPLpolfs files with extra bytes in
% data packet
clear temp;
bad_file = false;
...
while ~feof(fid)
    ...
    ver_num(t) = fread(fid, 1, 'uint16')./100; % Software version number*100
    ...
    energy_monitor(t) = fread(fid, 1, 'uint32')/1000; % Energy monitor reading in uJ, stored as emon_uJ*100
    ...
    bkgnd_crosspol(t) = fread(fid, 1, 'float32'); % Background signal mean in counts/microsecond
    bkgnd_crosspol_std(t) = fread(fid, 1, 'float32');% Background signal standard deviation in counts/microsecond
    %     if ver_num(t)>0
    ...
    bad_flag(t) = fread(fid,1,'char');
    file_ver(t) = fread(fid,1,'char');
    bkgnd_copol(t) = fread(fid, 1, 'float32'); % Background signal mean in counts/microsecond
    bkgnd_copol_std(t) = fread(fid, 1, 'float32');% Background signal standard deviation in counts/microsecond
    MCS_mode(t) = fread(fid,1,'char');
    %       /* McsMode can ony have a value of 167 or 199. If any other value is seen,
    %
    %%
    %     mark = ftell(fid);
    %     %%
    %     fseek(fid, mark, -1);
    first_bin(t) = fread(fid, 1, 'uint16');
    fread(fid,1,'uint8');
    fread(fid,2,'uint16');
    blip = fread(fid,1,'uint16');
    if blip==0 && ver_num(t)<4.13
        packetsize = 128 + 4*numbins(t)*numchannels(t);
%         dump = fread(fid, 7, 'char');% Future expansion
    else
%         fread(fid,1,'uint8');
%         fread(fid,2,'uint16');
%         blip = fread(fid,1,'uint16');
        packetsize = blip + 4*numbins(t)*numchannels(t);
        dmp = blip-128;
        blop = fread(fid,dmp,'uint8');
     end
%     
%     
%     if ver_num(t)<4.13
...
% before I forget
% I think what I want to do here is use all the bins up to but not
% inlcuding "first_bin" for the background range
% Then for each profile search for the first relative maxima or maybe the
% first background-subtracted value over some threshold value.  Let this bin represent 
% time zero.  Retain data for this bin out to 30 km.  If no zero bin  
% found or if found beyond some max range then assume a default value.
% 

nulls = (numchannels==0)|(numbins==0)|(range_bin_time==0);
good = sum(~nulls);
numchannels(nulls) = [];
numbins(nulls) = []; 
...
Year(nulls) = [];
az_deg(nulls) = [];
bkgnd_crosspol(nulls) = [];
bkgnd_crosspol_std(nulls) = [];
comp_deg(nulls) = [];
detector_temp(nulls) = [];
el_deg(nulls) = [];
