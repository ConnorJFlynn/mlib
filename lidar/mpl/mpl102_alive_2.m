...
elseif exist('mpl','var')&&exist(mpl,'file')
   mpl = ancload(mpl);
end
...
ol_corr = interp1(sub_olcorr.range, sub_olcorr.olcorr, mpl.range(mpl.r.lte_10),'linear','extrap');
mpl.prof(mpl.r.lte_10,:) = mpl.prof(mpl.r.lte_10,:).* (ol_corr*ones(size(mpl.time)));
%  mpl.hk.pulse_rep
% mpl.hk.shots_summed
