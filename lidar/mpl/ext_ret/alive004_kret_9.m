function mpl = alive004_kret_9(mpl);
% This version does not use mplnor at all.
% AWG Nov meeting
% Version 6 attempting to use a constant lidar C in absence of AOD
% disp('Cleaning up...');

% This is seeming overly painful.  What it attempts to do right now is:
% 1. Load screened AODs, finding those encompassed by the MPL time
% 2. If there are good AODs, then prompt for manual selection of MPL clear sky data profiles 
% (to assist in interactive cloud screen) or use a pre-saved selection of clear sky.
% 3. Manually screen clouds using trimsift_nolog
% 
% Seems like there is some crazy mismatch between "clean" profiles and those considered "non-missing" 
% so I get some bogus calibrations
% pause(.2);
close('all');
%ncclose('all'); clear
pause(.2);
...
pause(.1)
mpl_pname = ['C:\case_studies\Alive\data\sgpmplC1.a1\arm_netcdf\'];
%mplnor_pname = ['c:\case_studies\Aerosol_IOP\sgpmplnor1campC1.c1\cdf\'];
mpl_ret_pname = ['C:\case_studies\Alive\data\sgpmplC1.a1\ext\Nov2006\'];

load odC1.mat
odC1 = ancsift(odC1, odC1.dims.time, odC1.aero_2==1);
% aeroC1 = (odC1.vars.variability_flag.data<1e-4);
% odC1 = ancsift(odC1, odC1.dims.time, odC1.aero_2);
...
if max(size(mfr_times))>1
    ...
    if exist('clear_sky004.mat', 'file')
        ...
        if (upper(keep) == ' ') % Use mean of selected region for clear_sky
            ...
            save('clear_sky004.mat', 'clear_sky')
        else
    ...
    % At this point we have a clear sky profile to use in forming a
    % scattering ratio.
    %!!
    frac_fig = figure;
...
%     mpl.cal.lowess_mean_prof(missing) = -9999;
    ...
    for nm = length(non_missing):-1:1
        ...
        nearby_aod = find(abs(mpl.time(non_missing(nm))-odC1.time)< .125);
    ...
    if sum(good_aod)>2
        mpl.cal.C(non_missing(good_aod)) = mpl.cal.mean_prof(non_missing(good_aod))...
    ...
    if sum(clean_mpl_no_aod)>2
        mpl.klett.Sa(clean_mpl_no_aod) = interp1(mpl.time(clean_mpl), mpl.klett.Sa(clean_mpl),mpl.time(clean_mpl_no_aod));
    ...
    colorbar;
    title(['MPL004 extinction: ',datestr(mpl.time(1),'yyyy/mm/dd')]);
    xlabel('time (day of year 2005)');
    ylabel('height (km AGL)')
    print(ext_fig, [mpl_pname, '..\plots\mpl004_extprof.',datestr(mpl.time(1),'yyyymmdd'),'.png'],'-dpng');
    ...
    print(lidarC_fig, [mpl_pname, '..\plots\mpl004_aod_Sa_C.',datestr(mpl.time(1),'yyyymmdd'),'.png'],'-dpng');
    pause(.1)
    plots_default;
