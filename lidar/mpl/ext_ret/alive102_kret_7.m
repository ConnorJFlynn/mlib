function mpl = alive102_kret_7(mpl);
...
if max(size(mfr_times))>1
    ...
    clear avg
    missing = ((mpl.hk.bg<=-9998)&(mpl.hk.bg>=-10000));
    non_missing = ~missing;
    ...
    mpl.cal.atten_ray = 10.^(mean(real(log10(mpl.sonde.atten_ray(mpl.r.cal)))));
    %    mpl.cal.nor_sonde = mpl.sonde.atten_prof(mpl.r.cal)/mpl.cal.atten_ray;
    mpl.cal.mean_prof(non_missing&mpl.clean) = 10.^(mean(real(log10(mpl.prof(mpl.r.cal,(non_missing&mpl.clean))))));
    ...
    %     end
    mpl.cal.mean_prof(missing|~mpl.clean) = -9999;
    mpl.cal.lowess_mean_prof(non_missing&mpl.clean) = lowess(serial2doy(mpl.time(non_missing&mpl.clean))-floor(serial2doy(mpl.time((mpl.clean(1))))), mpl.cal.mean_prof(non_missing&mpl.clean), .02)';
    mpl.cal.lowess_mean_prof(missing|~mpl.clean) = -9999;
    mpl.cal.aod_523(missing|~mpl.clean) = NaN;

    for nm = find(non_missing)'
...
%             mpl.cal.aod_523(nm) = interp1(odC1.time(nearby_aod),odC1.aod523(nearby_aod), mpl.time((nm-3):(nm+3)),'pchip','extrap');
            mpl.cal.aod_523(nm) = local_val(odC1.time(nearby_aod)-mpl.time(nm),odC1.aod523(nearby_aod),0,.5);
        else
            mpl.cal.aod_523(nm) = NaN;
        end
    end
