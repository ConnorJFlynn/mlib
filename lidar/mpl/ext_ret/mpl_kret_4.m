...
pause(.2);
mpl_pname = ['c:\case_studies\Aerosol_IOP\sgpmplC1.a1\cdf\'];
...
sonde_pname = ['c:\case_studies\Aerosol_IOP\sgplssondeC1\cdf\'];
mpl_pname = ['c:\case_studies\Aerosol_IOP\sgpmplC1.a1\cdf\'];
mplnor_pname = ['c:\case_studies\Aerosol_IOP\sgpmplnor1campC1.c1\cdf\'];
mpl_ret_pname = ['c:\case_studies\Aerosol_IOP\sgpmplret1flynn.c1\cdf\'];
sonde_pname = ['c:\case_studies\Aerosol_IOP\sgplssondeC1\cdf\'];
...
nimfr_pname = ['c:\case_studies\Aerosol_IOP\xmfrx_od\joe\'];
nimfr_pname = ['C:\case_studies\Aerosol_IOP\xmfrx_od\joe\'];
...
if max(size(nimfr_times))>1
   ...
   %Nimfr data...
   nearest(non_missing) = interp1(serial2doy0(nimfr.time(nimfr_times))-floor(serial2doy0(nimfr.time(nimfr_times(1)))), nimfr.aod523(nimfr_times), serial2doy0(mpl.time(non_missing))-floor(serial2doy0(nimfr.time(nimfr_times(1)))), 'nearest', 'extrap');
   lin_interp(non_missing) = interp1(serial2doy0(nimfr.time(nimfr_times))-floor(serial2doy0(nimfr.time(nimfr_times(1)))), nimfr.aod523(nimfr_times), serial2doy0(mpl.time(non_missing))-floor(serial2doy0(nimfr.time(nimfr_times(1)))), 'linear', 'extrap');
   nearest(missing) = -9e-9;
   lin_interp(missing) = -9e-9;
   nimfr_fig = figure;
   figure(nimfr_fig);
   doy = floor(serial2doy0(mean(mpl.time)));
   plot(serial2doy0(nimfr.time), nimfr.aod523, 'g.', serial2doy0(mpl.time(non_missing)), nearest(non_missing), 'ro',serial2doy0(mpl.time(non_missing)), lin_interp(non_missing), 'bx'  ); zoom;
   title(['NIMFR aot 523 nm for period covering MPL data on ', datestr(mean(mpl.time),1),' (DOY=', num2str(doy), ').']);
   v = axis;
   axis([(doy -.5), (doy + 1.5), v(3), v(4) ]);
   v = axis;
   legend('nimfr data', 'nearest neighbor', 'linear interpolation');
   ...
   plot(serial2doy0(nimfr.time), nimfr.aod523, 'g.', serial2doy0(mpl.time(non_missing)), mpl.mfr.aod_523(non_missing),'r' ); zoom;
   title(['NIMFR aot 523 nm for period covering MPL data on ', datestr(mean(mpl.time),1),' (DOY=', num2str(doy), ').']);
   axis(v)
...
figure(prof_fig);
   ...
   for clear_bin = [clean_mpl]
      ...
      axis([ 1e-2, 10, min(range), max(range)])
