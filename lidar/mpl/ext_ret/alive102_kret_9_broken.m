function mpl = alive102_kret_9(mpl);
...
% Added pre-screen and envelope capture
aeroC1 = logical(odC1.aero_2);
...
mfr_ind = find((odC1.time>=(min(mpl.time)-1))&(odC1.time<=(max(mpl.time)+1)));
...
% 
%         save('clear_sky2.mat', 'clear_sky_prof')
%     end
%     %!!
    ...
    [avg] = mpl_timeavg3(mpl,5,clean_mpl);
...
%         xlim([.8, 10]);
        ...
        line([1,1],[ylim],'Color','r','LineStyle','--','LineWidth',2);
        xlabel('backscatter')
        legend('lidar profile','molecular','scattering ratio')
...
%         subplot(1,4,1);
    ...
    clean_mpl_no_aod_ind = find(clean_mpl_no_aod>0);
    if ((sum(clean_mpl_no_aod)>2)&&(length(clean_mpl_ind)>=2))
