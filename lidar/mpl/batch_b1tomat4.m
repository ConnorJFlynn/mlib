function [status,polavg] = batch_b1tomat4(in_dir, hour_chunk);
% [status,polavg] = batch_b1tomat3(in_dir, hour_chunk);
...
len = 2500; % This is the number of netcdf records to read at a time.

mpl_inarg.fstem = 'isdac_mplpol_3flynn.';
mpl_inarg.fstem = 'sgp_mplpol_3flynn.';
mpl_inarg.Nsecs = 150;
% Each of the following function stubs should point to the site
% corrections, which internally identify specific corrections according to
% time
mpl_inarg.dtc = @dtc_sgp_; %accept and return MHz
mpl_inarg.ap = @ap_sgp_; %accept range, return .cop, .crs
mpl_inarg.ol_corr = @ol_sgp_; % accept range, return ol_corr
mpl_inarg.cop_snr = 2;
mpl_inarg.ldr_snr = 1.5;
mpl_inarg.ldr_error_limit = .25;
mpl_inarg.fig = gcf;
mpl_inarg.vis = 'on';
mpl_inarg.cv_lin_bs = [1.5,4.5];
mpl_inarg.cv_log_bs = [0,1500];
