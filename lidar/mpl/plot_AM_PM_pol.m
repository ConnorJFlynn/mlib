...
[rgb,color_sqr] = rgb_weight(x,y,z2,z1,cmap,cv_dpr,cv_bs,0.2,mask(r.lte_15,:));
% ax(3) = subplot(3,1,3); 
figure(fig+1); 
set(fig+1,'visible',vis);
set(gcf,'position',[ 26 476 1043  319]);
ax2(1) = subplot('position',[0.0471    0.1100    0.6080    0.8150]);
ax2(2) = subplot('position',[0.6864    0.1151    0.2500    0.8150]);
...
set(fig,'visible','on');
   fname1 = [fstem,'bs_ldr.',datestr(polavg.time(1), 'yyyy-mm-dd'),am_pm_str2,'.'];
   saveas(fig, [figdir,filesep,fname1,'fig']);
   set(fig,'visible',vis);

axes(ax2(1));
set(fig+1,'visible','on');
   fname2 = [fstem,'comp.',datestr(polavg.time(1), 'yyyy-mm-dd'),am_pm_str2,'.'];
...
% figure(1);set(gcf,'visible',vis)
   ylim([0,rr]);
   fname1 = [fstem,'bs_ldr.',datestr(polavg.time(1), 'yyyy-mm-dd'),am_pm_str2,'.',num2str(rr),'km.'];
   fname2 = [fstem,'comp.',datestr(polavg.time(1), 'yyyy-mm-dd'),am_pm_str2,'.',num2str(rr),'km.'];
   if ~isempty(pngdir)
