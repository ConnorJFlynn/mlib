function load_mpl_and_process_clean_cjf
%-------------------------------------------------------------------------
% Israel Silber
...
for ii = 2:length(Flist) % number 450 is an interesting case.
    %% load raw data.
...
%         pcolor(mpl_dat.time_datenum, mpl_dat.rng, real(log10(mpl_dat.cop)));  shading flat; 
        imagegap(mpl_dat.time_datenum, mpl_dat.rng, real(log10(mpl_dat.cop)));
        h = colorbar;
        ...
        title('copol raw counts'); dynamicDateTicks; xlabel('date'); ylabel('range [km]');  ylabel(h, '[counts /microsec]')
        if Save_plot
            saveas(Fig,[save_plot_path, STR, 'miniMPL_cop_raw_2', datestr(mpl_dat_cor.time_datenum(1), 'yyyymmddHHMM'), '.png'])
