function [polavg,ind] = proc_mplpolfsraw_5(mplpol,inarg)
...
if ~isfield(mplpol,'ap')||(isfield(inarg,'assess_ap')&&inarg.assess_ap)
   mplpol = assess_ap(mplpol,inarg);
end

if ~isfield(mplpol,'ap')||(isfield(inarg,'replace_ap')&&inarg.replace_ap)
   mplpol = replace_ap(mplpol,inarg);
end
% dtc = interp1(mplpol.dtc.MHz, mplpol.dtc.correction,mplpol.rawcts_copol , 'linear','extrap');
dtc = 10.^interp1(mplpol.dtc.MHz,log10(mplpol.dtc.correction),mplpol.rawcts_copol,'linear','extrap');
dtc(mplpol.rawcts_copol<=0) = 1; 
mplpol.rawcts_copol = mplpol.rawcts_copol .* dtc;

% dtc = interp1(mplpol.dtc.MHz, mplpol.dtc.correction,mplpol.rawcts_crosspol , 'linear','extrap');
dtc = 10.^interp1(mplpol.dtc.MHz,log10(mplpol.dtc.correction),mplpol.rawcts_crosspol,'linear','extrap');
dtc(mplpol.rawcts_crosspol<=0) = 1; 
mplpol.rawcts_crosspol = mplpol.rawcts_crosspol .* dtc;

% The following lines are not 100% rigorous.  I'm just trying to avoid
% pathological behavior in the first few bins from the afterpulse and
% initial laser spike.% 
...
% Now being done before DTC
% then subtract bg, ap, ... 
% % ap = inarg.ap(polavg.range,polavg.time(1));
polavg.cop_ap = mplpol.r.ap_copol;
dtc = interp1(mplpol.dtc.MHz, mplpol.dtc.correction,polavg.cop_ap , 'linear','extrap');
polavg.cop_ap = polavg.cop_ap .* dtc;

polavg.crs_ap = mplpol.r.ap_crosspol;
dtc = interp1(mplpol.dtc.MHz, mplpol.dtc.correction,polavg.crs_ap , 'linear','extrap');
polavg.crs_ap = polavg.crs_ap .* dtc;

cop_ = polavg.cop; crs_ = polavg.crs;
polavg.cop = cop_; polavg.crs = crs_;
...
if ~isfield(polavg.r,'ol_corr')||inarg.replace_ol
polavg.r.ol_corr = inarg.ol_corr(polavg.range,polavg.time(1));
end
polavg.ol_corr = polavg.r.ol_corr;
