function mark_bad_mplnc_nz

files = getfullname('*.nc','minimpl_uc','Select UC miniMPL nc files to bundle into dailies.');
...
%      disp(files{f})
    try
        ...
        [pp,ff,ex] = fileparts(files{f});
        copyfile2(files{f},[pp,filesep,'bad',filesep,ff,'.bad',ex]);
    end
    date = floor(nc.time(1));
    [nc, nc_] = anc_sift(nc,nc.time<(date+1));
    while ~isempty(nc_)&&nc_.ncdef.recdim.length>0
