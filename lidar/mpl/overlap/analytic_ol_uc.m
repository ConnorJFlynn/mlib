...
figure; plot(rng, olc, 'o'); logx;logy
...
xl = xlim; x_top = xl(2);
olc = olc(rng>=x_bot & rng<=x_top);
rng = rng(rng>=x_bot & rng<=x_top);
if ~isavar('pin_R')
    pin_R = max(rng);
end
R = 10:10:10000; 
...
del = phi.*10.^linspace(-.8,-.1,10); % laser beam divergence as log-spaced factor of phi
clear Xi
...
figure; Rs = plot(rng, (ray*ones([1,length(del)])).* (X_ray.^2),'-',rng(1:10:end),ray(1:10:end).*olc(1:10:end),'-o'); logy
recolor(Rs,[1:length(Rs)]);
Rs = plot(R./1000,Xi.^2,'-',rng, olc,'c-',R./1000,olc_logmn,'m*'); xlabel('km'); logx; logy;
recolor(Rs,[1:length(Rs)])
% Construct basis set from Xi and stronger powers of Xi
V = [Xi'.^1.1, Xi'.^1.25,Xi'.^1.5, Xi'.^2]';
...
while isempty(K) || any(K==0) 
    ...
    K =olc_logmn/V; % K[1xR] = [1xC] / [RxC], when dividing matrices, flip denom dims, cancel like inner
     ...
     [R,xx] = unique([R,pin_R.*1000]); fitted_ol = [fitted_ol' top]; fitted_ol = fitted_ol(xx);
...
fitted_olc = 10.^interp1(log10(R),log10(fitted_ol),log10(rng_in*1000),'linear','extrap');
