function [vis_cal, nir_cal] = Radiance_cals_SASZe2_Ames_preAmazon_2013_09_06
% D:\case_studies\radiation_cals\2013_09_06.SASZE2_NASA_Ames_postrepair_preAmazon.Flynn
% Measurements taken by Connor at NASA Ames in May, just prior to GoAmazon deployment after the
% after the unit was broken following the post-TCAP2 calibration at GSFC. 
...
if exist([pname,nir_files(file_n).name],'file')&&exist([pname, strrep(nir_files(file_n).name, '_nir_','_vis_')], 'file')
    ...
    lamp_i = strfind(nir.pname, 'Lamps');
    if ~isempty(lamp_i)&&(lamp_i~=0)
        tmp = (strtok(fliplr(nir.pname(lamp_i + 5:end)),'_'));
        lamp = fliplr(tmp(2:end));
    end
...
for file_n = 1:length(nir_files)
    ...
    lamp_i = strfind(nir.pname, 'Lamps');
 if ~isempty(lamp_i)&&(lamp_i~=0)
        lamp = num2str(sscanf(nir.pname(lamp_i+5:end),'%d'));
    end
