function [vis_resp, nir_resp] = radcal_Ze2_EPC_Oriel_20230111
% [vis_resp, nir_resp] = radcal_Ze2_EPC_Oriel_20230111;
% Measurements taken by Connor at EPC with Oriel lamp and spectralon blank
% with SASZe2.  Multiple integration times for  VIS and NIR specs for NLC
%
...
% In reality the shutter was not active so must use open_lamp - open_diffuse
...
vis_stem = 'epcsaszevisM1';nir_stem = 'epcsaszenirM1';
...
clear header
header(1) = {'% SASZe2 radiance calibration at EPC by Connor Flynn'};
...
for v = length(ints) % :-1:1
    ...
    %     figure; plot(nir.wl, [nir_resp; nir_respa; nir_respb],'-', nir.wl, nresp_pct_err,'k-'); logy; zoom('on');
    ok = false;
    while ~ok
        figure_(832039284);
