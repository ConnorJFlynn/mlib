function [vis_cal, nir_cal] = Sphere_cals_SASZe_auto(sas_pname);
...
for ll = lamps
    ...
    if exist([sas_pname, lamp_str,filesep,nir_files(file_n).name],'file')&&...
        ...
        for spc_ = spec_str; spc = char(spc_);
            ...
            cal.(lamp_str).(spc).irad = rad_ii.(spc);
...
save([sas_pname,'SASZe_rawcal.', datestr(cal.(lamp_str).(spc).time,'yyyymmdd_HHMMSS'),'.mat'],'-struct','sascal');
save([sas_pname,'SASZe_radcal.', datestr(cal.(lamp_str).(spc).time,'yyyymmdd_HHMMSS'),'.mat'],'-struct','cal');
% Now, build "cals" structure
% cals.vis.lambda
% cals.vis.Lamps_N.tint
% cals.vis.Lamps_N.(spec([tints,lambda])
% cals.vis.tint_nn_ms.lamp
% cals.vis.tint_nn_ms.(spec[lamp,lambda])
% save([sas_pname, strrep(resp_stem,'.dat','.mat')],'-struct','cals');
 for spc_ = spec_str; spc = char(spc_);
all_tints = [];
     for ll = length(lamps):-1:1
         lamp_str = strrep(['Lamps_',sprintf('%d',lamps(ll))],'.','p');
         ...
         for ti = length(tints):-1:1
             ...
             cals.(spc).(lamp_str).light(ti,:) = cal.(lamp_str).(spc).(['light_',tint_str]);
             cals.(spc).(lamp_str).welldepth(ti,:) = cal.(lamp_str).(spc).(['welldepth_',tint_str]);
             cals.(spc).(lamp_str).dark(ti,:) = cal.(lamp_str).(spc).(['dark_',tint_str]);
             cals.(spc).(lamp_str).sig(ti,:) = cal.(lamp_str).(spc).(['sig_',tint_str]);
             cals.(spc).(lamp_str).rate(ti,:) = cal.(lamp_str).(spc).(['rate_',tint_str]);
