...
for ll = lamps
    ...
    nir = rd_spc_TCAP_v2([pname,lamp_str,filesep,'20131121_',fnum,'_NIR_park.dat']);
    vis = rd_spc_TCAP_v2([pname,lamp_str,filesep,'20131121_',fnum,'_VIS_park.dat']);
    shut = nir.t.shutter==0;
    shut(2:end)= shut(1:end-1)&shut(2:end), shut(1:end-1) = shut(1:end-1)&shut(2:end);
    sun = nir.t.shutter==1;
    sun(2:end) = sun(1:end-1)&sun(2:end); sun(1:end-1) = sun(1:end-1)&sun(2:end);
...
linkaxes(sb,'off');  
subplot(2,1,1);plot(vis.nm, cal.(lamp_str).vis.resp,'-'); 
title([lamp_str, ': ',vis.fname],'interp','none');
ylabel('resp');
