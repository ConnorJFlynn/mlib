function [rad,lamp] = planck_tungsten_fit(nm_in,Irad_in, nm_out)
% [rad,lamp] = planck_tungsten_fit(nm_in,Irad_in, nm_out)
...
W =  tungsten_spectral_emissivity_vs_temperature;
W.nm = lamp.nm;
...
[max_I, max_wi] = max(lamp.I);
T_peak = b./(lamp.nm(max_wi).*1e-9);
em = tungsten_spectral_emissivity_vs_lambda_at_T(W,T_peak,lamp.nm);
lamp_to_planck = lamp.I./em;

[max_I, max_wi] = max(lamp_to_planck);
T_peak = b./(lamp.nm(max_wi).*1e-9);
em = tungsten_spectral_emissivity_vs_lambda_at_T(W,T_peak,lamp.nm);
lamp_to_planck = lamp.I./em;

T_min = b./(lamp.nm(max_wi+2).*1e-9);
T_max = b./(lamp.nm(max_wi-2).*1e-9);
...
% W.emis_nm = interp1(W.um, W.emis_nm,'

I_planck = planck_in_wl((lamp.nm.*1e-9),T_peak);
N = I_planck(max_wi)./lamp_to_planck(max_wi);
% I_planck = I_planck./N;
resid = (w_i.*sqrt((I_planck(W_)-N.*lamp_to_planck(W_)).^4));

em = tungsten_spectral_emissivity_vs_lambda_at_T(W,T_peak-dT,lamp.nm);
lamp_to_planck = lamp.I./em;
I_planck_n = planck_in_wl((lamp.nm.*1e-9),T_peak-dT);
N = I_planck_n(max_wi)./lamp_to_planck(max_wi);
% I_planck_n = I_planck_n./N;
resid_n = (w_i.*sqrt((I_planck_n(W_)-N.*lamp_to_planck(W_)).^4));

em = tungsten_spectral_emissivity_vs_lambda_at_T(W,T_peak+dT,lamp.nm);
lamp_to_planck = lamp.I./em;
I_planck_p = planck_in_wl((lamp.nm.*1e-9),T_peak+dT);
N = I_planck_p(max_wi)./lamp_to_planck(max_wi);
% I_planck_p = I_planck_p./N;
resid_p = (w_i.*sqrt((I_planck_p(W_)-N.*lamp_to_planck(W_)).^4));

T = T_peak;
% plot(lamp.nm(W_), N.*lamp_to_planck(W_),'bo',lamp.nm, lamp.I,'k-',...
...
while mean(resid_p)<mean(resid) && mean(resid)<mean(resid_n) && T<T_max
    ...
    I_planck = I_planck_p;
    resid = resid_p;
    em = tungsten_spectral_emissivity_vs_lambda_at_T(W,T+dT,lamp.nm);
    lamp_to_planck = lamp.I./em;
%     [max_I, max_wi] = max(lamp_to_planck);
...
%     I_planck_p = I_planck_p./N;
    resid_p = (w_i.*sqrt((I_planck_p(W_)-N.*lamp_to_planck(W_)).^4));
%     plot(lamp.nm(W_), N.*lamp_to_planck(W_),'bo',lamp.nm, N.*lamp_to_planck,'k-',...
%         lamp.nm, I_planck_n,'r-',lamp.nm, I_planck,'g-',lamp.nm, I_planck_p,'b-');
%     xlim(xl);
    %  [mean(resid_n), mean(resid), mean(resid_p)]
end
...
%%
% plot(lamp.nm, N.*lamp_to_planck,'k-',lamp.nm(W_), N.*lamp_to_planck(W_),'bo',lamp.nm, I_planck,'g-');
% xlim(xl);
% xlabel('wavelength [nm]');
% ylabel('irradiance [W/(cm^2*nm)]')
% legend('calibration','points in fit','scaled planck curve')
...
lamp_corrected_to_plank = lamp.I ./ em;
best_plank = planck_in_wl(lamp.nm.*1e-9,T);
lamp.N = best_plank(max_wi)./lamp_corrected_to_plank(max_wi);
% figure; plot(lamp.nm, best_plank, 'k-',lamp.nm, lamp.N.*lamp_corrected_to_plank,'ro');
% legend(sprintf('T_b = %4.0f K',T), 'scaled lamp')
lamp.T_bb = BrightnessTemp_vs_nm(lamp.N.*lamp_corrected_to_plank, lamp.nm);

%     em = tungsten_spectral_emissivity_vs_lambda_at_T(W,T,lamp.nm(max_ii));
% lamp.N = planck_in_wl(lamp.nm(max_ii).*1e-9,T) ./ lamp.I(max_ii);
% % lamp.T_bb = BrightnessTemp_vs_nm(em(max_ii).*lamp.I.*lamp.N./em, lamp.nm);
% lamp.T_bb = BrightnessTemp_vs_nm(lamp.I.*lamp.N, lamp.nm);
rad.nm = nm;
rad.T_bb = interp1(lamp.nm, lamp.T_bb, rad.nm, 'linear','extrap');
for ni = length(rad.nm):-1:1
    emis(ni) = tungsten_spectral_emissivity_vs_lambda_at_T(W,rad.T_bb(ni),rad.nm(ni));
end
%%
rad.BB_rad = planck_in_wl(rad.nm.*1e-9, rad.T_bb); %BB values matching corrected lamp curve Tb
rad.N_planck_by_lamp = lamp.N;
rad.Irad = emis.*rad.BB_rad./rad.N_planck_by_lamp;
...
plot(lamp.nm, lamp.I,'ro',lamp.nm, em.*I_planck./N,'g-',rad.nm, rad.Irad,'k-');
xlim(xl);
ylabel('irad');
xlabel('Wavelength [nm]');
legend('Lamp Irad', 'scaled Planck','fitted Planck')
linkaxes(s,'x');
%             [mean(resid_n), mean(resid), mean(resid_p)]
%%
% save('fun.mat')
