...
% fid_506 = fopen(['C:\case_studies\radiation_cals\2012_12_12_SWS_NOAA_Schmidt\f506.txt']);
...
sws.Si_resp = sws.Si_sig./(1e7.*rad_577.Irad(1:256).*spec_panel.SWS_refl(1:256)./pi);
sws.In_resp = sws.In_sig./(1e7.*rad_577.Irad(257:end).*spec_panel.SWS_refl(257:end)./pi);
figure; plot(sws.Si_lambda, Si_resp.fel577_240ms,'rx-',sws.Si_lambda, sws.Si_resp,'o-',...
    sws.In_lambda, In_resp.fel577_100ms, 'rx-',sws.In_lambda, sws.In_resp,'o-');
title('Responsivity from FEL 577, Si 80 ms');
ylabel('resp [(counts/ms)/(W/m2/um/sr)]')
xlabel('wavelength [nm]')
legend('S Schmidt','Mine/2');
%%
resp_Si = [sws.Si_lambda, sws.Si_resp];
[resp_stem, resp_dir] = gen_sws_resp_files(resp_Si,floor(sws.time(1)), unique(sws.Si_ms),'C:\case_studies\radiation_cals\2012_12_12_SWS_NOAA_Schmidt\resp_files\');
%%
resp_InGaAs = [sws.In_lambda, sws.In_resp];
[resp_stem, resp_dir] = gen_sws_resp_files(resp_InGaAs,floor(sws.time(1)), unique(sws.In_ms),'C:\case_studies\radiation_cals\2012_12_12_SWS_NOAA_Schmidt\resp_files\');
%%
