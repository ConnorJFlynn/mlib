function radcal_sws_resp_all_ARC455_Apers_Nov2011
...
ames_path = 'C:\case_studies\radiation_cals\2011_Nov_ARC\2011_Nov_09_NASA_Ames\';
...
dark_ii = find(sws.shutter==1);
figure; 
sb(1) = subplot(2,1,1); 
plot([1:length(sws.time)], max(sws.Si_DN,[],1),'b-o',...
    dark_ii,max(sws.Si_DN(:,dark_ii),[],1),'r.'); zoom('on');
sb(2) = subplot(2,1,2); 
plot([1:length(sws.time)], max(sws.In_DN,[],1),'r-o',...
  dark_ii, max(sws.In_DN(:,dark_ii),[],1)  ); zoom('on');
linkaxes(sb,'x') 
%%
