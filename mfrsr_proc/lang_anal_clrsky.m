function [lang] = lang_anal_clrsky(arg)
...
%[what] = lang_anal(arg)
swf = ancload('C:\case_studies\langley_anal\sgp_clearsky.cdf');
mfr = ancload('C:\case_studies\langley_anal\sgpmfrsrC1.a0.20050401.000000.nc');


[ref_t, i, j] = unique(ref_t);
 ref_flag=ref_flag(i);
 flag = interp1(ref_t, single(ref_flag), in_t, 'linear');

tmp = flagor(swf.time, swf.vars.flag_clearsky_detection.data, mfr.time);
mfr.vars.skytest.data = bitset(mfr.vars.skytest.data,2,(tmp > .5));
...
mfr.vars.hour_angle.datatype = 5;
...
if ~isempty(swanal)
   ...
   if length(sw_file)>0
      ...
      mfr.vars.skytest.data = bitset(mfr.vars.skytest.data,2,1);
   end
   %Set clearsky to true if NaN
   NaNs = isnan(mfr.vars.skytest.data);
   mfr.vars.skytest.data(NaNs) = bitset(mfr.vars.skytest.data(NaNs),2);
end
...
%                   lang.midtime(ch,sky,am_num,t) = mean(mfr.time(mass));
                  ...
                  langplot.vars.good.data(am_num,:) = false;
...
%                   disp('No fits!')
                  ...
                  langplot.vars.good.data(am_num,good) = true;
                  %                if any(good)
                  ...
                  langplot.vars.unweight_goodfit.data(ch,sky,am_num) = new.goodfit;
...
mfr.vars.skytest.datatype = 4;
mfr.vars.skytest.atts = mfr.vars.time_offset.atts;
mfr.vars.skytest.atts.long_name.data = 'sky condition flag';
...
langplot.vars.airmass_range.datatype = 3;
...
langplot.vars.PM.atts.values.data = '0=AM, 1=PM';
 ...
 langplot.vars.skyflag.atts.values.datatype = 1;
 
 langplot.vars.skytest.dims = {'time'};
 langplot.vars.skytest.datatype = 1;
...
langplot.vars.good.data = (zeros(am_num,t)>1);
...
lang.vars.airmass_range.datatype = 3;
...
lang.dims.skyflag.isunlim = false;
 ...
 lang.vars.skyflag.atts.values.datatype = 1;
...
function arg = setarg;
arg.pname = 'E:\case_studies\Alive\data\sgpmfrsrC1\b1\solarday\';
...
% arg.Vo
arg.swanal.pname = ['E:\case_studies\Alive\data\sgp1swfanal\'];
