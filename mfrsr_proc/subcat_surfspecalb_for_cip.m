...
mfr = getfullname('*surfspecalb*.cdf','cip','Select a surfspecalb file from the directory you want to process.');
if iscell(mfr)
    mfr = mfr{1};
end
...
for f = 1:length(files)
    ...
    for fld = length(fields):-1 :1
        field =fields{fld};
        if isempty(strfind(field,'time'))&&isempty(strfind(field,'mfr'))&&isempty(strfind(field,'surface_type'))...
                &&isempty(strfind(field,'filter'))&&isempty(strfind(field,'lat'))&&isempty(strfind(field,'lon'))...
                &&isempty(strfind(field,'alt'))
