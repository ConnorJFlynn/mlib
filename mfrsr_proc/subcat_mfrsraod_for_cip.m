function nc = subcat_mfrsraod_for_cip
...
mfr = getfullname('*mfr*.cdf','cip','Select an MFRSR or NIMFR AOD file from the directory you want to process.');
if iscell(mfr)
    mfr = mfr{1};
end
...
for f = 1:length(files)
    ...
    vars.aerosol_optical_depth_filter2 = in.vars.aerosol_optical_depth_filter2;
    vars.aerosol_optical_depth_filter5 = in.vars.aerosol_optical_depth_filter5;
    vars.qc_aerosol_optical_depth_filter2 = in.vars.qc_aerosol_optical_depth_filter2;
    vars.qc_aerosol_optical_depth_filter5 = in.vars.qc_aerosol_optical_depth_filter5;
    vars.direct_diffuse_ratio_filter2 = in.vars.direct_diffuse_ratio_filter2;
    vars.direct_diffuse_ratio_filter5 = in.vars.direct_diffuse_ratio_filter5;
    ...
    for fld = length(fields):-1 :1
        ...
        if ~isempty(strfind(field,'direct_hor'))||~isempty(strfind(field,'total_optical'))||~isempty(strfind(field,'filter6'))...
                ...
                ||~isempty(strfind(field,'aerosol_optical'))||~isempty(strfind(field,'_filter'))
            in.vars = rmfield(in.vars,field);
        end
    end
    in.vars.aerosol_optical_depth_filter2 = vars.aerosol_optical_depth_filter2;
    in.vars.aerosol_optical_depth_filter5 = vars.aerosol_optical_depth_filter5;
    in.vars.qc_aerosol_optical_depth_filter2 = vars.qc_aerosol_optical_depth_filter2;
    in.vars.qc_aerosol_optical_depth_filter5 = vars.qc_aerosol_optical_depth_filter5;
    in.vars.direct_diffuse_ratio_filter2 = vars.direct_diffuse_ratio_filter2;
    in.vars.direct_diffuse_ratio_filter5 = vars.direct_diffuse_ratio_filter5;
