...
mfr_lbl_path = ['C:\MinGW\msys\1.0\home\LBLRTM\mfr7nch_cimel_1p6um\'];
...
for m = length(mfr_files):-1:1
   ...
   first_time(m) = mfr_.time(1); 
   head_id(m) = sscanf(mfr_.gatts.head_id,'%f');
   [m, head_id(m)]
end
miss = head_id <=0;
first_time(miss) = []; head_id(miss) = []; mfr_files(miss) = [];
ij = length(head_id);
% This loop doesn't really work.  But it doesn't really need to work automously so
% right now I'm not fixing it. 
while ij >= 1
   
   while ij>1 && (head_id(ij)== head_id(ij-1))
      ij = ij -1;
   end

   mfr7 = anc_load(mfr_files{ij}); % Make more robust to handle head changes and DOD changes.
