...
for f = 1:length(files)
    ...
    for fld = length(fields):-1 :1
        field =fields{fld};
        % Remove each of the fields found below
        % Careful.  The Io test must precede the dims test else number of
        % inputs problem with strfind will result.
        if ~isempty(strfind(field,'direct_hor'))||~isempty(strfind(field,'total_optical'))||~isempty(strfind(field,'filter6'))...
                ...
                ||~isempty(strfind(field,'_raw'))||~isempty(strfind(field,'Io_'))...
                ||~isempty(strfind(in.vars.(field).dims{:},'wavelength'))...
