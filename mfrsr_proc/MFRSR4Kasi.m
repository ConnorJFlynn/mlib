...
%     mfrsr.vars.(char(fields{f})).data =   nimfr.vars.(char(fields{f})).data(bina);
% end
...
[pname, ~, ~] = fileparts(mfr.fname);
fname = [datestr(mfr.time(noon),'yyyymmdd'),'.txt'];
...
fname = ['cos_normal_',datestr(mfr.time(1),'yyyymmdd'),'.txt'];
fid_out = fopen([pname, filesep, fname],'w');
...
if isfield(mfr.vars,'diffuse_hemisp_narrowband_filter1')
    ...
    fname = ['diffuse_',datestr(mfr.time(1),'yyyymmdd'),'.txt'];
    fid_out = fopen([pname, filesep, fname],'w');
