...
if sscanf(rsr.header.firmware_version,'%g')==13
   ...
   n = nchannels*((sscanf(rsr.header.alltime_channels,'%x')>0) ...
      + sscanf(rsr.header.daytime_channels, '%x'));
   rsr.vars.solar_azimuth.data = data(:,4+n);
   rsr.vars.solar_elevation.data = data(:,4+n+1);
   rsr.vars.solar_zenith.data = data(:,4+n+2);
   rsr.vars.hour_angle.data = data(:,4+n+3);
   rsr.vars.declination.data = data(:,4+n+4);
   rsr.vars.airmass.data = data(:,4+n+5);
   rsr.vars.solar_dist_au.data = data(:,4+n+6);
   rsr.vars.solar_time.data = data(:,4+n+7);
   for ch = 0:6;
      ...
      if daytime_channels>0

      rsr.vars.(['th_ch',num2str(ch+1)]).data = data(:,4 + nchannels+ch);
      NaNs = find(rsr.vars.(['th_ch',num2str(ch+1)]).data<-9000);
      rsr.vars.(['th_ch',num2str(ch+1)]).data(NaNs) = NaN;

      rsr.vars.(['dif_ch',num2str(ch+1)]).data = data(:,4 + nchannels*2+ch);
      NaNs = find(rsr.vars.(['dif_ch',num2str(ch+1)]).data<-9000);
      rsr.vars.(['dif_ch',num2str(ch+1)]).data(NaNs) = NaN;
      
      rsr.vars.(['dirhor_ch',num2str(ch+1)]).data = data(:,4 + nchannels*3+ch);
