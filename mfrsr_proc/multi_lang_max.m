function [good, Vo,tau,Vo_, tau_,settings, final_stats] = multi_lang_max(in_data,tests,show)
% [good, Vo,tau,Vo_, tau_] = multi_lang_max(in_data,tests,show);
...
settings = tests;
% settigns.stdev_mult=stdev_mult;
...
if show>0
   if prescreen.on
      fig1 = figure(1001);
      set(fig1,'position',[147    35   560   420]);
   end
   if show>1
   fig2 = figure(1005);
   set(fig2,'position',[757    34   560   420]);
   end
end
...
%       mad_ = max((dev_(good)));      
      ...
      N_test =  sum(good) < tests.Ntimes ; % This is the abort condition
      ...
      Vo_test_uw = (Vo_ > Vo_ray);%&&(Vo_ < Vo_max);
       if show==2
         ...
         set(ax(1),'position',[pos1(1:3),.3]);
         
         ax(2) = subplot(2,1,2);
         scatter(1./in_data.airmass(good), real(logV(good))./in_data.airmass(good), 6,(dev_(good))./sdev_);colorbar;
