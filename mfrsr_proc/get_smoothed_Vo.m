function Vo = get_smoothed_Vo(time, langs, N);
...
% Considering modification to compute robust ratios as well and use
% the ratios to extend the calibration from filter 2 to each
% subsequent channel.
...
% At this point we can cycle through the channels forming ratios vs
% one, run a forgan smoothing on this ratio time series, then multiply
% the smoothed filter2 Io by the smoothed ratio to get Io for each
% channel.
