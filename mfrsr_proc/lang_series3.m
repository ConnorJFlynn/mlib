function [lang_common] = lang_series3(inarg);
% [lang_common, langs] = lang_series3(inarg);
...
for fnum = 1:length(in_file)
   ...
   if sum(AM)>=inarg.tests.Ntimes
      ...
      [status, lang_result,langplot] = common_mfr_lang3(leg,inarg);
      if status
...
txt_fname = [Io_outdir, part1,'.daily_Ios.v',num2str(v),'.dat'];
