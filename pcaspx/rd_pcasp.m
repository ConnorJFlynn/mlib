function d = rd_pcasp(filename)
%d = rd_pcasp(filename);
% Reads PCASP data file in entirety. Uses internal fnt to read setup block
% computes adj_threshold accounting for range table and AD span.
% That is, adjust by increments of 4096 depending on range table selected
% Also computes adj_Avg dividing by bin width
...
if fid>0
    ...
    while ~feof(fid)
        try
            ...
            d.endRejects(i) = C{9};
            d.sumofParticles(i) = double(C{10});
            d.sumofTransit(i) = double(C{11});
...
d.airSpeed = double(C{2});
d.temperature = double(C{3});
format = ['%d %d %d %*[^\n]'];
C = textscan(fid, format, 1,'delimiter',', ');
d.pressure = double(C{1});
d.transitReject = double(C{2});
d.DOFReject = double(C{3});
format = ['%d %d %*[^\n]'];
C = textscan(fid, format, 1,'delimiter',', ');
d.avgTransitTime = double(C{1});
d.avgTransitTimeAccept = double(C{2});
format = ['%d %d %*[^\n]'];
C = textscan(fid, format, 1,'delimiter',', ');
d.displayInterval = double(C{1});
d.pollingInterval = double(C{2});
format = ['%d %d %d %d %d %d %*[^\n]'];
C = textscan(fid, format, 1,'delimiter',', ');
d.countMethod = C{1};
d.divisorFlag = C{2};
...
format = ['%f %f %d %*[^\n]'];
C = textscan(fid, format, 1,'delimiter',', ');
d.beamDiameter = double(C{1});
d.DOFLength = double(C{2});
d.clockSpeed = double(C{3});
format = ['%s %f %f %*[^\n]'];
C = textscan(fid, format, 1,'delimiter',', ');
d.serialNo = C{1};
