% 2009-05-08, CJF: stepping through and tweaking for reprocessing of MPL
...
CARL='Yes';
CARL='No';
Latitude_SGP=36.6;    %SGP
Longitude_SGP=-97.48;  %SGP
MPL='No';
MPL='Yes';
...
zmax=8.00; %max altitude for binning [km]
bw=0.020; %bin width for AATS-14
if strcmp(MPL,'Yes')
...
%             dist(L) = geodist(Latitude(L),Longitude(L),Latitude_SGP,Longitude_SGP)/1000;
%         end
        dist = geodist2(Latitude,Longitude,Latitude_SGP,Longitude_SGP)/1000;
%         dist = dist';
%         dist=deg2km(distance(Latitude,Longitude,Latitude_SGP,Longitude_SGP));
        ...
        [ResMat]  =binning([GPS_Altitude',dist'],bw,0,zmax);
        dist_bin = ResMat(:,2);
    ...
    if strcmp(product,'h2o_profile')
        ...
        H2O_dens_is=data(11,:);
        dist=deg2km(distance(Latitude_SGP,Longitude_SGP,Latitude,Longitude));
        %accumulate data
...
%     end
     if strcmp(CARL,'Yes') && strcmp(product,'h2o_profile')
        %load CARL data
        [CARL_UT,CARL_aod,CARL_z,CARL_aod_profile,CARL_aod_err,CARL_ext,CARL_ext_err,CARL_r,CARL_p,CARL_t,CARL_h2o_dens,CARL_w_max_height]=read_CARL_ALIVE('c:\beat\data\alive\carl\',day,month,year);
        %remove profiles that have only values less or equal 0
        ii=find(all(CARL_r<=0,2)==1);
        CARL_w_max_height(ii)=[];
        CARL_r(ii,:)=[];
        CARL_UT(ii)=[];
        CARL_p(ii,:)=[];
        CARL_t(ii,:)=[];
        ...
        CARL_w_max_height=mean(CARL_w_max_height(ii:jj));
        
        %CARL_w_max_height=zmax; % I think w_mx criteria is too conservative
        kk=find(CARL_z<=CARL_w_max_height);
        ...
        [ResMat]         = binning([CARL_z(kk),CARL_h2o_dens(:,kk)'],bw,0,zmax);
        ...
        set(gcf,'Paperposition',[0.2 0.1 8 10.7]);
        subplot('position',[0.06+col*0.19 0.85-row*0.16 0.16 0.14])
        plot(CARL_h2o_dens,CARL_z,'r.-',H2O_dens,GPS_Altitude,'b.-',H2O_dens_is,GPS_Altitude,'g.-')
        set(gca,'ylim',[0 8]);
        set(gca,'xlim',[0 24]);
        set(gca,'TickLength',[.03 .03]);
        set(gca,'xtick',0:4:24);
    ...
    if strcmp(CARL,'Yes') && strcmp(product,'aod_profile')
        %load CARL data
        [CARL_UT,CARL_aod,CARL_z,CARL_aod_profile,CARL_aod_err,CARL_ext,CARL_ext_err,CARL_r,CARL_p,CARL_t]=read_CARL_ALIVE('C:\case_studies\Alive\data\ferrare-carl\Feb2007\',day,month,year);
        %remove profiles that have only values less or equal 0
        ...
        CARL_aod_all = [CARL_aod_all;CARL_aod_profile_bin'];
        % plot AOD profile
        figure(5)
        set(gcf,'Paperposition',[0.2 0.1 8 10.7]);
        subplot('position',[0.06+col*0.19 0.85-row*0.16 0.16 0.14])
        ...
        hold on
        plot(CARL_aod_profile,CARL_z,'m.-');
        ...
        set(gcf,'Paperposition',[0.2 0.1 8 10.7]);
        subplot('position',[0.06+col*0.19 0.85-row*0.16 0.16 0.14])
        plot(CARL_ext,CARL_z,'m.-',Extinction(1,:),GPS_Altitude,'b.-')
        set(gca,'ylim',[0 8]);
    ...
    if strcmp(MPL,'Yes')
        %load MPLNET data
        [MPL_z,MPL_UT,MPL_aot,MPL_ext]=read_MPLARM_ALIVE('C:\case_studies\Alive\data\flynn-mpl-102\2007_03_05\ext\',0.319,day,month,year);
        ...
        kk=find(MPL_ext(ii,:)~=-9999); %remove altitudes with invalid retrievals
                % small problem here?  Looks like the following line removes kk
        % altitudes from ii:jj but was only derived from ii.  This means it
        % may leave some -9999 in the mean, and also strip out some valid
        % values for other (not ii) profiles
        ...
        MPL_aod_all=[MPL_aod_all; MPL_AOD_bin'];
...
if strcmp(CARL,'Yes') && strcmp(product,'aod_profile')
    ...
    x=AATS_ext353_all;
    y=CARL_ext_all;
    ii=find(dist_all<=30); % remove points beyond 30 km
    x=x(ii); y=y(ii);
    ii=find((~isnan(x).*~isnan(y))==1); %remove NaNs
    x=x(ii); y=y(ii);
