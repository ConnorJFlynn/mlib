...
%    close('all')
   ...
   mpl_pname = ['C:\case_studies\Alive\data\mpl4b\ext\'];
   % load MPL file
   mpl_file = dir([mpl_pname,'*.ext.',datestr(AATS.time(1),'yyyymmdd'),'*.cdf']);
   % apply corrections
%    mpl = mpl004_alive([mpl_pname, mpl_file(1).name]);
   mpl = mpl102_alive_2([mpl_pname, mpl_file(1).name]);
   mpl.range = mpl.vars.range.data;
   mpl.prof = mpl.vars.atten_bscat.data;
   mpl.hk.energy_monitor = mpl.vars.energy_monitor.data;
   mpl.hk.bg = mpl.vars.background_signal.data;
...
%    mpl = cutmpl(mpl,[ii:jj]);    
   if exist('clear_sky2.mat', 'file')
      clear_sky = loadinto('clear_sky2.mat');
   end
   
   % Now generate lidar calibration:
   % compute mean of MFRSR AOD and Ang, 1 and 2
   %
   mpl.r.cal= (mpl.range>=7.25) & (mpl.range<=8.75);
   mpl.r.lte_cal = (mpl.range>.1)&(mpl.range<=8.75);
%    mpl.cal.atten_ray = 10.^(mean(log10(mpl.sonde.atten_ray(mpl.r.cal))));
   ...
   mpl.cal.mean_prof = (10.^(mean(real(log10(mpl.prof(mpl.r.cal,ii:jj))))));
   mpl.cal.mean_em = mean(mpl.vars.energy_monitor.data(ii:jj));
   mpl.cal.mean_em = 6;
   mpl.cal.aod_523 = mean(mpl.vars.aod_523nm.data(ii:jj));
   tau = cumtrapz(mpl.range, mpl.vars.alpha_m.data);
   atten_ray = mpl.vars.beta_m.data .* exp(-2.*tau);
   cal_ray = 10.^(mean(real(log10(atten_ray(mpl.r.cal)))));
   %!!
   klett_fig = figure(4);
   ...
   beta_m = mpl.vars.beta_m.data(shorter);
   Sa = 30;
...
mean_prof = (10.^(mean(real(log10(mpl.prof(mpl.r.cal,:))))));
good_prof = find(mean_prof>1);
clean_mpl = intersect(good_prof,[ii:jj]);
%    clean_mpl = clean_mpl(~isnan(mpl.cal.aod_523));
   
   
   i = 0;
%       mpl.cal.C = mpl.cal.mean_prof ./ (mpl.cal.atten_ray .* exp(-2.*mpl.cal.aod_523));
  figure(klett_fig);
  close(klett_fig);
  figure(klett_fig);
   for clear_bin = [clean_mpl]
      figure(klett_fig);
      mean_prof = (10.^(mean(real(log10(mpl.prof(mpl.r.cal,clear_bin))))));
      ...
      mpl.cal.C = mean_prof ./ (cal_ray .* exp(-2.*aod)); 
      lidar_C = mpl.cal.C;
      subplot(1,2,1); semilogx(profile./(exp(-2*aod).*lidar_C), range, 'c',  atten_ray(mpl.r.lte_cal), mpl.range(mpl.r.lte_cal), 'r');
      axis([ 0.0001    0.1, min(range), max(range)])
      ...
      subplot(1,2,2); plot(mpl.klett.alpha_a(shorter, clear_bin),mpl.range(shorter), '-');
      grid('on');hold('on')
      axis([-0.1 .2 min(range), max(range) ]);
      tl = title(title_str);
      set(tl,'FontName','Tahoma','FontWeight','bold','Position',[-0.133    9.25   17.3205]);
      subplot(1,2,1);
      mpl.vars.lidar_C.data(clear_bin) = lidar_C;
      mpl.vars.Sa_Klett.data(clear_bin) = mpl.klett.Sa(clear_bin);
      mpl.vars.atten_bscat.data(:,clear_bin) = mpl.prof(:,clear_bin);
      mpl.vars.alpha_a_Klett.data(:,clear_bin) = mpl.klett.alpha_a(:,clear_bin);
      mpl.vars.beta_a_Klett.data(:,clear_bin) = mpl.klett.beta_a(:,clear_bin);
   end
   subplot(1,2,2);hold('off');
   mpl_ret_pname = ['C:\case_studies\Alive\data\mpl4b\ext\May2009\'];
   fstem = 'mpl102.ext.';
   dstem = datestr(mpl.time(1), 'yyyymmdd.');
   vstem = datestr(now, 'yyyymmdd');
   mpl.fname = [mpl_ret_pname,fstem,dstem,'ver_',vstem,'.cdf'];
   if exist(mpl.fname,'file')
      mpl_ = ancload(mpl.fname);
      if length(mpl.time)==length(mpl_.time)
         good = mpl.klett.Sa>0;
         disp(['Updating ',num2str(sum(good)),' new values.']);
         mpl_.vars.Sa_Klett.data(good) = mpl.vars.Sa_Klett.data(good);
         mpl_.vars.lidar_C.data(good) = mpl.vars.lidar_C.data(good)
         mpl_.vars.aod_523nm.data(good) =  mpl.vars.aod_523nm.data(good);
         mpl_.vars.beta_a_Klett.data(:,good) =  mpl.vars.beta_a_Klett.data(:,good);
         mpl_.vars.alpha_a_Klett.data(:,good) =  mpl.vars.alpha_a_Klett.data(:,good);
         mpl_.vars.atten_bscat.data(:,(good)) =  mpl.vars.atten_bscat.data(:,(good));
         mpl = mpl_; clear mpl_
      end
   end
   mpl = anccheck(mpl);
   ...
   mpl.clobber = true;
   ancsave(mpl);
   mpl.klett.alpha_a = mpl.vars.alpha_a_Klett.data;
   mpl.klett.aod_523nm = mpl.vars.aod_523nm.data;
   mpl.klett.Sa = mpl.vars.Sa_Klett.data;
...
%    [status,mpl_ext] = write_mpl_ret(mpl,[mpl_ret_pname,fstem,dstem,'v',vstem,'.cdf']);
   ...
   lidar.z = mpl.vars.range.data' + 0.319;
%disp(z');
