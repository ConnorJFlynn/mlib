function sws_sasze_same_source
...
sas_on_sws.sws_Si_rate = mean(sws.Si_rate(ainb_vis, sws.shutter==0),2);
sas_on_sws.sws_In_rate = mean(sws.In_rate(ainb_nir, sws.shutter==0),2);
sas_on_sws.sas_Si_rate = mean(sas_vis.rate(sas_vis.Shutter_open_TF==1,bina_vis))';
sas_on_sws.sas_In_rate =  mean(sas_nir.rate(sas_nir.Shutter_open_TF==1,bina_nir))';

figure; 
s(1) = subplot(2,1,1); plot(sas_on_sws.Si_lambda, sas_on_sws.sws_Si_rate, '-b',sas_on_sws.Si_lambda, sas_on_sws.sas_Si_rate, '-g', ...
    sas_on_sws.In_lambda, sas_on_sws.sws_In_rate, '-b',sas_on_sws.In_lambda, sas_on_sws.sas_In_rate, '-g'); legend('sws','sasze');
title(['SASZe and SWS ratio on ',datestr(sws.time(1), 'yyyy-mm-dd HH:MM:00')]);
s(2) = subplot(2,1,2); plot(sas_on_sws.Si_lambda, sas_on_sws.sas_Si_rate./sas_on_sws.sws_Si_rate, '-k', ...
    sas_on_sws.In_lambda,10.* sas_on_sws.sas_In_rate./sas_on_sws.sws_In_rate, '-r'); legend('sasze vis / sws vis', '10 x sasze nir / sws nir');
