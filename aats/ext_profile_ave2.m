...
pp  = csaps(r_mean,tau_aero_mean,aero_smoothing_para_high);
tau_aero_spline = fnval(pp,r_mean);
ext_coeff=-fnval(fnder(pp),r_mean);

pp  = csaps(r_mean,tau_aero_mean,aero_smoothing_para_low);
tau_aero_spline_low = fnval(pp,r_mean);
ext_coeff_low=-fnval(fnder(pp),r_mean);

ii=find (r_mean<=z_break);
tau_aero_spline(:,ii) = tau_aero_spline_low(:,ii);
ext_coeff(:,ii)=ext_coeff_low(:,ii);
...
if strcmp(Result_File,'ON')
    ...
    if strcmp(instrument,'AMES14#1_2002') | strcmp(instrument,'AMES14#1_2001') | strcmp(instrument,'AMES14#1_2000')
        ...
        elseif strcmp(site,'ACE-Asia') | strcmp(site,'Aerosol IOP')| strcmp(site,'ALIVE')
            ...
            fprintf(fid,'%s %g %g %g\r\n',    'AOD smoothing parameter (high/low/z_break):',aero_smoothing_para_high,aero_smoothing_para_low,z_break);
