 function [Q_ext,Q_scat,Q_bks,gasym,a,b]=mie2(x,m,N)
...
%     Matrix
% m - refractive index (i.e. 1.45 - 0.003i)
% N - summation index
...
% added Q_bks and gasym (C.F. Bohren,D.R. Huffmann, Absorption and Scattering of Light by Small Particles, Wiley Science Paperback Series, 1998, p. 120-122)
...
Q_bks=1./x.^2.*(abs(sum((n_vek*2+1).*((-1).^n_vek).*(a-b),2)).^2);

nvekm1(:,1:N-1)=n_vek(:,1:N-1);
coeff1=nvekm1.*(nvekm1+2)./(nvekm1+1);
coeff2=(2*nvekm1+1)./(nvekm1.*(nvekm1+1));
gasym=4./(x.^2.*Q_scat).*sum(coeff1.*real(a(:,1:N-1).*conj(a(:,2:N))+b(:,1:N-1).*conj(b(:,2:N)))+coeff2.*real(a(:,1:N-1).*conj(b(:,1:N-1))),2);

Q_ext=reshape(Q_ext,[MM NN]);
Q_scat=reshape(Q_scat,[MM NN]);
Q_bks=reshape(Q_bks,[MM NN]);
gasym=reshape(gasym,[MM NN]);
