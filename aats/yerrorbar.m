function hh = yerrorbar(axestype,xmin, xmax, ymin, ymax, x, y, l,u,symbol)
%loglogERRORBAR Error bar plot.
...
%   $Revision: 1.1 $  $Date: 2013/12/11 00:49:58 $
...
switch axestype
	case {'semilogy','linlin'}
      tee = .01*(xmax-xmin);  % make tee .01 x-distance for error bars
      xl = x - tee;
      xr = x + tee;
   case {'semilogx','loglog'}   
      tee = 0.15*log(xmax/xmin)/100;  % make tee .3 log(x-distance) for error bars
      xl = x*10^-tee;
      xr = x*10^tee;
end
