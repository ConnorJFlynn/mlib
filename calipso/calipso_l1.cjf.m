...
alt = (altitude<=30);
new_alt = [0.03:.03:30]';
new_z = interp1(altitude(alt),z1(alt,:),new_alt,'linear');
new_z = filter(ones(1,15)/15,1,new_z')';

X = 1e-4.*[1:9 10:5:80 100:100:1100];
...
zmap = new_z;
zmap(new_z<X(1)) = 1;
zmap(new_z>X(end)) = length(X);
for colr = 1:(length(X)-1)
  bin = (new_z>=X(colr))&(new_z<X(colr+1));
  zmap(bin)= colr;
end
figure; 
imagesc(lat1,new_alt,zmap);axis('xy')
colormap(cmap);
cb = colorbar;
set(cb,'ytick',[1:length(X)]);
for x = 1:length(X-1)
   yt_label{x} = sprintf('%1.1e',X(x));
end
for x = [2:9,11:24,26:33]
   tmp = yt_label{x};
   yt_label{x} = tmp(1:3);
end
yt_label{35} = [];
set(cb,'yticklabel',yt_label);
set(cb,'ygrid','on');
set(cb,'gridlinestyle','-');
