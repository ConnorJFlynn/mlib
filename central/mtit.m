...
% check input
	...
	if	ischar(varargin{1})||iscell(varargin{1})
		...
		figh=gcf;
		txt=varargin{1};
	elseif	any(ishandle(varargin{1}(:)))		&& ischar(varargin{2})
		vl(1:2)=false;
		figh=varargin{1};
		txt=varargin{2};
	else
...
% ...and title
		...
		set(th,'visible','on');
for i = 1:length(ah)
    set(ah(i),'LooseInset',get(ah(i),'TightInset'));
    opi(i) = {get(ah(i),'outerposition')};
end
for i = 1:length(ah)
    set(ah(i),'outerposition',opi{i}.*[1,.96,1,.96])
end
