function [P, S, mu, good] = rmad_bisec(X,Y,limits);
...
while ~done
   goods = sum(good);
   [m,b,r,sm,sb] = lsqbisec(X(good),Y(good));
   P = [m,b];
   dev(good) =  (polyval(P, X(good))-Y(good));
...
%    hold('on')
%    plot(X(~good),Y(~good),'kx',X(good),polyval(P,X(good)),'r-');
%    hold('off');
%    
