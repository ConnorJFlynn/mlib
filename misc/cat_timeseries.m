...
% if either is empty then the other is returned as "this"
if isempty(this) & ~isempty(that)
   this = that;
elseif ~isempty(that)& ~isempty(this)
   ...
   if isfield(this,'fname')
      if ~isfield(this,'file_num')
         ...
         fname = this.fname; if iscell(fname)&&length(fname)==1 fname = fname{1}; end
   ...
   for f = length(flds):-1:1
      catdim = find(size(this.(flds{f}))==length(init_time1)&size(that.(flds{f}))==length(init_time2),1,'last');
      if catdim ==1
