...
iwg_grid.time = [utime(1):(1./(24*60*60)):utime(end)]';
[ainb, bina] = nearest(utime,iwg_grid.time);
fields = fieldnames(iwg);
for f = 1:length(fields)
   field = fields{f};
   if ~strcmp(field,'time')&&~strcmp(field,'pname')&&~strcmp(field,'fname')
      iwg_grid.(field) = NaN(size(iwg_grid.time));
      iwg_grid.(field)(bina) = iwg.(field)(ii(ainb));
   end      
end

matdir = [iwg.pname, filesep,'..',filesep,'mats'];
if ~exist(matdir,'dir')
    mkdir(matdir);
