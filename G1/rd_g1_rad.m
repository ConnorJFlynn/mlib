function flight = rd_g1_rad(rad_file) 

if ~exist('rad_file','var')||~exist(rad_file,'file')
rad_file = getfullname_('*rad*.txt','tcap_g1','Select rad file');
end
...
figure; 
s(1)= subplot(2,1,1);
semilogy(serial2doy(flight.time), [flight.mfr500gnd_AD+0.068426, flight.mfr870gnd_AD+0.068145],'.');zoom('on')
[~,fname,ext] = fileparts(rad_file);
title(fname,'interp','none');
ylabel('AD counts');
legend('500 nm','870 nm');
s(2)= subplot(2,1,2);
semilogy(serial2doy(flight.time), [flight.mfr1625gnd_AD+4.2613],'.');zoom('on');
