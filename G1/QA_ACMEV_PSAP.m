...
psapo_grid.pname = strrep(psapo_grid.pname,'E:','H:');
flight = strtok(psapi.fname,'.');
...
Ba_B_scat = interp1(neph_grid.time, Ba_B_scat, psapo_grid.time,'nearest');
Ba_G_scat = 1e6.*(K1_green./Scat_K2).* Bs_G_psap;
Ba_G_scat = interp1(neph_grid.time, Ba_G_scat, psapo_grid.time,'nearest');
Ba_R_scat = 1e6.*(K1_red./Scat_K2).* Bs_R_psap;
Ba_R_scat = interp1(neph_grid.time, Ba_R_scat, psapo_grid.time,'nearest');

psapo_grid.Ba_B_Bond = psapo_grid.Ba_B_sm_Weiss - Ba_B_scat;
low_signal = Ba_B_scat<.1 | Ba_G_scat<.1;
psapo_grid.Ba_B_Bond(low_signal) = psapo_grid.Ba_B_sm_Weiss(low_signal);

psapo_grid.Ba_G_Bond = psapo_grid.Ba_G_sm_Weiss - Ba_G_scat;
low_signal = Ba_B_scat<.1 | Ba_R_scat<.1;
psapo_grid.Ba_G_Bond(low_signal) = psapo_grid.Ba_G_sm_Weiss(low_signal);

psapo_grid.Ba_R_Bond = psapo_grid.Ba_R_sm_Weiss - Ba_R_scat;
low_signal = Ba_R_scat<.1 | Ba_G_scat<.1;
...
iwg_file = [strrep(strrep(psapi.pname,'PSAP','IWG1'),'E:','H:'),filesep,'aaf.iwg1001s.g1.acmev.',flight,'.a2.txt'];
...
corr_STP_to_ambient = interp1(serial2doys(iwg_grid.time), corr_STP_to_ambient , serial2doys(main.t),'nearest');
Start_of_day = floor(min(main.t(good)));
Start_UTC = floor((main.t(good)-Start_of_day).*24*60*60);

data_in.Stop_UTC = Start_UTC + 4; info.Stop_UTC = ['seconds, stop time']; form.Stop_UTC = '%6.2f';
...
psap_ict_fname = ICARTTwriter_PSAP_ACMEV(datestr(min(main.t(good)),'yyyymmdd'),Start_UTC,data_in,info,form,rev,ICTdir);

psap_ict_in = ICARTTreader([ICTdir,psap_ict_fname]);
