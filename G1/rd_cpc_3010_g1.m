...
matdir = [cpcf.pname,'..',filesep,'mats',filesep];
if ~exist(matdir,'dir')
    mkdir(matdir);
...
if isafile(cpcf_mat)&&isafile(cpcf_grid_mat)
    ...
    cpcf_grid.time = [cpcf.time(1):(1./(24*60*60)):cpcf.time(end)]';
    [ainb, bina] = nearest(cpcf.time,cpcf_grid.time);
    ...
    for f = 1:length(fields)
        ...
        if ~strcmp(field,'time')&&~strcmp(field,'pname')&&~strcmp(field,'fname')
            ...
            cpcf_grid.(field)(bina) = cpcf.(field)(ainb);
        end
    end
