function aos = impact_aos_h_(aos);
...
aos.good_1um = submicron;
aos.good_10um = supmicron;
allNaNs = ~isfinite(aos.(fields{f}))|(aos.(fields{f})<-9);
...
%          NaNs = allNaNs(sub);
         ...
         if (sum(~NaNs)>2)&&((aos.time(sub(nind(end)))-aos.time(sub(nind(1))))>(1/48));
            %There seems to be a problem here in that we are extrapolating
            %over regions where neither the 1um or 10um measurements were
            %valid.
            ...
            big(sub(NaNs)) = aos.(fields{f})(sub(NaNs))>(2*max(aos.(fields{f})(sub(~NaNs))));
             ...
             aos.(['good',cut])(sub) = isfinite(aos.(fields{f})(sub));
         end
      end
   end
