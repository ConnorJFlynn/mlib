function [B,x_grid] = fill_img_nogap(A, x,x_grid); 
% [B,x_grid] = fill_img_nogap(A, x,x_grid); 
...
% If x_grid is not supplied, the grid runs from [min(x):interval:max(x)]
 % This function differs from "fill_img" in that after filling with nearest 
 % value matching the grid spacing, gaps are interpolated to the nearest
 % non-NaN value.
...
% 
skipped = all(isNaN(B));
B(:,skipped) = interp1(find(~skipped),B(:,~skipped)', find(skipped),'nearest','extrap')';
