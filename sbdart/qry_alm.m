...
if ~isfield(qry,'sza')&&~isfield(qry,'SZA')
   error('Must include SZA for ALM scan')
end
if ~isfield(qry,'WLBAER')&&~isfield(qry,'wlbaer')
   error('Must include WLBAER for ALM scan')
end
if ~isfield(qry,'QBAER')&&~isfield(qry,'qbaer')
   error('Must include QBAER for ALM scan')
end
if ~isfield(qry,'WBAER')&&~isfield(qry,'wbaer')
   error('Must include WBAER (ssa) for ALM scan')
end
if ~isfield(qry,'GBAER')&&~isfield(qry,'gbaer')
   error('Must include GBAER (asymmetry parameter) for ALM scan')
end

if ~isfield(qry,'nf')&&~isfield(qry,'NF')
   qry.NF=3;
end
