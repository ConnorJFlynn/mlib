function zrad_csphot_sasze(fig)
% zrad_csphot_sasze(fig)
...
try
   ...
   csphot_cldrad = read_cimel_cloudrad;
end
...
ppl_ij = find(aip_ppl.Nominal_Wavelength_nm_==ppl_wl & aip_ppl.time>sasze_.time(1) & aip_ppl.time<sasze_.time(end));
...
if isavar('fig')&&~isempty(fig)&&isgraphics(fig)
   if isavar('csphot_zrad')
      ...
      legend('csphot cld','csphot ppl','sasze/10')
   elseif isavar('csphot_cldrad')
      figure_(fig.Number+3); plot(csphot_cldrad.time, csphot_cldrad.A440nm(:),'o',...
         aip_ppl.time(ppl_ij), aip_ppl.zen_rad(ppl_ij),'x', ...
         sasze.time, sasze.vdata.(['zenith_radiance_',ze_filt])./10, '.'); dynamicDateTicks;
      % xlabel('time'); ylabel(csphot_zrad.vatts.zenith_sky_radiance_A.units,'interp','tex');
      legend('csphot A440','csphot ppl','sasze/10')

   else
      figure_(fig.Number+3); plot(aip_ppl.time(ppl_ij), aip_ppl.zen_rad(ppl_ij),'x', ...
         sasze.time, sasze.vdata.(['zenith_radiance_',ze_filt])./10, '.'); dynamicDateTicks;
      xlabel('time'); ylabel(csphot_zrad.vatts.zenith_sky_radiance_A.units,'interp','tex');
      legend('csphot ppl','sasze/10')
   end
