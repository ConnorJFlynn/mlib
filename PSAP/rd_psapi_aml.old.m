function [psapo_grid,psapo,psapi] = rd_psapi_aml(ins, spot_area, flow_cal)
% [psapo_grid,psapo,psapi] = rd_psapi_aml(ins);
% Parses a PSAP file from the AML test  containing "I" packets
...
% First, whip through the file just to count lines.  This will let up
% re-assign the size of the data block.
% Rewind file and read line by line.
...
if ~exist('ins','var') || ~exist(ins,'file')
    ins = getfullname_('psap_.csv','aaf_psap');
end
...
if exist(psapi_mat,'file')&&exist(psapo_mat,'file')&&exist(psapo_grid_mat,'file')
    ...
    if fid>0
        ...
        status_flag = hex2dec(A{18});
         fseek(fid,0,-1);
          [AA] = textscan(fid,'%s %s','delimiter','"'); BB= AA{2};
          fclose(fid);
        ...
        psapi.Ba_B = data_block(:,2); psapi.Ba_G = A{3}; psapi.Ba_R = A{4};
